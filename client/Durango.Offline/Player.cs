...
public class Player
{
	...
	public Player(string entityId, Connection connection, World world, PlayerContext context, bool isLocalPlayer)
	{
		...
		_connection.Recv(delegate(GetQuests msg, PacketHeader header)
		{
			...
			if (chapters != null)
			{
				...
				IEnumerable<IEnumerable<QuestToDo>> source = chapters.ChapterList.Select((Chapter chapter) => (chapter.Quests != null) ? chapter.Quests.Select(delegate(string q)
				{
	...
	private void HandleCheatMsg(string cheat, uint seq)
	{
		...
		string text = array[0];
		uint num6 = PlayerHash.ComputeStringHash(text);
		if (num6 <= 1194886160)
		{
			if (num6 <= 854911780)
			{
				if (num6 != 243880259)
				{
					if (num6 != 516436936)
					{
						if (num6 == 854911780 && text == "immortal")
						{
							goto IL_08b4;
						}
					}
					else if (text == "grazed_to_pet")
					{
						string id4 = array[1];
						List<Messages.Pet> pets = _world.GetPets();
						List<Messages.Pet> grazedPets = _world.GetGrazedPets();
						int index2 = grazedPets.FindIndex((Messages.Pet p) => p.EntityId == id4);
						pets.Add(grazedPets[index2]);
				...
				else if (text == "remove_pet")
				{
					string id3 = array[1];
					List<Messages.Pet> pets2 = _world.GetPets();
					int num7 = pets2.FindIndex((Messages.Pet p) => p.EntityId == id3);
					if (num7 != -1)
					{
						pets2.RemoveAt(num7);
						_world.BroadCast(new Messages.Pets
			...
			else
			{
				if (num6 != 910182291)
				{
					if (num6 != 1070481844)
					{
						if (num6 != 1194886160 || !(text == "it"))
						{
							goto IL_08dd;
						}
						InventoryUpdated msg = default(InventoryUpdated);
					...
					else
					{
						...
						int num8 = grazedPets2.FindIndex((Messages.Pet p) => p.EntityId == id2);
						if (num8 != -1)
						{
							grazedPets2.RemoveAt(num8);
		...
		if (num6 <= 3596982256u)
		{
			if (num6 != 1550195791)
			{
				if (num6 != 1854840628)
				{
					if (num6 != 3596982256u || !(text == "it_color"))
					{
						goto IL_08dd;
					}
					if (array.Length >= 2)
					{
						...
						int num9 = inventoryItems3.FindIndex((Item it) => it.Id == itemId2);
						if (num9 == -1)
						{
							return;
						}
						Item item2 = inventoryItems3[num9];
				...
				else
				{
					...
					messageBox.Show(T._("<em>{0}</em>{0:-을} 귀속하시겠습니까?", rein.PetName), T._("[icon=icon_make_alert] 한 번 귀속한 동물은 귀속해제 전까지 판매하거나 다른 사람에게 양도할 수 없습니다."), delegate(int index)
					{
						if (index == 0)
						{
							float num11 = UnityEngine.Random.Range(15420f, 38219f);
							Gauge gauge = new Gauge(num11, 0f, new GaugeNode[1]
							{
								new GaugeNode
								{
									Time = 0.0,
			...
			else
			{
				...
				int index3 = pets3.FindIndex((Messages.Pet p) => p.EntityId == id);
				grazedPets3.Add(pets3[index3]);
				_world.BroadCast(new GrazedPets
				{
		...
		else
		{
			...
			if (num6 != 3753552150u)
			{
				if (num6 != 3893112696u || !(text == "m"))
				{
					goto IL_08dd;
				}
				if (array.Length >= 3)
				{
					Teleported msg2 = default(Teleported);
					msg2.Type = TeleportType.Unknown;
					if (int.TryParse(array[1], out msg2.Tile.x) && int.TryParse(array[2], out msg2.Tile.y))
					{
						Send(msg2);
					}
				}
			...
			else
			{
				...
				if (array.Length >= 4)
				{
					int x = array[1].ToInt();
					int y = array[2].ToInt();
					int num10 = array[3].ToInt();
					_world.AddNatural(new Point2(x, y), (ushort)num10);
				}
			}
		}
		goto IL_08ec;
		IL_08b4:
		AddOns? addons;
		AppearArtifact? appearArtifact = Cheats.MakeAppearArtifact(array, out addons);
	...
	public void HandleTouchMsg(Messages.Touch touch, uint seq)
	{
		...
		else if (DataHelper.IsNaturalObject(touch.EntityType))
		{
			...
			if (flag)
			{
				...
				Touched touched = default(Touched);
	...
	public Item? GenItem(string prototypeId, int level, Result result)
	{
		...
		else
		{
			...
			ItemIconTex.TryGetDefaultColor(itemPrototype.ColorG, out var _, hashCode, Color.gray);
			ItemIconTex.TryGetDefaultColor(itemPrototype.ColorB, out var _, hashCode, Color.black);
	...
	public void BackToStableIsland(int slot, string id)
	{
		...
		int islandPort = Server.GetIslandPort();
		obj.GatewayUrlRoot = "http://127.0.0.1:" + islandPort;
		obj.LocalPlayer = Json.Write(_context);
	...
	private void SendBowWeaponAttack()
	{
		...
		switch (battleAction.Data.Id)
		{
		case "ranged_bow_default_a":
			_damageMsg.Value = (int)CalcWeaponDamage("normal");
			break;
		case "ranged_bow_default_b":
			_damageMsg.Value = (int)CalcWeaponDamage("normal");
			break;
		case "ranged_bow_default_c":
			_damageMsg.Value = (int)CalcWeaponDamage("normal");
			break;
		case "ranged_crossbow_default":
			_damageMsg.Value = (int)CalcWeaponDamage("normal");
			break;
		case "ranged_bow_aimedshot":
		case "ranged_crossbow_aimedshot":
			_damageMsg.Value = (int)CalcWeaponDamage("strong");
			break;
		case "ranged_bow_quickshot":
		case "ranged_crossbow_quickshot":
			OnUseFastAttack(3);
			return;
		case "melee_tackle":
			_damageMsg.Value = (int)CalcWeaponDamage("tackle");
