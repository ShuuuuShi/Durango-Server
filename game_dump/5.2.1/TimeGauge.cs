...
public class TimeGauge : MonoBehaviour
{
	...
	private void Update()
	{
		...
		float num = (float)(Connections.Frontend.GetPredictedServerTime() % (double)totalTimeInSec);
	...
	public static bool CheckTime(float begin, float end)
	{
		...
		if (num <= num2)
		{
			if (_normalizedTime >= num)
			{
				return _normalizedTime < num2;
			}
			return false;
		}
		if (!(_normalizedTime >= num))
	...
	public static float GetNormalizedTimeForDayNight()
	{
		...
		if (IsSunUp)
		{
			return (((!(num2 > (float)SunriseBegin)) ? 24f : 0f) + num2 - (float)SunriseBegin) / (float)num;
		}
		return (((!(num2 > (float)SunsetEnd)) ? 24f : 0f) + num2 - (float)SunsetEnd) / (float)(24 - num);
	}
	...
	{
		return DaysPassedWhile(Connections.Frontend.GetPredictedServerTime() - beginningUnixTime);
	}
