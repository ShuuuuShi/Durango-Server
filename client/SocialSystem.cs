...
public class SocialSystem : GameSystem<SocialSystem>
{
	...
	private void ReceiveChatLog(ChatLogs msg, ChannelType type)
	{
		for (int i = 0; i < msg.Logs.Length; i++)
		{
			...
			chatStruct.TranslationOn = AutoTranslation;
			ChatStruct chatStruct2 = chatStruct;
			if (message_.Speaker.HasValue)
			{
				chatStruct2.Name = message_.Speaker.Value.Name;
			}
			_chattingList.Add(chatStruct2);
		}
	...
	private void OnSay(SayInExclusiveChannel msg, PacketHeader header)
	{
		...
		{
			chatStruct2.Name = msg.Message.Speaker.Value.Name;
		}
		if (msg.Message.Body is RadioNotice)
		{
			if (GameManager.Region.IsPvpIsland())
			{
				chatStruct2.NoBubble = true;
			}
	...
	private void OnSayConversation(SayInConversation msg, PacketHeader header)
	{
		...
		conversation.AddMessage(chatStruct2);
	}
	...
	public void Say(string message, bool isDictation = false)
	{
		...
		if (!string.IsNullOrEmpty(message))
		{
			...
			Connection connection = ((GameManager.ClusterMode != 0) ? Connections.Frontend : Connections.Radiotower);
			connection.Send(msg);
			AddContinuousChatTime(channelType);
		}
	}
	...
	public void SystemSay(object body, ChannelType? channelType = null, string conversationId = null)
	{
		...
		if (IsAllowedChannel(channelType.Value))
		{
			...
			if (channelType == ChannelType.Conversation)
			{
				SayInConversation sayInConversation = default(SayInConversation);
	...
	{
		return Social.FollowingEntityIds != null && Social.FollowingEntityIds.IndexOf(entityId) != -1;
	}
	...
	{
		return (Social.FriendEntities == null) ? Shared.Player.FriendType.Invalid : Social.FriendEntities.Get(entityId, Shared.Player.FriendType.Invalid);
	}
	...
	{
		return filterType == ChatFilterType.Clan || filterType == ChatFilterType.ClanWar;
	}
