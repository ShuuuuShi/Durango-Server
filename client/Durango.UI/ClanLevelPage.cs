...
public class ClanLevelPage : ClanMenuPage, IUIInitializable
{
	...
	private void InitLevelRewards()
	{
		...
		Dictionary<int, ClanLevelReward> levelRewards = Singleton<ClanYaml>.Instance.LevelRewards;
		foreach (KeyValuePair<int, ClanLevelReward> item in levelRewards)
		{
	...
	private void SetLevel(int level)
	{
		...
		for (int i = 0; i < nodes.Count; i++)
		{
			KeyValuePair<int, ClanLevelReward> keyValuePair = _levelRewardInfos[i];
			ClanLevelInfoNode component = nodes[i].GetComponent<ClanLevelInfoNode>();
			component.Disabled = keyValuePair.Key > level;
		}
