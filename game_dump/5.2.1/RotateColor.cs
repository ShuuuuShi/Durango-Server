...
public class RotateColor : UISprite
{
	...
	public override void OnFill(UIGeometry.Arguments arguments)
	{
		...
		float num = ((!Application.isPlaying) ? Time.realtimeSinceStartup : Time.time) * _speed % ((size.x + size.y) * 2f);
		float num2 = SideToRadian(size, num);
		float r = SideToRadian(size, num - _spotSize * 0.5f);
		float r2 = SideToRadian(size, num + _spotSize * 0.5f);
		BetterList<Color> cols = arguments.cols;
		for (int i = 0; i < cols.size; i++)
		{
			Vector3 vector2 = geometry.verts[i] - vector;
			float r3 = Mathf.Atan2(vector2.y, vector2.x);
			float t = ((!(RadianDiff(r3, num2) < 0f)) ? (RadianDiff(r3, r2) / RadianDiff(num2, r2)) : (RadianDiff(r3, r) / RadianDiff(num2, r)));
			float a = cols[i].a;
