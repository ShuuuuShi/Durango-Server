...
public class QuestSystem : GameSystem<QuestSystem>
{
	...
	public IEnumerable<Category> VisibleCategories => from pair in _questCategories
		where pair.Key != EpicCategory
		select pair.Value;
	...
	private void OnUpdateQuestScoreReward(string category, QuestScoreReward[] rewards)
	{
		...
		{
			QuestScoreReward questScoreReward = rewards[i];
			if (questScoreReward.State == QuestScoreRewardState.Available)
			{
				hasScoreReward = true;
				break;
			}
	...
	public float GetChapterProgress(Chapter chapter)
	{
		...
		int num = chapter.Quests.Count((string questId) => quests.Any((QuestToDo quest) => quest.Id == questId && quest.Finished));
		return (float)num / (float)chapter.Quests.Length;
	}
	...
	public QuestToDo GetEpicQuest(string questId)
	{
		...
		List<QuestToDo> cachedQuestList = category.GetCachedQuestList();
		return cachedQuestList.Find((QuestToDo x) => x.Id == questId);
	}
}
