...
public class ResistanceInfoPopup : TooltipBase
{
	...
	protected override void FillData()
	{
		...
		foreach (KeyValuePair<Biome, Derived> item in Singleton<Constants>.Instance.Resistance.TypeByBiome)
		{
			...
			ResistanceInfo component = _itemPool.GetNext().GetComponent<ResistanceInfo>();
			component.Set(key, typeName, iconName, resistanceLevel, currentAndMaxResistanceExp.Item1, currentAndMaxResistanceExp.Item2, resistanceExpCap.ExpRate, biome == key);
			if (key != biome)
			{
				continue;
			}
			...
			uIEventListener.onClick = (UIEventListener.VoidDelegate)Delegate.Combine(uIEventListener.onClick, (UIEventListener.VoidDelegate)delegate(GameObject go)
			{
				SyncString title = new SyncString(delegate(out string text, out float period)
				{
					...
					string format = T._("신체 저항 레벨의 경험치는 일일 획득량이 제한됩니다.\n초기화까지 남은 시간 {0}");
					SyncString.UpdateRemainTimeMsg(resistanceExpCap2.ExpiresAt, format, out text, out period, string.Empty);
				});
