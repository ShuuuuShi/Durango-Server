...
public class EquipWidgetBase : UIWidget, IUIInitializable
{
	...
	{
		return SelectedEquipPreset != EquipSlotType.Invalid && SelectedSlot != EquipSystem.Slot.Invalid && !_waitForChangingPreset && !GameSystem<EquipSystem>.Instance().IsLockedPreset(SelectedEquipPreset);
	}
	...
	{
		EquipSystem.EquipPreset equipPreset = GameSystem<EquipSystem>.Instance().GetEquipPreset(EquipSlotType.Avatar);
