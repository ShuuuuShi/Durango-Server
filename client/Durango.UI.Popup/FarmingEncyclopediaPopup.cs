...
public class FarmingEncyclopediaPopup : TooltipBase
{
	...
	protected override void FillData()
	{
		...
		if (array != null)
		{
			...
			for (int i = 0; i < array2.Length; i++)
			{
				...
				FarmingMasteryWidget component = _masteries.Nodes.GetNext().GetComponent<FarmingMasteryWidget>();
				component.Set(keyValuePair.Key, value, keyValuePair.Value);
			}
		}
	...
	private IEnumerator CoProgressAnimation()
	{
		float ratio = 0f;
		float speedRatio = 0f;
		while (ratio < _expRatio)
		{
			_progressSprite.fillAmount = ratio;
			speedRatio += Time.deltaTime * 2f;
			ratio += Time.deltaTime * speedRatio;
			yield return null;
		}
		_progressSprite.fillAmount = _expRatio;
	}
