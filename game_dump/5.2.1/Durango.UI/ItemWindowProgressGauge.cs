...
public class ItemWindowProgressGauge : ProgressGauge
{
	...
	private void Set(ref IconItemPair icon, ItemData item, Vector3 pos)
	{
		...
		UIEventListener.Get(icon.Icon.gameObject).onPress = OnItemIconTouch;
	...
	{
		IconItemPair obj = new IconItemPair
		{
			Icon = base.gameObject.AddChild(_itemIcon.gameObject).GetComponent<ItemIconTex>()
		};
		obj.Transform = obj.Icon.transform;
		obj.Upper = obj.Transform.Find("Upper").GetComponent<UISprite>();
		obj.Plus = obj.Transform.Find("Plus").GetComponent<UISprite>();
		return obj;
	}
