...
public class BoneLookAtTarget : MonoBehaviour
{
	...
	private GameObject FindRandomTarget()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (!(gameObject == base.gameObject))
			{
				...
				if (!(component == null))
				{
					float value = Vector3.Magnitude(collider.transform.position - position);
	...
	{
		if (!(PlayerBehavior.LocalPlayer == null) && _character.IsAlive && _character.WillBeRendered)
		{
			return (PlayerBehavior.LocalPlayer.CurrentPosition - _character.CurrentPosition).sqrMagnitude > _cullDistance * _cullDistance;
		}
		return true;
	}
	...
	private void ProcessHeadTransformation()
	{
		...
		if (!(_globalRatio <= 0f))
		{
			for (int num = _affectedBones.Count - 1; num >= 0; num--)
			{
	...
	private Vector3 LimitTargetPos(Vector3 forward, Vector3 targetPos, Vector3 bodyPos)
	{
		...
		if (!(0f - _yawLimitDeg <= num3) || !(num3 <= _yawLimitDeg))
		{
			...
			Vector3 vector = Maths.CalcDirectionFromYaw(num2 + num3);
