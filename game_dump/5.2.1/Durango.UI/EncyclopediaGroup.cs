...
public class EncyclopediaGroup : UIBase
{
	...
	private void InitializeTab()
	{
		...
		if (role != Role.Tutorial && role != Role.Safehouse && GameManager.ClusterMode == Mode.Online)
		{
			...
			foreach (EncyclopediaType key in array)
			{
	...
	private void OnMemoCollect(MemoType type, int index)
	{
		...
		{
			string text = ((20 >= memoText.Length) ? memoText : (memoText.Substring(0, 20) + "..."));
			UIManager.Alarm.ShowNotify(memoTitle + ": " + text, "alarm_memo", major: true, 1.8f, delegate
			{
				Open(type, index);
			});
		}
