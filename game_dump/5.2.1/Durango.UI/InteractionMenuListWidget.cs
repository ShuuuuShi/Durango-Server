...
public class InteractionMenuListWidget : InteractionMenuListWidgetBase
{
	...
	private void RepositionMenuItems()
	{
		...
		for (int k = 0; k < count; k++)
		{
			...
			float num9 = Mathf.Repeat(VisibleStartDegree + (float)num8 * num3, 360f) * ((float)Math.PI / 180f);
	...
	private void RepositionMenuMultipleLines(float radius, int eraseWidth)
	{
		...
		for (int visibleCountPerPage = base.VisibleCountPerPage; i < visibleCountPerPage; i++)
		{
			...
			if (num4 != -1)
			{
				float num5 = ((Menus[num4].Type != 0) ? InteractionMenuListWidgetBase.MinorScale : InteractionMenuListWidgetBase.MajorScale);
			...
			if (num4 != -1)
			{
				float num7 = ((Menus[num4].Type != 0) ? InteractionMenuListWidgetBase.MinorScale : InteractionMenuListWidgetBase.MajorScale);
