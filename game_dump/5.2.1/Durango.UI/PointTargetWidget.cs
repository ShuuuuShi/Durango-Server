...
public class PointTargetWidget : MonoBehaviour
{
	...
	public bool Tick()
	{
		...
		Vector3 normalized = ((Vector3)Singleton<ScreenAreaManager>.Instance().GetBorder(num2 + 1f) - vector4).normalized;
		normalized = new Vector3(normalized.y, 0f - normalized.x);
		Vector3 vector5 = Singleton<ScreenAreaManager>.Instance().GetBorder(num2);
		vector5 -= normalized * 50f;
		Vector2 vector6 = vector5 - zero;
		if (float.IsNaN(vector5.x) || float.IsNaN(vector5.y))
		{
			return false;
		}
		if (vector3.sqrMagnitude < vector6.sqrMagnitude)
		{
