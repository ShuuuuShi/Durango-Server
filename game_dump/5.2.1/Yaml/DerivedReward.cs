...
public class DerivedReward
{
	...
	public string ToDescription()
	{
		...
		case RewardType.DynamicRecipe:
		{
			...
			{
				return string.Format("<ref>ui://Recipe/Crafting/{1},{0}</ref>", recipe.Name, RecipeId);
			}
			return T._("{0} 제작법 획득", (recipe != null) ? recipe.Name : RecipeId);
		}
		...
		case RewardType.DynamicBlueprint:
		{
			...
			{
				return string.Format("<ref>ui://Recipe/Building/{1},{0}</ref>", blueprint.Name, BlueprintId);
			}
			return T._("{0} 건설법 획득", (blueprint != null) ? blueprint.Name : BlueprintId);
		}
