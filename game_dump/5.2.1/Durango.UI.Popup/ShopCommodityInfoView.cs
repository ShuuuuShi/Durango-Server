...
public class ShopCommodityInfoView : UIWidget, RectLayout.ICompatible
{
	...
	private void SetPurchaseLimitLabel(Durango.Logic.Shop.Commodity commodity)
	{
		...
		else if (purchaseLimit.PeriodicCountsLimit.Counts > 0)
		{
			...
			else
			{
				...
				string text3 = ((purchaseLimit.PeriodicCountsLimit.Counts <= 1) ? string.Empty : T._(" ({0}남음)", num2));
				text = T._("{0}일 {1}회", purchaseLimit.PeriodicCountsLimit.Days, purchaseLimit.PeriodicCountsLimit.Counts) + text3;
			}
		}
		else if (purchaseLimit.PeriodicLimit.Days > 0)
		{
			if (commodity.CommodityInfo.PeriodicPurchasableAt.HasValue)
			{
				string text4 = TimedeltaFormatter.Format(commodity.CommodityInfo.PeriodicPurchasableAt.Value - Connections.Frontend.GetPredictedServerTime(), 2, "hour");
				text = T._("{0} 후 구매가능", text4);
			}
			else
			{
	...
	private static bool GetCaptionText(Durango.Logic.Shop.Commodity commodity, out string text, out string detailUri)
	{
		...
		if (string.IsNullOrEmpty(text))
		{
			return !string.IsNullOrEmpty(detailUri);
		}
		return true;
	}
