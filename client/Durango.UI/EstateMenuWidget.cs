...
public class EstateMenuWidget : MonoBehaviour
{
	...
	private static string ToString(OwnerType type, Menu menu)
	{
		...
		case Menu.Licenses:
			return T._("권한 조정");
		case Menu.Remove:
			return (type != OwnerType.ClanEstate) ? T._("사유지 권한 포기") : T._("부족 영토 권한 포기");
		case Menu.Extend:
			return (type != OwnerType.ClanEstate) ? T._("사유지 기간 연장") : T._("부족 영토 기간 연장");
		case Menu.ClanInfo:
			return T._("부족 정보");
		case Menu.Bookmark:
			return T._("섬 즐겨찾기");
	...
	public void MenuLoadEnd()
	{
		...
		if (estateMenuItem != null && _licenses.HasValue)
		{
			...
			{
				double num = value.DepositRunsOutAt.Value - predictedServerTime;
				flag2 = num < 86400.0;
			}
		}
