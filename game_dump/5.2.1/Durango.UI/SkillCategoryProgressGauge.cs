...
public class SkillCategoryProgressGauge : UIWidget
{
	...
	public void Set(Shared.Skill.Category category)
	{
		...
		if (max > 0)
		{
			...
			if (GameSystem<SkillSystem>.Instance().GetCategoryLevel(_category.Type) < level)
			{
				SetMainWidget(_readyToResearchWidget);
				int num = SingletonDict<Shared.Skill.Category, Yaml.SkillCategory>.Get(_category.Type).ResearchTimes.Get(_category.Level, 0);
	...
	private static Pair<float, Durango.Logic.StatusEffect> GetResearchTimeReducer(int level)
	{
		...
		if (level <= 50)
		{
			foreach (Durango.Logic.StatusEffect item2 in GameSystem<StatusEffectSystem>.Instance().GetStatusEffects().List)
			{
	...
	private void UpdateResearchingTimer()
	{
		...
		if (num2 > 0.0)
		{
			...
			{
				int num3 = SingletonDict<Shared.Skill.Category, Yaml.SkillCategory>.Get(_category.Type).ResearchTimes.Get(_category.Level, 0);
	...
	private void OnClickSkipResearch()
	{
		...
		UIManager.MessageBox.ShowPayConfirm(num, Currency.Gem, T._("<em>{0}</em>{0:-을} 즉시 완료 하시겠습니까?", Durango.Logic.Skill.Util.CategoryLocalizeName(_category.Type)), delegate(bool ok)
		{
			if (ok)
			{
