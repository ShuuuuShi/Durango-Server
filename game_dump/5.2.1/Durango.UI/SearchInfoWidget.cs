...
public class SearchInfoWidget : MonoBehaviour
{
	...
	public void Set(SearchOption option)
	{
		...
		if (!string.IsNullOrEmpty(option.Prototype))
		{
			...
			{
				nodes.GetNext().GetComponent<SearchInfoItemNode>().Set(string.Format("{0}: {1}", T._("원형"), list[0].Name), delegate
				{
		...
		{
			nodes.GetNext().GetComponent<SearchInfoItemNode>().Set(string.Format("{0}: {1}", T._("검색"), option.SearchKeyword), delegate
			{
		...
		{
			nodes.GetNext().GetComponent<SearchInfoItemNode>().Set(T._("[icon={0}] {1}", Durango.Logic.Item.Inventory.GetIcon(Currency.TStone), string.Format("{0} - {1}", (!price.Min.HasValue) ? string.Empty : price.Min.Value.ToString("N0", T.Culture), (!price.Max.HasValue) ? string.Empty : price.Max.Value.ToString("N0", T.Culture)).Trim()), delegate
			{
		...
		{
			nodes.GetNext().GetComponent<SearchInfoItemNode>().Set(("Lv: " + ((!level.Min.HasValue) ? string.Empty : level.Min.ToString()) + " - " + ((!level.Max.HasValue) ? string.Empty : level.Max.ToString())).Trim(), delegate
			{
		...
		{
			nodes.GetNext().GetComponent<SearchInfoItemNode>().Set(elem2.GetName(), delegate
			{
		...
		{
			nodes.GetNext().GetComponent<SearchInfoItemNode>().Set(elem.GetName(), delegate
			{
