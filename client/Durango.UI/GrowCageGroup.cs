...
public class GrowCageGroup : UIBase
{
	...
	private void OnUpdateCage(GrowCage? prev, GrowCage? current)
	{
		...
		Messages.Pet[] array = data2;
		for (int i = 0; i < array.Length; i++)
		{
			Messages.Pet pet = array[i];
			Messages.Pet[] array2 = data;
			for (int j = 0; j < array2.Length; j++)
			{
				Messages.Pet pet2 = array2[j];
				if (pet.EntityId == pet2.EntityId)
				{
					if (pet2.Statistics.Level < pet.Statistics.Level)
					{
						string comment = T._("{0:이} {1:이} 되었습니다.", pet.GetPetName(), LocalizeUtil.FormatLevel(pet.Statistics.Level));
		...
		PetManager.GetPetList(delegate(PetsInfo? petsInfo)
		{
			...
			if (petsInfo.HasValue)
			{
				...
				else
				{
					...
					selectPetPopup.SetTitle(T._("축사에 동물을 넣겠습니까?")).SetCapacity(cage.Value.Size - cage.Value.RemainSize, cage.Value.Size).SetList(list)
