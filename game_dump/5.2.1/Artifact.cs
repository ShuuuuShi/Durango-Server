...
public sealed class Artifact : ImmovableBase
{
	...
	public string LocalizedName
	{
		get
		{
			if (ArtifactState.ChangedName == null)
			{
				if (Blueprint == null)
				{
					return string.Empty;
			...
			return ArtifactState.ChangedName;
		}
	}
	...
	public Vector2 CenterTile => base.WorldTile.ToVector2() + Size.ToVector2() * 0.5f;
	...
	private void UpdateCollider()
	{
		...
		if (Floor.HasValue && height > 1 && GetArtifactComponent<Stairs>() == null)
		{
			...
			_heightCollider.transform.localPosition = Vector3.up * 200f;
	...
	private void GroundSiteObjectLoaded(UnityEngine.Object asset)
	{
		...
		for (int j = 0; j < size.y; j++)
		{
			...
			{
				Transform obj = array[j * size.x + k].transform;
				obj.parent = transform;
				obj.localPosition = new Vector3(k, 0f, j) * 200f;
			}
		}
	}
	...
	private void ScaffoldingObjectLoaded(UnityEngine.Object asset)
	{
		...
		for (int j = 0; j < size.y; j++)
		{
			for (int k = 0; k < size.x; k++)
			{
				...
				obj.parent = transform;
				obj.localPosition = new Vector3(k, 0f, j) * 200f;
				obj.localRotation = Quaternion.Euler(0f, 90 * random.Next(4), 0f);
			}
		}
	}
	...
	public void SetArtifactState(ArtifactState state, double eventTime)
	{
		...
		bool num = RepairTimerUpdate();
		bool flag = CrackTimerUpdate();
		if (!num && !flag && ArtifactTimer != null)
		{
			ArtifactTimer.Stop();
		}
	...
	private bool IsLocalPlayerInside()
	{
		...
		Point2 size = Size;
		if (currentTile.x >= base.WorldTile.x && currentTile.y >= base.WorldTile.y && currentTile.x < base.WorldTile.x + size.x)
		{
			return currentTile.y < base.WorldTile.y + size.y;
		}
		return false;
	}
	...
	private void UpdateArtifactTimer(string subject, float duration, float ratio, bool isCraterTimer = false)
	{
		if (ArtifactTimer == null || ArtifactTimer.IsStop)
		{
			...
			if (isCraterTimer)
			{
				Durango.Logic.Timer.Timer.Play<CraterProgressGauge>(ArtifactTimer).SetTarget(base.gameObject, new Vector3(Size.x, 0.5f, Size.y) * 200f);
			}
			else
			{
				Durango.Logic.Timer.Timer.Play<TimerProgressGauge>(ArtifactTimer).SetTarget(base.gameObject, new Vector3(Size.x, 1f, Size.y) * 200f * 0.5f);
			}
		}
		else
		{
	...
	private void PostprocessTimeUpdate()
	{
		...
		if (PostProcessTimer == null || PostProcessTimer.IsStop)
		{
			...
			Durango.Logic.Timer.Timer.Play<TimerProgressGauge>(PostProcessTimer).SetTarget(base.gameObject, new Vector3(Size.x, (float)Height * 2f, Size.y) * 200f * 0.5f);
		}
		else
		{
	...
	public bool CheckEstatePermission(EstateInfo locatedEstate, Shared.Player.FriendType friendTypeForMe)
	{
		...
		case OwnerType.PersonalPlayer:
			...
			return HasEnoughtAccessRights((value.ForFriends != null) ? value.ForFriends.Get(friendTypeForMe, Shared.Estate.AccessRights.None) : Shared.Estate.AccessRights.None, intuitiveAccessRights);
		case OwnerType.ClanEstate:
		{
			...
			return HasEnoughtAccessRights(value.ForClanMembers.Get(roleId, Shared.Estate.AccessRights.None), intuitiveAccessRights);
		}
		default:
			return false;
		}
	...
	public bool CheckArtifactPermission(EstateInfo locatedEstate, Shared.Player.FriendType friendTypeForMe)
	{
		...
		case OwnerType.PersonalPlayer:
			...
			{
				return value.Friends.Get(friendTypeForMe, defaultValue: false);
			}
			return false;
		case OwnerType.ClanEstate:
		{
	...
	{
		foreach (string text in blueprintComponents)
		{
			if (text == "Port" || text == "Crack")
			{
				return false;
			}
		}
