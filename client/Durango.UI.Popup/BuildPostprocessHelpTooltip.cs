...
public class BuildPostprocessHelpTooltip : TooltipBase
{
	...
	private void UpdatePortraitsLayout()
	{
		...
		for (int i = 0; i < _portraits.Count; i++)
		{
			UIWidget component = _portraits[i].GetComponent<UIWidget>();
			component.SetPosition(vector + GetPortraitOffset(baseSize, num2, i), 0f, 1f);
		}
		UIWidget component2 = _buttonHelp.GetComponent<UIWidget>();
		component2.SetPosition(vector + GetPortraitOffset(baseSize, num2, _portraits.Count), 0f, 1f);
	}
