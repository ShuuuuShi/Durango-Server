...
public class ConcertTrackWidget : UIWidget
{
	...
	private void SetPlayer(PlayerInfo info)
	{
		...
		for (int i = 0; i < isMeObjects.Length; i++)
		{
			isMeObjects[i].SetActive(active);
		}
		UpdatePlayerWidgetState();
		if (info == null || !info.Valid)
		{
			if (_track != null && !string.IsNullOrEmpty(_track.PlayerId))
			{
				...
				isMeObjects = _emptyPlayerObject;
				for (int i = 0; i < isMeObjects.Length; i++)
				{
					isMeObjects[i].gameObject.SetActive(value: false);
				}
			}
			else
			{
				...
				for (int i = 0; i < isMeObjects.Length; i++)
				{
					isMeObjects[i].gameObject.SetActive(value: true);
				}
			}
		}
		else
	...
	private void SetMusicName(string musicName)
	{
		...
		for (int i = 0; i < concertHostObjects.Length; i++)
		{
			concertHostObjects[i].SetActive(flag);
		}
