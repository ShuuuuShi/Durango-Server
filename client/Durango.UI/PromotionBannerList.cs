...
public class PromotionBannerList : MonoBehaviour, IUIInitializable, RectLayout.ICompatible
{
	...
	void IUIInitializable.Init()
	{
		...
		{
			PromotionBannerWidget component = obj.GetComponent<PromotionBannerWidget>();
			component.Pressed += OnLinkPress;
		});
	...
	public bool Set(IList<PromotionLink> promotionLinks)
	{
		...
		for (int size = KUtility.GetSize(promotionLinks); i < size; i++)
		{
			...
			{
				PromotionBannerWidget component = _scroll.Nodes.GetNext().GetComponent<PromotionBannerWidget>();
				component.Set(promotionLink);
			}
		}
