...
public class LinkedPrefabs
{
	...
	private static string GetUIPrefabName(string name)
	{
		...
		return (!name.EndsWith("_PC", StringComparison.InvariantCultureIgnoreCase)) ? name : name.Substring(0, name.Length - 3);
	}
	...
	private void InitializeObject(GameObject obj)
	{
		...
		if (component != null)
		{
			...
			object[] customAttributes = type.GetCustomAttributes(typeof(UriAttribute), inherit: false);
