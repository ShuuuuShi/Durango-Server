...
public class ArchipelagoMissionSystem : GameSystem<ArchipelagoMissionSystem>
{
	...
	public void OnCurrentArchipelagoTodos(CurrentArchipelagoTodos packet, PacketHeader header)
	{
		...
		if (packet.CurrentTodos.HasValue)
		{
			...
			for (int i = 0; i < todos.Length; i++)
			{
				...
				ArchipelagoToDo archipelagoToDo2 = new ArchipelagoToDo();
				archipelagoToDo2.Key = archipelagoToDo.Id;
				archipelagoToDo2.CurrentProgress = archipelagoToDo.Progress;
				archipelagoToDo2.TargetProgress = archipelagoToDo.GoalCount;
				archipelagoToDo2.LocalText = ((toDoContents != null) ? toDoContents.Subject.ToString() : string.Empty);
				archipelagoToDo2.Point = toDoContents?.Point ?? 0;
				ArchipelagoToDo item = archipelagoToDo2;
	...
	public string GetNextRegion()
	{
		...
		for (int i = 0; i < KUtility.GetSize(includedRegions); i++)
		{
			...
			{
				return (i != includedRegions.Length - 1) ? includedRegions[i + 1].Id : null;
			}
		}
		return null;
	}
