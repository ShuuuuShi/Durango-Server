...
public class PlayerPreviewPage : UIWidget
{
	...
	private void OnButtonClicked()
	{
		...
		if (GameSystem<PlayerSelectionSystem>.Instance().PlayerSlotExceeded)
		{
		...
		UIManager.MessageBox.Show(T._("캐릭터를 삭제하시겠습니까?"), delegate(bool ok)
		{
			if (ok)
			{
				OnRequestDeletePlayer();
	...
	{
		UIManager.MessageBox.Show(T._("보유한 [B82E2E]듀랑고 코인[-] 및 상점 보관함의 [B82E2E]모든 아이템들[-]을 잃게 됩니다.\n그래도 삭제하시겠습니까?"), delegate(bool ok)
		{
			if (ok)
			{
				UIManager.Popup.LoadingRing.AttachToWidget(_button.gameObject);
