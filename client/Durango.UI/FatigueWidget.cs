...
public class FatigueWidget : MonoBehaviour
{
	...
	private void OnUpdateFatigue()
	{
		...
		for (int i = 0; i < fatigueVelocities.Count; i++)
		{
			...
			{
				FatigueMomentum component = nodes.GetNext().GetComponent<FatigueMomentum>();
				component.Set(fatigueVelocity2);
			}
		}
		...
		{
			float detail = item.GetDetail(EffectType.Survival, "fatigue");
			if (Mathf.Abs(detail) > 0f)
			{
				FatigueMomentum component2 = nodes.GetNext().GetComponent<FatigueMomentum>();
				component2.Set(item);
			}
		}
		...
		if (biomeFatigue.HasValue && biomeFatigue.Value.Velocity > 0f)
		{
			...
			{
				FatigueMomentum component3 = nodes.GetNext().GetComponent<FatigueMomentum>();
				component3.Set(biomeFatigue.Value, derived);
			}
		}
		nodes.EndLoad();
