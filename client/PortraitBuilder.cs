...
public class PortraitBuilder : ResourceSingleton<PortraitBuilder>
{
	...
	{
		int randomPortraitType = ResourceSingleton<PortraitBuilder>.Instance().GetRandomPortraitType(isMale, key);
		return MakeArgument(randomPortraitType, 0, Color.white, isMale, PortraitEmotion.Normal, ColorTableLoader.GetRandom("color_skin.raw", key), ColorTableLoader.GetRandom("color_hair.raw", key), ColorTableLoader.GetRandom("color_eyes.raw", key), ColorTableLoader.GetRandom((!isMale) ? "color_lips_female.raw" : "color_lips_male.raw", key));
	}
