...
public class ArtifactManager : Durango.Utils.Singleton<ArtifactManager>
{
	...
	private static bool IsFarArtifact(TerrainBase terrain, [NotNull] Artifact artifact)
	{
		...
		for (int i = 0; i < size.y; i++)
		{
			...
			{
				Point2 worldTile2 = worldTile + new Point2(j, i);
				Point2 coord = Util.TilePositionToChunkCoords(worldTile2);
	...
	{
		GameObject gameObject = UnityEngine.Object.Instantiate(_artifactPrefab, base.transform);
		gameObject.name = blueprint.Id;
		Artifact component = gameObject.GetComponent<Artifact>();
		int i = 0;
	...
	public bool ShowArtifactInteriorMsg([NotNull] Artifact artifact, ArtifactState state, bool hidePreviousMsg = false)
	{
		...
		string text = (string.IsNullOrEmpty(interiorSetName) ? (string.IsNullOrEmpty(interiorMoodName) ? string.Empty : T._("{0} 공간", interiorMoodName)) : (string.IsNullOrEmpty(interiorMoodName) ? interiorSetName : $"{interiorMoodName} {interiorSetName}"));
