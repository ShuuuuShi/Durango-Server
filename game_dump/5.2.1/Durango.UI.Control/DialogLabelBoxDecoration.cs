...
public class DialogLabelBoxDecoration : UIWidget, ITextLinkWithValue, ITextLink
{
	[SerializeField]
	...
	LinkLayoutOption ITextLink.UpdateLayout(TextBuilder builder, int size)
	{
		...
		_arrow.transform.localPosition = new Vector3((float)(-point.x) / 2f - _arrow.localSize.x / 2f, 0f, 0f);
		LinkLayoutOption result = default(LinkLayoutOption);
		result.Offset = -_fontPadding;
		return result;
	}
}
