...
public class WarpRushRewardPhaseWidget : MonoBehaviour
{
	...
	public void Init(UIScrollView scrollView)
	{
		...
		foreach (NestedPrefabLinker nestedPrefabLinker in reward)
		{
			nestedPrefabLinker.Object.GetComponent<WarpRushRewardItem>().SetScrollView(scrollView);
		}
	}
	...
	private void RefreshRewardWidget(WarpRushSystem.RewardType rewardType)
	{
		...
		if (rewardState != WarpRushSystem.RewardState.Invalid)
		{
			...
			component.Clicked = ((rewardState != 0 || flag) ? new Action(component.ShowTooltip) : _requestReward[(int)rewardType]);
