...
internal static class CollectionUtils
{
	...
	public static object CreateAndPopulateList(Type listType, Action<IList, bool> populateList)
	{
		...
		{
			list = (typeof(IList).IsAssignableFrom(listType) ? (ReflectionUtils.IsInstantiatableType(listType) ? ((IList)Activator.CreateInstance(listType)) : (((object)listType != typeof(IList)) ? null : new List<object>())) : ((!ReflectionUtils.ImplementsGenericDefinition(listType, typeof(ICollection<>))) ? null : ((!ReflectionUtils.IsInstantiatableType(listType)) ? null : CreateCollectionWrapper(Activator.CreateInstance(listType)))));
		}
		else
		{
