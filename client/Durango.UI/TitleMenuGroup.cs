...
public class TitleMenuGroup : MonoBehaviour
{
	...
	private void ApplyEmigrationMode()
	{
		...
		{
			GameObject[] array = titleOptions.Objects;
			for (int i = 0; i < array.Length; i++)
			{
	...
	private void OnRequestSucceed(string response)
	{
		...
		case State.NPAGetUser:
		{
			string text4 = jObject.Get<string>("session_token");
			if (string.IsNullOrEmpty(text4))
			{
				CurState = State.Error;
				break;
			}
			GameManager.SessionToken = text4;
			CurState = ((!string.IsNullOrEmpty(GameManager.PlayerId)) ? State.PostPrerequsite : State.FadeOutPrologue);
			break;
		}
		case State.GetFrontend:
		{
			string text2 = jObject.Get<string>("dispatch_to");
			if (text2 != null)
			{
			...
			if (GameManager.ConnectCluster != null)
			{
				string text3 = GameManager.ConnectCluster.GatewayUrlRoot;
	...
	private IEnumerator CheckUpdate()
	{
		...
		if (new FileInfo(text3 + "2").Exists)
		{
			File.Delete(text3);
			File.Move(sourceFileName, text3);
			Thread.Sleep(100);
		}
		string text4 = Directory.GetCurrentDirectory() + "\\" + Process.GetCurrentProcess().ProcessName + "_Data\\Programs\\UpdateManager\\DurangoV2_UpdateManager.pdb2";
		if (new FileInfo(text4).Exists)
		{
			File.Delete(text3.Replace(".exe", ".pdb"));
			File.Move(text4, text3.Replace(".exe", ".pdb"));
			Thread.Sleep(100);
		}
		...
		process.WaitForExit();
		if (text5.IndexOf("Error") != -1 || text5.IndexOf("True") == -1)
		{
			KnockSystem();
			yield break;
		}
		...
		string text2 = webClient.DownloadString("http://db.kyllox.pe.kr/durango/update/indispensable.txt");
		...
		if (LocalizeSystem.LocaleLanguage == "ko")
		{
			...
			UserControl.ShowMessageBox(title, explain, okAction, cancelAction, okButtonLabel, cancelButtonLabel);
		}
		else
		{
			...
			UserControl.ShowMessageBox(title2, explain2, okAction, cancelAction, okButtonLabel2, cancelButtonLabel2);
		}
	}
	...
	private IEnumerator DownloadUpdate()
	{
		...
		if (LocalizeSystem.LocaleLanguage == "ko")
		{
			...
			Action okAction = UserControl.CloseMessageBox;
			UserControl.ShowMessageBox(title, explain, okAction);
		}
		else
		{
			...
			Action okAction2 = UserControl.CloseMessageBox;
			UserControl.ShowMessageBox(title2, explain2, okAction2);
		}
		yield break;
	}
