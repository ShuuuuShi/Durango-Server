...
public class JsonArrayContract : JsonContract
{
		...
		: base(underlyingType)
	{
		...
		else if (underlyingType.IsGenericType && (object)underlyingType.GetGenericTypeDefinition() == typeof(IEnumerable<>))
		{
	...
	{
		if ((list is IList && ((object)CollectionItemType == null || !_isCollectionItemTypeNullableType)) || base.UnderlyingType.IsArray)
		{
			return new CollectionWrapper<object>((IList)list);
		}
		if ((object)_genericCollectionDefinitionType != null)
		{
	...
	{
		if ((object)_genericWrapperType == null)
		{
			_genericWrapperType = ReflectionUtils.MakeGenericType(typeof(CollectionWrapper<>), CollectionItemType);
			Type type = ((!ReflectionUtils.InheritsGenericDefinition(_genericCollectionDefinitionType, typeof(List<>)) && (object)_genericCollectionDefinitionType.GetGenericTypeDefinition() != typeof(IEnumerable<>)) ? _genericCollectionDefinitionType : ReflectionUtils.MakeGenericType(typeof(ICollection<>), CollectionItemType));
