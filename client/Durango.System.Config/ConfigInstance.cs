...
public static class ConfigInstance
{
	...
	{
		string fileName = ((!Platform.Instance.UsePCUI) ? "config_menu" : "config_menu_pc");
		Dictionary<string, JObject> dictionary = Json.ReadFromFile<Dictionary<string, JObject>>(fileName);
		...
		foreach (KeyValuePair<string, JObject> item in dictionary)
		{
			...
			{
				string source = item2.Value.Value<string>("Type");
				Setting setting;
				switch (source.ToEnum(SettingType.Invalid))
				{
		...
		foreach (KeyValuePair<string, List<Setting>> setting in Settings)
		{
			foreach (Setting item in setting.Value)
			{
				...
				case SettingType.Category:
				{
					...
					if (item is ToggleSetting toggleSetting && !toggleSetting.Contains(value))
					{
	...
	public static void NotifyAction(string key, ValueSetting op = null)
	{
		...
		case "coupon":
		{
			...
			textInputPopup.Show(SendCoupon, T._("쿠폰번호를 입력해주세요."));
			break;
		}
		case "credit":
	...
	{
		bool screenMode = value == "fullscreen";
		ScreenInfo.SetScreenMode(screenMode);
		return value;
	}
	...
	{
		int targetFrameRate = ((!(value == "quality")) ? 30 : 60);
	...
	private static IEnumerator NoRestartChangeLocalize()
	{
		...
		while (Loader.LoadState != Loader.State.Succees)
		{
			yield return null;
		}
