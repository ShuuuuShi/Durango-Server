...
public class RecipeSystem : GameSystem<RecipeSystem>
{
	...
	private void OnReady()
	{
		...
		string text = (TrackingTarget = Preferences.GetString("TrackingRecipe", null, Preferences.Level.Player));
		string trackingToDo = text;
	...
	public Crafting.Recipe GetDyeingRecipe(ColorChannel channel)
	{
		...
		if (channel < ColorChannel.ColorR || (int)channel >= _dyeingRecipes.Length)
		{
			return null;
		}
		return _dyeingRecipes[(int)channel];
	}
	...
	public Crafting.Recipe GetBleachingRecipe(ColorChannel channel)
	{
		...
		if (channel < ColorChannel.ColorR || (int)channel >= _bleachingRecipes.Length)
		{
			return null;
		}
		return _bleachingRecipes[(int)channel];
	}

	public bool CanCraftNow(CategoryItem categoryItem)
	{
		if (GameManager.ClusterMode != Mode.Editable)
		{
			return true;
		}
		...
		if (categoryItem is Building.Blueprint)
		{
			return HasMaterials(categoryItem as Building.Blueprint);
		}
		return false;
	}
