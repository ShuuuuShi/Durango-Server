	...
	public void Init(int minLv, int maxLv)
	{
		...
		for (int j = 0; j < unstableAreaList.Count; j++)
		{
			...
			if (biomeGridIndex != -1)
			{
				...
				UIDragScrollView uIDragScrollView = component.gameObject.AddComponent<UIDragScrollView>();
				uIDragScrollView.scrollView = _parent.ScrollView;
			...
			while (true)
			{
				...
				for (int i = 0; i < _exploreAreaNodes.Length; i++)
				{
					for (int j = 0; j < _exploreAreaNodes[i].Count; j++)
					{
						...
						int level = exploreAreaNode.Template.Level;
						if (level > num)
						{
				...
				for (int k = 0; k < _exploreAreaNodes.Length; k++)
				{
					...
					{
						ExploreAreaNode exploreAreaNode2 = _exploreAreaNodes[k][l];
						int level2 = exploreAreaNode2.Template.Level;
						if (level2 > num)
						{
							if (level2 > num4)
							{
							...
							exploreAreaNode2.transform.localPosition = localPosition;
							num6++;
							if (GameSystem<StatisticsSystem>.Instance().Level < exploreAreaNode2.Template.AvailableLevel)
							{
