...
public static class Collisions
{
	...
	public static Vector3 ProcessSimpleSliding(CollisionParam param)
	{
		for (int i = 0; i < 4; i++)
		{
			if (CheckCollision(param, collideOnOverlapped: false, out var raycastHit))
			{
				...
				float y = ((!(Vector3.Dot(param.Direction, rhs) > 0f)) ? (-45f) : 45f);
	...
	public static RayCastResult TryCapsuleCast(CollisionParam param, out RaycastHit raycastHit)
	{
		...
		{
			return RayCastResult.Overlapped;
		}
		return RayCastResult.Blocked;
	}
	...
	{
		int count;
		Transform transformOfNearestHit = GetTransformOfNearestHit(RayCast(ray, float.PositiveInfinity, mask, out count), count, tagname);
