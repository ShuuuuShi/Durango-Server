...
public class Farm : ArtifactComponent
{
	...
	private void UpdateGrowTimer()
	{
		...
		if (_growTimer == null || _growTimer.IsStop)
		{
			...
			TimerProgressGauge timerProgressGauge = Durango.Logic.Timer.Timer.Play<TimerProgressGauge>(_growTimer);
			timerProgressGauge.SetTarget(base.Artifact.gameObject, new Vector3((float)(200 * base.Artifact.Size.y) * 0.5f, 50f, (float)(200 * base.Artifact.Size.x) * 0.5f));
		}
		else
		{
	...
	public override Color GetColor()
	{
		...
		if (farming.HasValue)
		{
			return (!(farming.Value.Water.x >= farming.Value.Water.y)) ? Color.white : new Color(0.6f, 0.6f, 0.6f);
		}
		return Color.white;
	}
}
