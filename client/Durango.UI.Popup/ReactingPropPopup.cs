...
public class ReactingPropPopup : TooltipBase
{
	public struct RequiredItemTags
	{
		...
		{
			return itemData.HasTag(_tagFilter, ignoreLevel: true) && !itemData.IsDestroyed();
		}
	}
	...
	private void RefreshResourceItems(RequiredItemTags? requiredItemTags, Messages.Cost? requiredMoney)
	{
		...
		if (requiredMoney.HasValue)
		{
			ReactingPropResourceWidget next = _resourceWidgets.GetNext();
			if (!next.Set(requiredMoney.Value))
			{
		...
		{
			ReactingPropResourceWidget next2 = _resourceWidgets.GetNext();
			if (!next2.Set(requiredItemTags.Value))
			{
				_notEnoughResources = true;
			}
	...
	private void RefreshGivingItems(Messages.RewardItem[] givingItems)
	{
		...
		{
			ReactingPropItemWidget next = _itemWidgets.GetNext();
			next.Set(item);
		}
	...
	private void RefreshAvailableAtText(bool notAvailable)
	{
		...
		if (_notAvailable)
		{
			...
			{
				double num = SyncString.UpdateRemainTimeMsg(_availableAt, T._("{0} 후 이용 가능"), out text, out period, string.Empty);
				if (num <= 0.0)
				{
