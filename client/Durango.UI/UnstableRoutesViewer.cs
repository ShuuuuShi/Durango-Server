...
public class UnstableRoutesViewer : AnimationWidget
{
	...
	private void Update()
	{
		...
		if (!Mathf.Approximately(offsetRatio, _prevOffsetRatio))
		{
			...
			if (_prevIndex != num2)
			{
				...
				UIWidget node = _scroll.GetNode(num2);
				ArchipelagoRoute archipelagoRoute = node.GetComponent<Archipelago>().ArchipelagoRoute;
	...
	public void Set(RegionTemplate template)
	{
		...
		else
		{
			...
			foreach (ArchipelagoRoute item in archipelagoRoutes.OrderBy((ArchipelagoRoute route) => route.UnstableFactor))
			{
				...
				string unstableFactor = ((!item.IsAllConditionSatisfied()) ? null : item.UnstableFactor.ToString());
				bool flag = _unstableFactors.Count == num;
				_unstableFactors.Add(component3);
				component3.Set(unstableFactor);
	...
	{
		Archipelago component = _scroll.GetNode(_scroll.GetGoalNodeIndex()).GetComponent<Archipelago>();
		return component.GetIslandTransform();
	}
}
