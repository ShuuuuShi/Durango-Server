...
public class ArtifactInfoManageRightsNode : UIWidget
{
	...
	public bool IsChanged
	{
		get
		{
			...
			if (Value == _originValue)
			{
				...
				num = ((valueOrDefault != inventoryAccessCount.GetValueOrDefault() || (originInventoryAccessCount.HasValue ^ inventoryAccessCount.HasValue)) ? 1 : 0);
			}
			else
			{
				num = 1;
	...
	private void TextUpdate()
	{
		...
		{
			string text2 = (Value ? (InventoryAccessCount.Value switch
			{
			...
			}) : string.Format("{0} [FFFFFF4C][icon=img_market_arrowright][-]\n[size=20][BBBBBB]{1}[-][/size]", text, T._("사유지 권한에 따름")));
			text = text2;
		}
