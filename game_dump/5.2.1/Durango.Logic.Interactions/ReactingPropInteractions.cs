...
public static class ReactingPropInteractions
{
	...
	private static void RescueWithCPR(InteractionObject target)
	{
		...
		}).On(delegate(Messages.Timer msg, PacketHeader header)
		{
			...
			{
				UIManager.FindScript<CPRGroup>().FinishCPR(interrupted: true);
			};
			Durango.Logic.Timer.Timer.Play<IconProgressGauge>(timer).AddIcon(GetGaugeIcon(Interaction.RescueWithCpr));
		});
	}
	...
	private static void ContactReactingProp(InteractionObject target, Interaction interaction, ItemIcon guageIcon, string motionName, string[] itemIds = null)
	{
		...
		{
			Durango.Logic.Timer.Timer.Play<IconProgressGauge>(new Durango.Logic.Timer.Timer("reacting_prop", msg.Duration)).AddIcon(guageIcon);
		...
		}).On(delegate(ReactingPropRewarded msg, PacketHeader header)
		{
			...
			{
				UIManager.Popup.Tooltip<ReceiveRewardsPopup>().ShowReactingPropRewardItems(msg.Items);
			}
	...
	{
		if (interaction == Interaction.ConfirmIdentity)
		{
			return OnSuccessConfirmIdentity;
		}
		return null;
	}
	...
	private static void OnSuccessConfirmIdentity()
	{
		ChatStruct chat = new ChatStruct
		{
			...
			Chatter = PlayerBehavior.LocalPlayer.ChatableBase,
			Body = new RadioNotice
			{
				Text = MourningWords[UnityEngine.Random.Range(0, MourningWords.Length)]
			},
			Name = PlayerBehavior.LocalPlayer.GetName(),
