...
public class TagSelectPopup : TooltipBase, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		_entireTags = instance.Where(delegate(KeyValuePair<string, Tag> elem)
		{
			KeyValuePair<string, Tag> keyValuePair3 = elem;
			if (keyValuePair3.Value.Visible)
			{
				keyValuePair3 = elem;
				return !keyValuePair3.Value.Unsearchable;
			}
			return false;
		}).OrderByDescending(delegate(KeyValuePair<string, Tag> elem)
		{
			KeyValuePair<string, Tag> keyValuePair2 = elem;
			return keyValuePair2.Value.Grade;
		}).ThenBy((KeyValuePair<string, Tag> elem) => elem.Key, StringComparer.Ordinal)
			.Select(delegate(KeyValuePair<string, Tag> elem)
			{
				KeyValuePair<string, Tag> keyValuePair = elem;
	...
	private void UpdateSelectedTagsWidget()
	{
		...
		{
			string tagId = list[i];
			nodes.GetNext().GetComponent<ItemTagWidget>().Set((!_duplicatedNameTags.Contains(tagId)) ? TagData.GetTagName(tagId) : TagData.GetTagNameAndPurpose(tagId), delegate
			{
