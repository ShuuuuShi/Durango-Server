...
public class ClanSystem : GameSystem<ClanSystem>
{
	...
	private void RequestClanInfo(string clanId, Clan cachedInfo, Action<string, Clan> callback)
	{
		...
		Http.RequestYml(string.Format((!isDetail) ? "{0}/clans/{1}" : "{0}/clans/{1}?detail", GameManager.GatewayUrl, clanId), delegate(ClanJson json)
		{
	...
	public void RequestClanInfo(string clanName, Action<IList<Clan>> callback)
	{
		...
		Http.RequestYml(GameManager.GatewayUrl + "/clans?keyword=" + WWW.EscapeURL(clanName), delegate(ClanJsonContainer res)
		{
	...
	private void OnChangePlayerClan(PlayerBehavior player)
	{
		...
		string obj = ((PlayerClan != null) ? PlayerClan.Id : string.Empty);
		RequestPlayerClan();
		if (!(obj != player.ClanId))
		{
			return;
		}
	...
	private void UpdateLocalplayerMember()
	{
		...
		{
			if (WaitingClan.GetApplier(localPlayer.EntityId) == null)
			{
				WaitingClan = null;
			}
		}
	...
	{
		PlayerBehavior localPlayer = PlayerBehavior.LocalPlayer;
		if (localPlayer.HasClan)
		{
			return localPlayer.ClanId == clanId;
		}
		return false;
	}
	...
	public static bool IsMyClanOrAlliance([NotNull] PlayerBehavior other)
	{
		...
		if (other.HasClan)
		{
			return IsMyClanOrAlliance(other.ClanId);
		}
		return false;
	}
	...
	public static bool IsMyClanOrAlliance(string clanId)
	{
		if (!IsMyClan(clanId))
		{
			return GameSystem<ClanSystem>.Instance().IsAlliedClan(clanId);
		}
		return true;
	}
