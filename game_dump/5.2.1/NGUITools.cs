...
public static class NGUITools
{
	...
	public const string defaultShaderName = "Durango/NGUI/Transparent";
	...
	public static Shader defaultShader => Shader.Find("Durango/NGUI/Transparent");
	...
	public static void NormalizeWidgetDepths(UIWidget[] list)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			int num4 = (uIWidget.depth = num2 + 1);
			num2 = num4;
		}
	}
	...
	public static UIPanel CreateUI(Transform trans, bool advanced3D, int layer)
	{
		...
		if (uIPanel2 == null)
		{
			...
			AudioListener[] array3 = FindActive<AudioListener>();
	...
	public static UISprite AddSprite(this GameObject go, string spriteName, int depth = int.MaxValue)
	{
		ResourceSingleton<UISpriteManager>.Instance().TryGet(spriteName, out var _, out var spriteData);
		UISprite uISprite = go.AddWidget<UISprite>(depth);
		uISprite.type = ((spriteData != null && spriteData.hasBorder) ? UIBasicSprite.Type.Sliced : UIBasicSprite.Type.Simple);
	...
	public static void Execute<T>(GameObject go, string funcName) where T : Component
	{
		...
		{
			val.GetType().GetMethod(funcName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.Invoke(val, null);
		}
	}
