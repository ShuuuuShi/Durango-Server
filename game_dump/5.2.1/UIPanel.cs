...
public class UIPanel : UIRect
{
	...
	public override float alpha
	{
		...
		set
		{
			...
			if (mAlpha != num)
			{
				bool flag = mAlpha * mVisibleRatio > 0.001f;
				mAlphaFrameID = -1;
				mResized = true;
				mAlpha = num;
				int i = 0;
				...
				bool flag2 = mAlpha * mVisibleRatio > 0.001f;
				Invalidate(flag != flag2);
			}
		}
	}
	...
	public Vector3 drawCallOffset
	{
		get
		{
			if (mHalfPixelOffset && base.anchorCamera != null && mCam.orthographic)
			{
				...
				float num = ((!(base.root != null)) ? 1f : base.root.pixelSizeAdjustment) / windowSize.y / mCam.orthographicSize;
	...
	public override float CalculateFinalAlpha(int frameID)
	{
		if (mAlphaFrameID != frameID)
		{
			...
			float num = mAlpha * mVisibleRatio;
			finalAlpha = ((!(base.parent != null)) ? num : (uIRect.CalculateFinalAlpha(frameID) * num));
		}
		return finalAlpha;
	}
	...
	private void LateUpdate()
	{
		...
		{
			list[j].LateUpdateSelf();
		}
		int num = 3000;
		int k = 0;
		for (int count3 = list.Count; k < count3; k++)
		{
			UIPanel uIPanel = list[k];
	...
	private void FillAllDrawCalls()
	{
		...
		for (int j = 0; j < widgets.Count; j++)
		{
			...
			if (uIWidget.isVisible && uIWidget.hasVertices)
			{
				...
				if (generateNormals)
				{
					uIWidget.WriteToBuffers(uIDrawCall.verts, uIDrawCall.uvs, uIDrawCall.cols, uIDrawCall.norms, uIDrawCall.tans, uIDrawCall.extentionUvs);
				}
				else
				{
					uIWidget.WriteToBuffers(uIDrawCall.verts, uIDrawCall.uvs, uIDrawCall.cols, null, null, uIDrawCall.extentionUvs);
				}
			...
			while (num2 < widgets.Count)
			{
				...
				if (uIWidget.drawCall == dc)
				{
					if (uIWidget.isVisible && uIWidget.hasVertices)
					{
						...
						if (generateNormals)
						{
							uIWidget.WriteToBuffers(dc.verts, dc.uvs, dc.cols, dc.norms, dc.tans, dc.extentionUvs);
						}
						else
						{
							uIWidget.WriteToBuffers(dc.verts, dc.uvs, dc.cols, null, null, dc.extentionUvs);
						}
						if (uIWidget.mOnRender != null)
						{
							if (mOnRender == null)
