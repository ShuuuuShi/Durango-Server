...
public class ConcertPopup : TooltipBase
{
	...
	private void UpdateBandstand([NotNull] Artifact artifact)
	{
		...
		else if (_concert == null)
		{
			Hide();
		}
		else if (!(_concert.Bandstand.EntityId != artifact.EntityId))
		{
			Bandstand? bandstand = artifact.ArtifactState.Bandstand;
			if (!bandstand.HasValue || string.IsNullOrEmpty(bandstand.Value.Host))
			{
				Hide();
				return;
			}
			if (_concert.Host != bandstand.Value.Host)
			{
				Hide();
				return;
			}
	...
	private void OnBandstandUpdate()
	{
		...
		if (_concert != null)
		{
			...
			{
				string value = tracks[i].PlayerId.Value;
	...
	public void Set(Artifact artifact)
	{
		...
		{
			_scrollView.Nodes.GetNext().GetComponent<ConcertTrackWidget>().SetTrack(track, _concert.Host);
		}
