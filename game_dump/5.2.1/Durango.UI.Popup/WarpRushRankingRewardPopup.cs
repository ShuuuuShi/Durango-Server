...
public class WarpRushRankingRewardPopup : TooltipBase
{
	...
	private void Select(int index)
	{
		...
		{
			_tabs[i].GetComponent<IconTabWidget>().Selected = i == index;
		}
	...
	protected override void FillData()
	{
		...
		if (list != null)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				_itemList.Nodes.GetNext().GetComponent<SupplyRewardNode>().SetNode(rankingText, rankingReward.GetRewards());
			}
	...
	private static string GetRankingText(RankingReward prev, RankingReward current, bool isHighRank)
	{
		...
		if (prev == null)
		{
			string text2 = ((current.Ranking != 0) ? text : current.RankingPecentage);
			return "<em>[icon=crown] " + text2 + "</em>";
		}
		if (prev.Ranking != 0)
		{
