...
public static class InputCommandConverter
{
	...
	{
		if (message.Command == InputCommand.TouchPicking)
		{
			return Picking(message.Command, message.Ray, message.TouchEvent, KeyCode.None);
		}
		return Touch(message.Command, message.Touches);
	}
	...
	public static InputCommandMessage Convert(InputMouse.Message message)
	{
		...
		if (message.Command == InputCommand.GestureTwoFingerDrag)
		{
			return Gesture(InputCommand.GestureTwoFingerDrag, message.Direction, touchedUI: false);
		}
		return Move(message.Command, message.Direction);
	}
