...
public class PopupGroup : UIBase
{
	...
	private void Awake()
	{
		...
		try
		{
			...
			IEnumerable<GameObject> popupList = GetPopupList();
			foreach (GameObject item in popupList)
			{
				TooltipBase component = item.GetComponent<TooltipBase>();
				if (!(component == null))
				{
					int depth = 7000;
					...
					GameObject gameObject2 = UnityEngine.Object.Instantiate(item, value.transform);
					gameObject2.GetComponent<TooltipBase>().InitializePanelDepth(depth);
				}
			}
			foreach (KeyValuePair<TooltipBase.DepthEnum, GameObject> item2 in dictionary)
			{
				NGUITools.SetLayer(item2.Value, item2.Value.layer);
			}
		}
	...
	{
		GameObject[] source = Resources.LoadAll<GameObject>("Popup");
		return source.Where((GameObject x) => !x.name.ContainsIgnoreCase("_PC"));
	}
