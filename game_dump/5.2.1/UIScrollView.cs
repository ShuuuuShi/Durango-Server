...
public class UIScrollView : MonoBehaviour
{
	...
	public static float FixedScrollWheelFactor = 0f;
	...
	public virtual Bounds bounds
	{
		get
		{
			if (!mCalculatedBounds && !mFixedBounds)
			{
	...
	private void Start()
	{
		...
		if (FixedScrollWheelFactor > 0f)
		{
			scrollWheelFactor = FixedScrollWheelFactor;
		}
	}
	...
	public void SetFixedBounds(Bounds value)
	{
		mFixedBounds = true;
		mBounds = value;
	}
	...
	private void Update()
	{
		if (!Application.isPlaying || !mShouldMove)
		{
			return;
		}
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0f)
			{
				...
				if (component != null && component.enabled)
				{
					...
					absolute.x = NGUIMath.SpringDampen(ref mMomentum.x, (localPosition.x != component.target.x) ? 9f : 3f, deltaTime);
					absolute.y = NGUIMath.SpringDampen(ref mMomentum.y, (localPosition.y != component.target.y) ? 9f : 3f, deltaTime);
				}
				else
				{
					absolute = NGUIMath.SpringDampen(ref mMomentum, 3f, deltaTime);
				}
