	...
	protected override void FillData()
	{
		...
		if (pioneerCostExchangeRate != null)
		{
			...
			for (int i = 0; i < KUtility.GetSize(pioneerCostExchangeRate.Rates); i++)
			{
				...
				next.FindComponent<UILabel>("Section").text = ((!pioneerRate.Paid) ? T._("{0:P0} 구간", pioneerRate.Rate) : T._("<em>[icon=icon_amplifier]</em> {0:P0} 구간", pioneerRate.Rate));
				float num = info.DailyExchangedPoints.Get(pioneerRate.Rate, 0f);
				next.FindComponent<UILabel>("Points").text = string.Format("{0:0.#} / {1}", num, (pioneerRate.Point < 0) ? "∞" : pioneerRate.Point.ToString());
				float fillAmount = ((pioneerRate.Point <= 0) ? 0f : (num / (float)pioneerRate.Point));
				next.FindComponent<UISprite>("Bar").fillAmount = fillAmount;
			}
			_nodes.EndLoad();
