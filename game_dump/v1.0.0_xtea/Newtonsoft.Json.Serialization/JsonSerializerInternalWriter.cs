...
internal class JsonSerializerInternalWriter : JsonSerializerInternalBase
{
	...
	{
		if ((object)contract.UnderlyingType == typeof(byte[]) && ShouldWriteType(TypeNameHandling.Objects, contract, member, collectionValueContract))
		{
	...
	private void WriteMemberInfoProperty(JsonWriter writer, object memberValue, JsonProperty property, JsonContract contract)
	{
		...
		if (HasFlag(property.DefaultValueHandling.GetValueOrDefault(base.Serializer.DefaultValueHandling), DefaultValueHandling.Ignore))
		{
			if (defaultValue == null)
			{
				defaultValue = GetDefaultValue(memberValue);
			}
			if (MiscellaneousUtils.ValueEquals(memberValue, defaultValue))
			{
				return;
			}
		}
	...
	private bool ShouldWriteType(TypeNameHandling typeNameHandlingFlag, JsonContract contract, JsonProperty member, JsonContract collectionValueContract)
	{
		...
		if (member != null)
		{
			...
			if (((!typeNameHandling2.HasValue) ? base.Serializer.TypeNameHandling : typeNameHandling2.Value) == TypeNameHandling.Auto && (object)contract.UnderlyingType != member.PropertyType)
			{
		...
		else if (collectionValueContract != null && base.Serializer.TypeNameHandling == TypeNameHandling.Auto && (object)contract.UnderlyingType != collectionValueContract.UnderlyingType)
		{
			return true;
		}
		return false;
