...
public class SelectModularPartTexturePopup : TooltipBase
{
	...
	private void FillTextureList()
	{
		...
		{
			SelectableWidget component = _textureScrollView.Nodes.GetNext().GetComponent<SelectableWidget>();
	...
	private void SelectTexture(string texture)
	{
		...
		for (int i = 0; i < _textures.Count; i++)
		{
			...
			_textureScrollView.Nodes[i].GetComponent<Selectable>().Selected = selected;
		}
	...
	{
		if (_selectedId == null || _remodelingParts[_selectedId] == null)
		{
			return;
		}
		...
		string[] obj = ((_categories.Count > 0) ? _categories.ToArray() : new string[1] { _selectedId });
		...
		foreach (string key in array)
		{
			...
			{
				if (dictionary == null || string.IsNullOrEmpty(dictionary.Get(key)))
				{
					return;
				}
			...
			if (dictionary.Get(key) == selectedTexture)
			{
				return;
			}
