...
public static class StoreReview
{
	...
	{
		_isReviewed = KUtility.GetSize(storage.Get("store_review")) > 0;
	}
	...
	private static bool IsAlreadyReviewed()
	{
		if (!_isReviewed)
		{
			return Preferences.GetBool("store_review");
		}
		return true;
	}
	...
	{
		SetReviewed();
		Application.OpenURL((Application.platform != RuntimePlatform.Android) ? "https://apps.apple.com/app/id1281442514?action=write-review" : ("https://play.google.com/store/apps/details?id=" + Platform.Instance.AppBundleId));
	}
}
