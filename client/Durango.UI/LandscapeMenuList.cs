...
public class LandscapeMenuList : LandscapeMenuListBase, IUIInitializable
{
	...
	{
		IEnumerable<MenuType> source = MenuContainer.FirstDepthMenus.Where((MenuType menu) => (!MenuContainer.HasChildren(menu)) ? GameSystem<MenuSystem>.Instance().IsEnabled(menu) : MenuContainer.GetChildren(menu).Any((MenuType c) => GameSystem<MenuSystem>.Instance().IsEnabled(c)));
