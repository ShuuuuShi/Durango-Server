...
public class AccessRightsPage : MonoBehaviour
{
	...
	private void Init()
	{
		...
		{
			AccessRightNode component = o.GetComponent<AccessRightNode>();
			component.Clicked = (Action)Delegate.Combine(component.Clicked, new Action(OnClickRightNode));
		});
		...
		for (int i = 0; i < values.Length; i++)
		{
			...
			if (accessRights != 0)
			{
				_scrollView.Nodes.GetNext().GetComponent<AccessRightNode>().Set(accessRights);
			}
		}
