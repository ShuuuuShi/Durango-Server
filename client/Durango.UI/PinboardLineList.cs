	...
	public void Refresh([CanBeNull] ReadPinboard readPinboard)
	{
		...
		if (readPinboard != null)
		{
			...
			for (int i = 0; i < readPinboard.contents.Length; i++)
			{
				...
				{
					PinboardLine pinboardLine2 = ((!(pinboardLine != null) || !(pinboardLine.EntityId == pinboardContent.id)) ? AddNewLine() : pinboardLine);
					pinboardLine2.AddContent(pinboardContent);
					pinboardLine = pinboardLine2;
				}
			}
		}
