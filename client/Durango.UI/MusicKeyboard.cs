...
public class MusicKeyboard : MonoBehaviour
{
	...
	public void Init(int min, int max)
	{
		...
		{
			int num2 = i % num;
			ListObjectPool<MusicKeyboardItem> listObjectPool = ((!WhiteFlag[num2]) ? _blacks : _whites);
			MusicKeyboardItem musicKeyboardItem = listObjectPool.Add();
	...
	private void Reposition()
	{
		...
		{
			MusicKeyboardItem musicKeyboardItem = _keyboards[i];
			musicKeyboardItem.transform.localPosition = localPosition;
	...
	public void SelectKey(int midi, bool select)
	{
		...
		{
			MusicKeyboardItem musicKeyboardItem = _keyboards[num];
			musicKeyboardItem.Select(select);
		}
	}
	...
	public void ClearSelectedKeyboard()
	{
		...
		{
			MusicKeyboardItem musicKeyboardItem = _keyboards[i];
			musicKeyboardItem.Select(select: false);
		}
	}
}
