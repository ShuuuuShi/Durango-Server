...
public class WarpRushRanking : MonoBehaviour, IScreenResizeReceiver
{
	public struct TabInfo
	{
		...
		public string GetTabName()
		{
			...
			if (IsExpired)
			{
				arg = T.GetParticularString("The duration of the respective league has expired", "종료");
			}
			else
			{
				...
				arg = string.Format(T.Culture, "{0:M.d}~{1:M.d}", result.ToLocalTime(), result2.ToLocalTime());
			}
			return $"{Revision.Name.ToString()} [size=20]({arg})[/size]";
		}
		...
		public string GetLeftDays()
		{
			...
			double totalSeconds = (result - dateTime).TotalSeconds;
			return TimedeltaFormatter.Format(totalSeconds, 1, "day");
		}
	}
	...
	private void SelectTab(int index)
	{
		...
		GameSystem<WarpRushSystem>.Instance().GetRanking(tabInfo.Category, tabInfo.RevisionId, delegate(RankingInfo info)
		{
			if (info == null || !(info.RevisionId != _tabInfos[_currentTabIndex].RevisionId))
			{
				...
				{
					float num = _rankingItemBaseObject.GetComponent<UIWidget>().height;
					offset = num * ((float)info.MyRecord.Rank - 0.5f) - _scrollView.ScrollView.panel.height / 2f;
				}
