...
public class Faction
{
	...
	public int GetMaxLevel()
	{
		...
		return faction.LevelThresholds.Length;
	}
	...
	private bool IsStarted()
	{
		if (StartsAt > 0.0)
		{
			...
			if (StartsAt <= predictedServerTime)
			{
				return EndsAt > predictedServerTime;
			}
			return false;
		}
		return true;
	...
	{
		double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
		if (IsAvailable() && MissionAvailableAt > 0.0)
		{
			return MissionAvailableAt < predictedServerTime;
		}
		return false;
	}
	...
	{
		double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
		if (IsAvailable() && SupportRequestAvailableAt >= 0.0)
		{
			return SupportRequestAvailableAt < predictedServerTime;
		}
		return false;
	}
	...
	{
		if (IsSupportRequestAvailable())
		{
			return SupportRequests.Any((SupportRequest x) => x.Level <= Level && x.IsAvailable());
		}
		return false;
	}
}
