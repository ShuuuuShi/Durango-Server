...
public class RecipeFilterWidget : MonoBehaviour, IUICursorChangable
{
	...
	{
		return (!string.IsNullOrEmpty(SearchText)) ? _recipeSubLists.Where((KeyValuePair<string, RecipeListWidget.SubList> item) => item.Value.ContainsFilteredItem(SearchText)) : _recipeSubLists;
	}
