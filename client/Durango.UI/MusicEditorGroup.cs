...
public class MusicEditorGroup : UIBase
{
	...
	private void Start()
	{
		Durango.Utils.Singleton<PlayerController>.Instance().TryMoveWhenLocked += delegate
		{
			if (PlayerBehavior.LocalPlayer.IsPlayingMusic() && !UIBase.HasOpenedUI)
			{
				...
				messageBox.Show(T._("연주를 종료하겠습니까?"), delegate(bool ok)
				{
					if (ok)
					{
		...
		{
			bool flag = IsValidBandstand(prev.Bandstand);
			ArtifactState artifactState = artifact.ArtifactState;
			bool flag2 = IsValidBandstand(artifactState.Bandstand);
			if (flag || flag2)
			{
				ChatBubbleGroup chatBubbleGroup2 = UIManager.FindScript<ChatBubbleGroup>();
				if (flag2)
				{
					double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
					Bandstand? bandstand = artifactState.Bandstand;
					double num = (bandstand.HasValue ? artifactState.Bandstand.Value.ExpiresAt.GetValueOrDefault() : 0.0);
					if (predictedServerTime < num)
					{
						chatBubbleGroup2.Show(artifact.ChatableBase, null, null, "icon_hormony", Color.black, ChatBubble.TargetPivot.Up, Vector3.zero, alwaysInScreen: false, (float)(num - predictedServerTime));
						return;
					}
				}
				chatBubbleGroup2.Hide(artifact.EntityId);
			}
		};
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.HostConcert, delegate(InteractionObject obj)
		{
			...
			if (!(targetComponent2 == null))
			{
				...
				ConcertPopup concertPopup2 = UIManager.Popup.Tooltip<ConcertPopup>();
				concertPopup2.SetReserve(targetComponent2.EntityId);
			}
		});
	...
	private void CheckMusicEditCloseWithoutSave([NotNull] Action func)
	{
		...
		{
			MessageBox messageBox = UIManager.MessageBox;
			messageBox.Show(T._("종료하시겠습니까?"), T._("저장 되지 않은 악보는 사라집니다."), delegate(bool ok)
			{
				if (ok)
				{
	...
	private void PlayMusic(MusicId musicId)
	{
		...
		popupItemSelector.MyInventory().Title(T._("연주 악기 선택")).HelpText(music.Name)
			.AutoFillText(T._("확인"))
	...
	{
		List<KeyValuePair<string, Messages.Music>> list = new List<KeyValuePair<string, Messages.Music>>();
		GenericSelector selector = UIManager.Popup.Tooltip<GenericSelector>();
		MusicManager.Instrument[] instruments = Durango.Utils.Singleton<MusicManager>.Instance().GetInstruments();
		string[] texts = instruments.Select((MusicManager.Instrument item) => item.Name).ToArray();
		foreach (KeyValuePair<MusicId, Messages.Music> i in musics)
		{
			selector.ResetArguments();
			selector.SetTitle(i.Value.Name);
			string[] array = texts;
			...
			{
				MusicManager.Instrument instrument = instruments[index];
				list.Add(new KeyValuePair<string, Messages.Music>(instrument.Id, i.Value));
			});
			selector.Show();
		...
		foreach (KeyValuePair<string, Messages.Music> item in list)
		{
