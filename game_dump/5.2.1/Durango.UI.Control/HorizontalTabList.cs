...
public class HorizontalTabList : UIWidget
{
	...
	public void UpdateLayout(FitStyle fitStyle, int minFixedSize = 0)
	{
		...
		{
			node2.GetComponent<HorizontalTabWidget>().UpdateLayout(num);
		}
	...
	public void Select(int index)
	{
		...
		{
			nodes[i].GetComponent<HorizontalTabWidget>().Selected = i == index;
		}
		if (index >= 0)
		{
	...
	public void ClearSelection()
	{
		...
		for (int i = 0; i < nodes.Count; i++)
		{
			nodes[i].GetComponent<HorizontalTabWidget>().Selected = false;
		}
	}
	...
	public void SetNotification(int index, bool on, Durango.Logic.Notification.Type type)
	{
		...
		if (index >= 0 && index < nodes.Count)
		{
			nodes[index].GetComponent<HorizontalTabWidget>().NotificationOn(on, type);
		}
	}
