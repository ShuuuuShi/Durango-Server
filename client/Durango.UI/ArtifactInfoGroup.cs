...
public class ArtifactInfoGroup : UIBase
{
	...
	private void ShowGridArea()
	{
		...
		if (_artifact.ArtifactState.Sprinkler.HasValue)
		{
			...
			simpleTileEdgeArea.Tile = _artifact.WorldTile;
			simpleTileEdgeArea.Offsets = _artifact.Blueprint.GetEffectTiles(_artifact.Rotation);
			simpleTileEdgeArea.TileColorFunc = delegate(Point2 tile, out Color color)
			{
				color = Sprinklable.WateringTileColor;
				return true;
			};
			SimpleTileEdgeArea simpleTileEdgeArea2 = simpleTileEdgeArea;
			Singleton<GridAreaViewer>.Instance().Show(new GridAreaBase[1] { simpleTileEdgeArea2 }, GridAreaViewer.LayerType.Upper);
		}
		else
		{
			Singleton<GridAreaViewer>.Instance().Hide();
	...
	private void PhaseChangeImpl()
	{
		...
		if (_phaseChange.On)
		{
			...
			float num3 = (time - _phaseChange.Since) / (_phaseChange.Until - _phaseChange.Since);
			num = (int)(num3 * 3f);
			num2 = num3 * 3f % 1f;
		}
		int num4;
