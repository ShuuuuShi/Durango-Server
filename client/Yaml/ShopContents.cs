...
public struct ShopContents
{
	...
	{
		return KUtility.GetSize(Items) > 0 || KUtility.GetSize(Money) > 0 || KUtility.GetSize(StatusEffects) > 0 || KUtility.GetSize(Motions) > 0 || KUtility.GetSize(Modulars) > 0 || KUtility.GetSize(Vouchers) > 0 || KUtility.GetSize(RefillVouchers) > 0 || KUtility.GetSize(WeightedMotions) > 0 || KUtility.GetSize(WeightedItems) > 0;
	}

	public bool HasRandomContents()
	{
		return KUtility.GetSize(WeightedMotions) > 0 || KUtility.GetSize(WeightedItems) > 0;
	}
}
