...
public class QuestNodeWidget : MonoBehaviour
{
	...
	private void UpdateQuestRewards()
	{
		...
		{
			_questItemPool.GetNext().SetItem(items[i], _quest.Finished);
		}
		for (int j = 0; j < size2; j++)
		{
			_questItemPool.GetNext().SetRecipe(recipeIds[j], _quest.Finished);
		}
		for (int k = 0; k < size3; k++)
		{
			_questItemPool.GetNext().SetBlueprint(blueprintIds[k], _quest.Finished);
		}
		for (int l = 0; l < size4; l++)
		{
			_questItemPool.GetNext().SetTitle(titles[l], _quest.Finished);
		}
