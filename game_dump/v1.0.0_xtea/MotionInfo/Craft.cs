...
public class Craft
{
	public Dictionary<string, Dictionary<string, List<CraftMotion>>> motions;
	...
	public bool TryGetValue(string recipeID, string workbench, List<TagData> materialTags, out string motion, out string equip)
	{
		...
		if (motions.TryGetValue(workbench, out var value))
		{
			...
			{
				if (!value.TryGetValue(recipeID, out value2))
				{
					value.TryGetValue("default", out value2);
				}
