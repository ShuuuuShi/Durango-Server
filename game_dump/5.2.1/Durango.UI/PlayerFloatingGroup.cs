...
public class PlayerFloatingGroup : UIBase
{
	...
	{
		StatusEffect statusEffect = GameSystem<StatusEffectSystem>.Instance().GetStatusEffects().List.FirstOrDefault((StatusEffect x) => !string.IsNullOrEmpty(x.FloatingIcon));
		if (statusEffect != null)
		{
			return statusEffect.FloatingIcon;
		}
		return string.Empty;
	}
	...
	private void Refresh()
	{
		...
		foreach (PlayerBehavior player in Singleton<PlayerManager>.Instance().GetPlayers())
		{
			MakeControl(player);
		}
	}
