...
public class PrologueInteractionButton_PC : MonoBehaviour
{
	...
	private void Init(Action<PrologueInteractionButton_PC> onClick)
	{
		if (!_init)
		{
			UIEventListener.Get(_characterButton).onClick = delegate
			{
				onClick(this);
			};
			UIEventListener.Get(_propButtonCollider.gameObject).onClick = delegate
			{
				onClick(this);
			};
	...
	public void SetWorldPosition(Vector3 worldPos)
	{
		...
		{
			bool active = Vector3.Distance(PlayerBehavior.LocalPlayer.CurrentPosition, worldPos) < _shortcutDisplayRange;
