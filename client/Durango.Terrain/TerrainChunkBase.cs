...
public abstract class TerrainChunkBase : MonoBehaviour
{
	...
	{
		return (ChunkHash != null) ? ChunkHash.Value(tileX, tileY, category, offset) : 0f;
	}
	...
	protected GameObject CreateLocalObject(string objName)
	{
		...
		gameObject.transform.localPosition = new Vector3(-1600f, 0f, -1600f);
	...
	{
		yield return StartCoroutine(LoadNaturals(chunkData.Naturals));
		yield return StartCoroutine(LoadGrass(chunkData.Biomes));
	...
	private void SetLandmark([NotNull] GameObject obj, LandmarkInfo info, string poolName)
	{
		...
		if (tileObject == null)
		{
			Debug.LogError("GetTileObject failed - " + point);
			return;
		}
