...
public class DomesticationRewardPopup : TooltipBase
{
	...
	protected override void FillData()
	{
		...
		if (_result.Rank.HasValue)
		{
			...
			if (_result.Rank.Value == PetRank.S)
			{
		...
		using (Reusable<StringBuilder> reusable = ReusableStringBuilder.Pop())
		{
			...
			if (_level > 0)
			{
				...
				value.Append(LocalizeUtil.FormatLevel(_level));
			}
			_itemDesc.text = value.ToString();
		}
	...
	private void ShowTags(DomesticationResult result)
	{
		...
		foreach (string key in tags.Keys)
		{
			...
			if (tag != null)
			{
				_tagsScroll.Nodes.GetNext().GetComponent<UILabel>().text = T._("{0} {1:lv:}", tag.Name, tags[key]);
			}
		}
	...
	private static Pair<string, string> GetStatData(DomesticationResult result, Derived type)
	{
		...
		if (type == Derived.LifeSpan)
		{
			...
			text2 = TimedeltaFormatter.Format(num - num2);
		}
		else
		{
			text = Mathf.RoundToInt(num).ToString();
			text2 = Mathf.RoundToInt(num - num2).ToString("+0;-#");
		}
		return new Pair<string, string>(type.GetName(), (num != num2) ? (text + " <em>" + text2 + "</em>") : text);
	}
}
