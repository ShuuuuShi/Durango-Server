...
public class Gateway
{
	...
	public Gateway(GameServer gameServer, WorldContext worldCtx, PlayerContext playerCtx)
	{
		...
		_webServer.GetRoute["/knock"] = delegate(HttpListenerRequest request, Dictionary<string, string> postData)
		{
			...
			string arg = ((!Debug.isDebugBuild) ? "live" : "release");
			string text3 = request.QueryString.Get("platform");
			text3 = ((text3 != null && text3.Equals("iPhonePlayer", StringComparison.OrdinalIgnoreCase)) ? "ios" : ((text3 == null || !text3.Equals("Android", StringComparison.OrdinalIgnoreCase)) ? "windows" : "android"));
			jObject["assetbundle_index_url"] = $"http://assetbundles.k.nexon.com/{arg}/{text3}/Info.5.2.1.json";
			jObject["assetbundle_url_root"] = $"http://durango-assetbundles.akamaized.net/{arg}/{text3}/";
	...
	private static void UpdateAppearPlayer(PlayerContext player, Dictionary<string, string> postData)
	{
		...
		PrologueManager.PlayerDisplay playerDisplay = Json.Read<PrologueManager.PlayerDisplay>(json);
