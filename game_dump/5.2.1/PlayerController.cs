...
public class PlayerController : Durango.Utils.Singleton<PlayerController>
{
	...
	public float RotateSpeed
	{
		get
		{
			if (Player.IsRiding)
			{
				return Player.Driver.RotateSpeed;
			}
			return _rotateSpeed;
		}
		set
	...
	protected override void OnAwake()
	{
		...
		PlayerBehavior obj = (PlayerBehavior.LocalPlayer = Durango.Utils.Singleton<PlayerManager>.Instance().MakePlayerObject(male: false, null, GameManager.PlayerId, "Barehand_Stand", loadClips: false));
		obj.gameObject.name = "Player";
		obj.Revived += Player_Revived;
		obj.Died += Player_Died;
		obj.TileChanged += Player_TileChanged;
		obj.MotionConditionChanged += Player_MotionConditionChanged;
	...
	private void ProcessInputDirectionalMove()
	{
		...
		{
			bool num = _manualMoveDir.sqrMagnitude > 0f;
			_lastMoveDir = null;
			_manualMoveDir = Vector3.zero;
			if (num && this.TryMoveWhenLocked != null)
			{
				this.TryMoveWhenLocked();
			}
		}
		Vector3 vector = _manualMoveDir;
		bool flag = Math.Abs(vector.sqrMagnitude) > 0f;
		if (flag && _ignoreSimilarMoveDir)
		{
			_ignoreSimilarMoveDir = false;
			Vector3? lastMoveDir = _lastMoveDir;
			if (lastMoveDir.HasValue && Vector2.Angle(new Vector2(vector.x, vector.z), new Vector2(_lastMoveDir.Value.x, _lastMoveDir.Value.z)) < 10f)
			{
				vector = Vector3.zero;
				flag = false;
		...
		if (flag != _manualDirectionalMove)
		{
			_manualDirectionalMove = flag;
	...
	{
		if (Player.IsAlive && _manualDirectionalMove)
		{
			return !GameSystem<InputSystem>.Instance().MovedByManual();
		}
		return false;
	}
	...
	public bool IsMovablePosition(Vector3 clientPos)
	{
		...
		if (!Durango.Utils.Singleton<TerrainBase>.Instance().IsCollidableMasked(vector))
		{
			return !TerrainWater.IsTooDeepToSwim(Durango.Utils.Singleton<TerrainBase>.Instance().GetWaterDepth(vector), Player.SwimmableDepthRatio);
		}
		return false;
	}
	...
	public bool CanTryJump()
	{
		...
		if (localPlayer.IsAlive && waterDepthLevel <= TerrainWater.WaterDepthLevel.Foot && (bool)localPlayer.IsMoving && _manualDirectionalMove && !localPlayer.IsRiding)
		{
			return !localPlayer.IsTired;
		}
		return false;
	}
	...
	public void PrepareCPR(CharacterBehavior target)
	{
		...
		float yaw = Maths.CalcYawWithTarget(InteractionObject.GetInteractionPosition(target.gameObject), Player.CurrentPosition);
