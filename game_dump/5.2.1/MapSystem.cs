...
public class MapSystem : GameSystem<MapSystem>
{
	...
	public bool CanPurchaseMap
	{
		get
		{
			if (GameManager.Region.Role() == Role.Risky)
			{
				return !_exploredPOIs.IsOpenedMap;
			}
			return false;
		}
	}
	...
	private static void TravelToStableRegion(string regionId)
	{
		...
		{
			Packet.IsSuccess(packet);
		});
	}

	public static void RequestBiomes(string terrainId, Action<byte[]> onSucceeded, Action onFailed)
	{
		Http.Request(GameManager.GatewayUrl + "/terrains/" + terrainId + "/whole_biomes", delegate(byte[] bytes, HTTPResponse response)
		{
			if (bytes == null)
			{
				if (onFailed != null)
