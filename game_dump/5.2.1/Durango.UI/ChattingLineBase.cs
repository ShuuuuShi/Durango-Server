...
public class ChattingLineBase : MonoBehaviour
{
	...
	public ChatStruct.ChatMsgType MsgType
	{
		get
		{
			if (ChatData == null)
			{
				return ChatStruct.ChatMsgType.Talk;
			}
			return ChatData.MsgType;
		}
	}
	...
	private void SetEventMessage(ChatStruct chat, IList<Durango.Player.PlayerInfo> playerInfos)
	{
		...
		{
			text2 = T._("{0} 님이 입장했습니다.", text);
		}
		else if (chat.Body is RadioLeft)
		{
			text2 = T._("{0} 님이 퇴장했습니다.", text);
		}
		if (!string.IsNullOrEmpty(chat.Name) && chat.Name.StartsWith("#"))
		{
			string text3 = T._(LocalizeSystem.Get(chat.Name), text);
			if (!string.IsNullOrEmpty(text3))
			{
				text2 = text3;
			}
		}
		string text4 = "[7e7e64][icon=icon_person] " + text2 + "[-]";
		_eventMessageLabel.text = text4;
		UpdateWidgetHeight();
	}
