...
public class RouteInfoTooltip : MonoBehaviour
{
	...
	private void Refresh()
	{
		...
		{
			string arg = T._("{0:lv:} {1}", _route.Region().Level, LocalizeUtil.Get(_route.Region().Role()));
			string text = $"[size=20][c][9F9784]{arg}[-][/c][/size]\n{_route.Region().Name}";
		...
		else
		{
			...
			string text2 = Durango.Logic.Item.Inventory.ToCurrencyButtonText(T._("항해"), money.Amount, money.Currency);
			Tooltip.SetButton(text2, regionId, OnClickButton);
		}
		if (!IsVisible)
		{
			IsVisible = true;
	...
	public static void ShowUnknown(InfoTooltip tooltip, Action<Route> onClick)
	{
		...
		routeInfoTooltip.SetUnknown(onClick);
	}
