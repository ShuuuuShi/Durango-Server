...
public class EditPlayerCostumePage : MonoBehaviour, IEditPlayerDisplayPage
{
	...
	private void SetTextList(IList<string> texts)
	{
		...
		{
			UILabel component = nodes.GetNext().transform.Find("Text").GetComponent<UILabel>();
	...
	private void SetColorList(IList<Color> colors)
	{
		...
		{
			UISprite component = nodes.GetNext().transform.Find("Bg").GetComponent<UISprite>();
			component.color = colors[i];
		}
		nodes.EndLoad();
