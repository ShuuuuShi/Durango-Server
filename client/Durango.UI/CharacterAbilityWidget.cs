...
public class CharacterAbilityWidget : UIWidget
{
	...
	private void Init()
	{
		if (!_isInit)
		{
			...
			foreach (RepresentType representType in types)
			{
				...
				RectLayoutComponent component = next.GetComponent<RectLayoutComponent>();
				component.UpdateOnSizeChange();
	...
	private void AddResistance()
	{
		...
		{
			GameObject gameObject = _scrollView.Nodes.Add();
			string text = string.Format("[icon=icon_skin_hardness:1.25]  {0}", T._("신체 저항 레벨"));
			gameObject.FindComponent<UILabel>("Key").text = text;
			gameObject.GetComponent<RectLayoutComponent>().UpdateOnSizeChange();
	...
	public void Refersh()
	{
		...
		{
			GameObject obj = _scrollView.Nodes[i];
			UILabel uILabel = obj.FindComponent<UILabel>("Value");
		...
		{
			GameObject obj2 = _scrollView.Nodes[size];
			UILabel uILabel2 = obj2.FindComponent<UILabel>("Value");
			uILabel2.text = GameSystem<StatisticsSystem>.Instance().GetAverageResistanceLevel().ToString();
		}
