...
public class SelectableButton : Selectable, RectLayout.ICompatible
{
	...
	public void SetEffect(PresetButton.Effect effect)
	{
		...
		if (!(effect2 == null))
		{
			...
			Transform transform = _effectObject.transform;
	...
	private void MakeButton()
	{
		...
		if (style != _prevStyle)
		{
			...
			Transform transform = _button.transform;
			transform.localPosition = Vector3.zero;
			transform.localRotation = Quaternion.identity;
			transform.localScale = Vector3.one;
			...
			Transform transform2 = _button.transform.Find("Sub");
			if (transform2 != null)
			{
				_subButton = transform2.GetComponent<Selectable>();
		...
		while (stack.Count > 0)
		{
			Transform transform3 = stack.Pop();
			transform3.gameObject.hideFlags = HideFlags.DontSave | HideFlags.NotEditable;
			int j = 0;
			for (int childCount = transform3.childCount; j < childCount; j++)
			{
				stack.Push(transform3.GetChild(j));
			}
		}
	}
