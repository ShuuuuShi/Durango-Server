...
public class MapIndicators : Durango.Utils.Singleton<MapIndicators>
{
	...
	protected override void OnAwake()
	{
		...
		MapIndicator[] array = componentsInChildren;
		foreach (MapIndicator mapIndicator in array)
		{
	...
	{
		return (!Durango.Utils.Singleton<MapIndicators>.HasInstance()) ? ((T)null) : Durango.Utils.Singleton<MapIndicators>.Instance().GetOrAddIndicator<T>(id, type);
	}
	...
	public void HideTypesClear()
	{
		...
		{
			MapIndicator mapIndicator = _indicators[j];
			mapIndicator.ToggleHideFlag(MapIndicator.HideFlag.Type, hide: false);
		}
	}
	...
	{
		IndicatorLabel indicatorLabel = _indicatorLabels.Add();
	...
	private void UpdateIndicatorLabels()
	{
		...
		{
			IndicatorLabel indicatorLabel = _indicatorLabels[i];
			indicatorLabel.UpdatePosition(_indicatorLabelOffset);
		}
	}
	...
	private void UpdateIndicators()
	{
		...
		for (int num = _indicators.Count - 1; num >= 0; num--)
		{
			...
			else
			{
				...
				{
					Transform transform = mapIndicator.transform;
	...
	{
		MapAnimalIndicator orAddIndicator = GetOrAddIndicator<MapAnimalIndicator>(animal.EntityId, IndicatorType.Animal);
		orAddIndicator.SetAnimal(animal);
	}
	...
	private void OnUpdateWarpAccelerators()
	{
		...
		{
			if (item.Warpaccelerator.CurrentPhase > 0)
			{
				MapWarpAcceleratorIndicator orAddIndicator = GetOrAddIndicator<MapWarpAcceleratorIndicator>(item.EntityId, IndicatorType.WarpAccelerator);
				orAddIndicator.SetInfo(item);
				value.Remove(item.EntityId);
			}
		}
		foreach (string item2 in value)
		{
			Remove(item2, IndicatorType.WarpAccelerator);
		}
	}
	...
	private void PartySystem_MembersUpdated()
	{
		...
		for (int size = KUtility.GetSize(_indicators); i < size; i++)
		{
			...
			{
				MapPlayerIndicator mapPlayerIndicator = _indicators[i] as MapPlayerIndicator;
				mapPlayerIndicator.SetPartyMember(null, -1);
			}
		}
		...
		for (int memberCount = partySystem.MemberCount; j < memberCount; j++)
		{
			...
			{
				MapPlayerIndicator orAddIndicator = GetOrAddIndicator<MapPlayerIndicator>(member.EntityId, IndicatorType.Player);
				orAddIndicator.SetPartyMember(member, j + 1);
			}
		}
	}
	...
	private void WarpRushSystem_MembersUpdated()
	{
		...
		for (int size = KUtility.GetSize(_indicators); i < size; i++)
		{
			...
			{
				MapPlayerIndicator mapPlayerIndicator = _indicators[i] as MapPlayerIndicator;
				mapPlayerIndicator.SetWarpRushMember(null);
			}
		}
		WarpRushSystem warpRushSystem = GameSystem<WarpRushSystem>.Instance();
		foreach (Durango.Logic.WarpRush.Member member in warpRushSystem.Members)
		{
			if (!(member.EntityId == PlayerBehavior.LocalPlayer.EntityId))
			{
				MapPlayerIndicator orAddIndicator = GetOrAddIndicator<MapPlayerIndicator>(member.EntityId, IndicatorType.Player);
				orAddIndicator.SetWarpRushMember(member);
			}
		}
	}
	...
	private void UpdateArtifactIndicator(Artifact artifact)
	{
		if (!TerrainMeta.HasGlobalIndicator(artifact.EntityType, artifact.WorldTile))
		{
			...
			{
				MapArtifactIndicator orAddIndicator = GetOrAddIndicator<MapArtifactIndicator>(artifact.EntityId, IndicatorType.Artifact);
				orAddIndicator.SetArtifact(artifact.gameObject, artifact.Blueprint.Indicator);
			}
			else
			{
				Remove(artifact.EntityId, IndicatorType.Artifact);
