...
public class AccessRightsManageGroup : UIBase
{
	...
	private void _MakeTabList()
	{
		...
		{
			_tabScrollView.Nodes.GetNext().GetComponent<AccessRightsTab>().Set(subText: (array != null && array.Length > i) ? array[i].ToString() : string.Empty, text: nameText);
		}
	...
	private void SelectTab(int index)
	{
		if (_isLoaded)
		{
			...
			{
				_tabScrollView.Nodes[i].GetComponent<AccessRightsTab>().Selected = i == index;
			}
