...
public class UIInput : MonoBehaviour
{
	...
	public bool ShouldWaitFlushedInputString { get; set; }
	...
	protected void Init()
	{
		if (mDoInit && label != null)
		{
			...
			label.wrapAlways = true;
			mEllipsis = label.overflowEllipsis;
			mSupportEncoding = label.supportEncoding;
	...
	protected void OnDeselectEvent()
	{
		...
		if (label != null && NGUITools.GetActive(this))
		{
			...
			{
				label.supportEncoding = mSupportEncoding;
	...
	protected virtual void Update()
	{
		...
		if (ShouldWaitFlushedInputString)
		{
			ShouldWaitFlushedInputString = false;
			return;
		}
	...
	private void OnKey(KeyCode key)
	{
		...
		if (mIgnoreKey != frameCount && mCam != null && (key == mCam.cancelKey0 || key == mCam.cancelKey1))
		{
	...
	public virtual bool ProcessEvent(Event ev)
	{
		...
		case KeyCode.DownArrow:
			...
			if (!string.IsNullOrEmpty(mValue))
			{
				...
				if (mSelectionEnd != label.ProcessedTokens.Count)
				{
					mSelectionEnd += mDrawStart;
				}
				else
	...
	public void UpdateLabel()
	{
		...
		label.supportEncoding = !flag && flag2 && mSupportEncoding;
		label.text = text2;
		if (flag)
		{
