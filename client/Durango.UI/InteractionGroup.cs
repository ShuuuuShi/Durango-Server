...
public class InteractionGroup : UIBase
{
	...
	private void OnTouchPickingObject(InputCommandMessage message)
	{
		...
		GameObject selectedObject = ((_selectedPickingObject != null) ? _selectedPickingObject.Target : null);
		bool isPrev;
		GameObject gameObject = InteractionUtil.PickingObject(selectedObject, pickingRay, pickingTouchEvent.CurrentPos, out isPrev, null);
		if (gameObject != null)
		{
	...
	private static void AddInteractionHandler()
	{
		...
		{
			string mainText2 = ((GameManager.Region.Role() == Role.Risky) ? ((!GameSystem<MapSystem>.Instance().Points.CampPoint.HasValue) ? T._("부활하시겠습니까?\n죽은 곳에 아이템 일부가 떨어집니다.\n(상점에서 구매한 아이템 포함)") : T._("캠프로 <em>귀환</em>해 부활하시겠습니까?\n죽은 곳에 아이템 일부가 떨어집니다.\n(상점에서 구매한 아이템 포함)")) : ((!GameSystem<StatisticsSystem>.Instance().IsNewbie) ? T._("<em>귀환 지점</em>에서 부활하시겠습니까?\n죽은 곳에 아이템 일부가 떨어집니다.\n(상점에서 구매한 아이템 포함)") : T._("<em>귀환</em> 하시겠습니까?")));
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.ReviveImmediately, delegate
		{
			...
			if (reviveImmediately.HasVoucherFromCommodity())
			{
				...
				messageBox2.SetLowerText(packageEffectText);
			...
			messageBox2.SetVoucherInfo(reviveImmediately.GetVoucher().VoucherId);
			...
			reference2 = T._("취소");
			messageBox2.Show(mainText, onSelect, array);
		});
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.Whisper, delegate(InteractionObject target)
		{
			...
			if (!(targetComponent4 == null) && !string.IsNullOrEmpty(targetComponent4.EntityId))
			{
				...
				chattingGroupBase.Open(targetComponent4.EntityId);
			}
		});
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.GiveUpDistribution, delegate(InteractionObject target)
		{
			...
			messageBox.Show(T._("전리품의 권한을 포기하시겠습니까?"), T._("[icon=icon_make_alert] 모두에게 전리품 획득 권한이 공유됩니다."), delegate(bool ok)
			{
				if (ok)
				{
