...
public class ColorSelectorWidget : MonoBehaviour
{
	...
	public void FillData()
	{
		...
		{
			_sprite[i].GetComponent<UISprite>().color = array[i];
		}
		...
		{
			num = Mathf.Max((int)_tab[j].GetComponent<ColorSelectorTab>().Set(_tabs[j]), num);
		}
		_tabWidget.width = num;
		int k = 0;
		for (int count3 = _tab.Count; k < count3; k++)
		{
			ColorSelectorTab component = _tab[k].GetComponent<ColorSelectorTab>();
			component.Select(k == _currentTab);
	...
	public void UpdateLayout(int height = -1)
	{
		...
		int num4 = _tab.BaseObject.GetComponent<UIWidget>().height + 10;
		Vector3 localPosition3 = _tab.BaseObject.transform.localPosition;
		int j = 0;
		for (int count2 = _tab.Count; j < count2; j++)
		{
			_tab[j].transform.localPosition = localPosition3 + Vector3.down * j * num4;
		}
	}
}
