...
public class ItemList : EmptyBoxScrollView, IEnumerable<ItemData>, IEnumerable
{
	...
	public int SelectableCount
	{
		get
		{
			return (_selectableCountGetter == null) ? _selectableCount : _selectableCountGetter();
		}
		set
		{
	...
	public bool IsMultiSelectMode => _selectableCount == -1 || _selectableCount > 1;
	...
	[CanBeNull]
	public ItemData LastSelectedItem => (_selectedList.Count <= 0) ? null : _selectedList[_selectedList.Count - 1];
	...
	private void SelectItemIcon(ItemIconWidget icon, bool sendEvent, bool scrollTo, bool deselectSelectedItem = true)
	{
		...
		if (icon.IsSelectable && (EquipmentsSelectable || !itemData.IsEquipments))
		{
			int num = SelectedIndexOf(itemData.Id);
			bool flag = true;
			if (num == -1)
			{
				_selectedList.Add(itemData);
	...
	public void DeselectSimilarItems(string prototypeId)
	{
		...
		{
			ItemData itemData = _selectedList[num];
			if (!(itemData.PrototypeId != prototypeId))
			{
				_selectedList.RemoveAt(num);
			}
		}
