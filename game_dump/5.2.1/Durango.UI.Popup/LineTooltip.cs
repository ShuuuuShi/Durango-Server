...
public class LineTooltip : TooltipBase
{
	...
	{
		int index = go.GetComponent<LineTooltipItem>().Index;
	...
	protected override void FillData()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				string text2 = ((!_lines[i].HideChild) ? _defaultMinusMacker : _defaultPlusMacker);
				text = "[icon=" + text2 + "]";
			}
	...
	protected override void UpdateLayout()
	{
		...
		{
			_lineItems[_lineItems.Count - 1].GetComponent<LineTooltipItem>().LineActive(active: false);
	...
	private static bool IsNodeType(Type type)
	{
		...
		{
			return Array.IndexOf(NodeTypes, type) != -1;
		}
		return true;
	}
