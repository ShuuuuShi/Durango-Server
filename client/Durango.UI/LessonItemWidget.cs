...
public class LessonItemWidget : MonoBehaviour
{
	...
	{
		LearningGuideSystem learningGuideSystem = GameSystem<LearningGuideSystem>.Instance();
		bool clickable = learningGuideSystem.TargetAdvice == subject;
		for (int i = 0; i < _learningSkillWidgets.Count; i++)
		{
			LearningSkillWidget learningSkillWidget = _learningSkillWidgets[i];
			learningSkillWidget.Refresh(clickable);
		}
	}
	...
	private void SetSkills(Lesson lesson, bool clickable)
	{
		...
		{
			LearningSkillWidget learningSkillWidget = CreateNewSkillWidget(localPosition, showUpperDotLine);
			learningSkillWidget.SetSkill(requiredSkill, clickable);
		...
		if (_learningSkillWidgets.Count < 2)
		{
			...
			{
				LearningSkillWidget learningSkillWidget2 = CreateNewSkillWidget(localPosition, showUpperDotLine);
				learningSkillWidget2.SetSkill();
				localPosition += Vector3.down * _skillWidgetHeight;
				showUpperDotLine = true;
			}
		}
