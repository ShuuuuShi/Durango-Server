...
public class ReceiveRewardsPopup : TooltipBase
{
	...
	private void Show(Argument arg)
	{
		...
		for (int size = KUtility.GetSize(arg.Items); i < size; i++)
		{
			...
			{
				RewardItemWidget component = _kScrollView.Nodes.GetNext().GetComponent<RewardItemWidget>();
				component.SetTitle(itemArgument.Title, itemArgument.GetSubText()).SetIcon(itemArgument.Icon, itemArgument.IconColor, itemArgument.IconRTable, itemArgument.IconGTable, itemArgument.IconBTable).SetSupText(itemArgument.Sup)
		...
		for (int size2 = KUtility.GetSize(arg.Items); j < size2; j++)
		{
			...
			{
				RewardItemWidget component2 = _kScrollView.Nodes.GetNext().GetComponent<RewardItemWidget>();
				component2.SetTitle(itemArgument2.Title, itemArgument2.GetSubText()).SetIcon(itemArgument2.Icon, itemArgument2.IconColor, itemArgument2.IconRTable, itemArgument2.IconGTable, itemArgument2.IconBTable).SetSupText(itemArgument2.Sup)
	...
	public static void AddRewardedItems(List<ItemArgument> items, RewardInfo reward, bool isBonus)
	{
		...
		{
			Messages.RewardItem[] items2 = reward.Items;
			foreach (Messages.RewardItem msg2 in items2)
			{
		...
		{
			string[] blueprintIds = reward.BlueprintIds;
		...
		{
			string[] recipeIds = reward.RecipeIds;
	...
	{
		if (currencyValue != 0 && currency != Currency.Invalid)
		{
			items.Add(new ItemArgument
			{
