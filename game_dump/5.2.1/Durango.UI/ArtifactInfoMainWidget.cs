...
public class ArtifactInfoMainWidget : MonoBehaviour
{
	...
	public void UpdateScrollOffset()
	{
		...
		else
		{
			...
			float b2 = 0f - ((float)_titleHeight - num);
			num3 = Mathf.Max(currentOffset, b2);
		}
	...
	private void FillStats()
	{
		...
		{
			GetInfoLabel(OnClickComfortStats).Set(T._("총 안락함"), GetStatFactorText(value.Comfort.Factor, value.Comfort.Complexity));
			GetInfoLabel(OnClickAntibacterialStats).Set(T._("총 항균력"), GetStatFactorText(value.Antibacterial.Factor, value.Antibacterial.Complexity));
			return;
		}
		if (value.Comfort.Factor != 0)
		{
			GetInfoLabel(OnClickComfortStats).Set(string.Format("{0}[icon=img_loading_unknown_question2]", T._("안락함")), value.Comfort.Factor.ToString());
		}
		if (value.Antibacterial.Factor != 0)
		{
			GetInfoLabel(OnClickAntibacterialStats).Set(string.Format("{0}[icon=img_loading_unknown_question2]", T._("항균력")), value.Antibacterial.Factor.ToString());
		}
	}
	...
	private void FillWarehouse()
	{
		...
		{
			GetInfoLabel().Set(T._("{0:l:{}|, }", reusable.Value), null);
		}
	}
	...
	private void FillEffector()
	{
		if (_artifact.ArtifactState.Effector.HasValue)
		{
			...
			GetInfoLabel().Set(T._("남은 사용 횟수"), T._("{0}회", value.RemainCount));
		}
	}
	...
	private void FillInventory()
	{
		...
		if (KUtility.GetSize(value.StorableTags) > 0)
		{
			...
			if (list.Count > 0)
			{
				GetInfoLabel().Set(T._("보관 물품 종류"), T._("{0:l:{}|, }", list));
			}
		}
		if (KUtility.GetSize(value.UnstorableTags) > 0)
		{
			...
			if (list2.Count > 0)
			{
				GetInfoLabel().Set(T._("보관 불가 품목"), T._("{0:l:{}|, }", list2));
			}
		}
		if (value.ReduceDurabilityVelocity > 0f)
		{
			GetInfoLabel().Set(T._("내구도 감소 완화 정도"), $"{value.ReduceDurabilityVelocity:p0}");
		}
	}

	private void FillLandowner()
	{
		if (_artifact.GetArtifactComponent<Landowner>() == null)
		{
			return;
		}
		...
		if (estateInfo != null && estateInfo.License.IsProtected())
		{
			...
			GetInfoLabel().Set(T._("보호기간 종료까지 남은 시간"), new SyncString(delegate(out string text, out float period)
			{
			...
			GetInfoLabel().Set(T._("보호 기간이 종료되면 화물 워프홀은 동작을 정지합니다. 전쟁에서 일정 기간 방어에 성공하면 다시 보호 기간이 됩니다."), null);
		}
	}
	...
	private void FillCatapult()
	{
		if (_artifact.ArtifactState.Catapult.HasValue)
		{
			...
			GetInfoLabel().Set(T._("투척물"), $"{value.RemainedProjectilesSize} / {value.MaxProjectilesSize}");
		}
	}
	...
	private void FillDurability(out string text, out float period)
	{
		...
		{
			bool flag = ((artifactState.BuildingState == BuildingState.Completed) ? (_artifact.Condition >= Shared.Building.Condition.Worn) : (num3 > 0.0 && num4 < 86400.0));
	...
	{
		if ((float)Mathf.Abs(complexity) > 0f)
		{
			return $"<alert>{factor}</alert> [icon=icon_information]";
		}
		return $"{factor} [icon=icon_information]";
	}
