...
public class ClientAnimalActor : MonoBehaviour
{
	...
	private void PlayWander()
	{
		...
		Vector3 clientPosition = BasePosition + new Vector3(Mathf.Cos(f) * _wanderRadius, 0f, Mathf.Sin(f) * _wanderRadius);
		Vector3 end = Util.ClientPositionToWorldPosition(clientPosition);
	...
	private float ChooseWanderRadian()
	{
		if (HasBasePosition)
		{
			...
			return num + (float)Math.PI * UnityEngine.Random.value;
		}
		InitializeBasePosition();
