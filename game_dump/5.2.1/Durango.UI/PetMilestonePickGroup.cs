...
public class PetMilestonePickGroup : UIBase
{
	...
	public void ShowPetMilestonePick(Messages.Pet pet, int milestoneId, Action picked)
	{
		...
		int num;
		string text;
		if (!_pet.Stat.LastMilestoneAccepted)
		{
			num = ((!PetUtil.HasAcquiredMilestone(pet)) ? 1 : 0);
			if (num == 0)
			{
				text = T._("현재 적용 중인 속성");
				goto IL_0086;
			}
		}
		else
	...
	private void SetPreviewModel(Messages.Pet pet)
	{
		...
		bool num = pet.Stat.Life == null || pet.Stat.Life.Ratio() <= 0f;
	...
	{
		UIManager.MessageBox.Show(T._("정말 진행하시겠습니까?"), T._("<alert>[icon=icon_make_alert] 재접속, 전투, 상점 등으로 화면을 전환하면 의도와 다른 속성 및 특수 행동으로 결정될 수 있습니다. 안전한 장소에서 진행하는 것을 추천합니다.</alert>"), delegate(bool ok)
		{
			if (ok)
			{
				Confirm();
	...
	private void RequestRedrawActiveSkill()
	{
		...
		if (lastDrawSkillResult.HasValue)
		{
			GetRevertActiveSkillCost(_lastDrawSkillResult.Value).Payable(InventorySystem.Wallet, out byVoucher, out byCurrency);
		}
		else
		{
			Money? retryCost = _pet.Stat.RetryCost;
			if (retryCost.HasValue)
			{
				GetRevertActiveSkillCost(_pet.Stat.RetryCost.Value).Payable(InventorySystem.Wallet, out byVoucher, out byCurrency);
			}
		}
		_isChangedPet = true;
	...
	private void RequestPickMilestoneAgain()
	{
		...
		if (lastMilestoneResult.HasValue)
		{
			GetRevertMilestoneCost(_lastMilestoneResult.Value).Payable(InventorySystem.Wallet, out byVoucher, out byCurrency);
		}
		else
		{
			Money? retryCost = _pet.Stat.RetryCost;
			if (retryCost.HasValue)
			{
				GetRevertMilestoneCost(_pet.Stat.RetryCost.Value).Payable(InventorySystem.Wallet, out byVoucher, out byCurrency);
			}
		}
