...
public class JiggleBonesController : MonoBehaviour
{
	...
	public class JiggleBone
	{
		...
		private void ApplySphereConstraint(List<SphereConstraint> sphereConstraints)
		{
			foreach (SphereConstraint sphereConstraint in sphereConstraints)
			{
				...
				if (!(vector.sqrMagnitude >= sphereConstraint.RadiusSquared))
				{
					vector.Normalize();
		...
		private Vector3 ApplyLimit(Vector3 goalForward, Vector3 goalTip, Vector3 goalBasePosition, float length)
		{
			...
			vector.Normalize();
			if (Mathf.Acos(Vector3.Dot(vector, goalForward)) * 57.29578f > _data.AngleLimit)
			{
				float num = length * Mathf.Sin(_data.AngleLimit * ((float)Math.PI / 180f));
				Vector3 vector2 = goalTip - _tipPos;
				vector2.Normalize();
				_tipPos = goalTip - num * vector2;
	...
	public void UpdateFramework(Dictionary<string, Transform> childTransformCache = null)
	{
		...
		if (childTransformCache == null)
		{
			...
			Transform[] componentsInChildren = _owner.transform.GetChild(0).GetComponentsInChildren<Transform>();
		...
		{
			sphereConstraint.UpdateBone(childTransformCache);
		}
	}
	...
	private void LateUpdate()
	{
		if ((!(_owner != null) || _owner.WillBeRendered) && _jiggleBones.Count > 0)
		{
			...
			{
				_sphereConstraints[i].Update();
			}
			int j = 0;
			for (int count2 = _jiggleBones.Count; j < count2; j++)
			{
				_jiggleBones[j].Update(_sphereConstraints);
			}
		}
	}
	...
	public void Add(JiggleBonesController srcController, Transform[] dstBones, CharacterCostume.CostumeType type)
	{
		...
		{
			if (jiggleBonesData[num].Bone == null)
			{
		...
		{
			GameObject gameObject = new GameObject(jiggleBonesData[i].Bone.name);
			array[i] = gameObject.transform;
		}
		for (int j = 0; j < size; j++)
		{
			JiggleBoneData jiggleBoneData = jiggleBonesData[j];
			JiggleBoneData jiggleBoneData2 = jiggleBoneData.Clone();
			string parentName = jiggleBoneData2.Bone.parent.name;
			Transform transform = FindParent(dstBones, parentName);
			if (transform == null)
			{
				transform = FindParent(array, parentName);
			}
			jiggleBoneData2.Bone = array[j];
			jiggleBoneData2.Bone.parent = transform;
			jiggleBoneData2.Bone.localPosition = jiggleBoneData.Bone.localPosition;
			jiggleBoneData2.Bone.localRotation = jiggleBoneData.Bone.localRotation;
			jiggleBoneData2.Bone.localScale = jiggleBoneData.Bone.localScale;
			JiggleBone jiggleBone = new JiggleBone(jiggleBoneData2, jiggleBoneData, srcController);
			jiggleBone.Type = type;
			_jiggleBones.Add(jiggleBone);
			_jiggleBonesData.Add(jiggleBoneData2);
		}
	}
