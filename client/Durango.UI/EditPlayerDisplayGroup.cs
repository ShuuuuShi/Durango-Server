...
public class EditPlayerDisplayGroup : UIBase
{
	...
	private void SetPage(int page, bool instant)
	{
		...
		IEditPlayerDisplayPage editPlayerDisplayPage = _pageViewers[(int)page3.State];
		editPlayerDisplayPage.WaitForLoading(loading: false);
		editPlayerDisplayPage.SetConfirmText((page >= _pages.Count - 1) ? T._("확인") : T._("다음"));
	}
