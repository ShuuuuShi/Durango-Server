...
public class AnimationEventController : MonoBehaviour
{
	...
	public Vector3 GetEmitPosition(AnimationEventInfo.Position positionType, Vector3 position, Transform target, bool isTrail)
	{
		...
		Transform transform = ((positionType != AnimationEventInfo.Position.ToBoneLocal) ? base.gameObject.transform : target);
		Vector3 vector = transform.TransformVector(position);
		return (!isTrail) ? (target.position + vector) : vector;
	}
	...
	public Transform FindTargetBone(string boneName)
	{
		...
		return (!(transform == null)) ? transform : base.transform;
	}
	...
	private void EmitParticleNew(AnimationEventInfo animEventInfo, string effectPath)
	{
		...
		case AnimationEventInfo.Emission.PulledTowardCamera:
		{
			...
			bool flag = animEventInfo.EmissionType == AnimationEventInfo.Emission.PulledTowardCamera;
			string assetPath = effectPath;
			Vector3 emitPosition = vector2;
			Quaternion rotation = emitRotation;
			bool comeForwardToCamera = flag;
			Vector3 scale = vector;
			particleId = ParticleManager.Emit(assetPath, emitPosition, rotation, comeForwardToCamera, groundDecal: false, scale);
			break;
		}
		case AnimationEventInfo.Emission.Attach:
		{
			string assetPath = effectPath;
			Vector3 scale = animEventInfo.EmitPosition;
			Quaternion rotation = Quaternion.Euler(animEventInfo.EmitRotation);
			Transform followingParent = transform;
			Vector3 emitPosition = vector;
			particleId = ParticleManager.EmitFollow(assetPath, scale, rotation, followingParent, useLocalPosition: true, comeForwardToCamera: false, groundDecal: false, emitPosition);
			break;
		}
		case AnimationEventInfo.Emission.ChasePosition:
		{
			...
			particleId = ParticleManager.EmitFollow(assetPath, emitPosition, rotation, followingParent, useLocalPosition: true, comeForwardToCamera: false, animEventInfo.AdjustHeightToGround, scale, chasingTarget);
			break;
		}
		}
		OnParticleEmitted(particleId, animEventInfo.removeEnded);
	...
	private void EmitSound(AnimationEventInfo animEventInfo)
	{
		...
		{
			string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(animEventInfo.gameObjectPath);
			uint num = SoundManager.PlayEvent(fileNameWithoutExtension, (!animEventInfo.Use2dSound) ? SoundPosition.Fix(_targetPlayable.GetCurrentPosition()) : SoundPosition.Empty);
	...
	private PlayerBehavior GetTargetPlayer()
	{
		...
		return (!(gameObject != null)) ? null : gameObject.GetComponent<PlayerBehavior>();
	}

	private CharacterBehavior GetTargetCharacter()
	{
		GameObject gameObject = _targetPlayable.GetGameObject();
		return (!(gameObject != null)) ? null : gameObject.GetComponent<CharacterBehavior>();
	}
	...
	private void SetTrailOption(AnimationEventInfo animEventInfo, float timePassed, PlaneTrail planeTrail, CharacterBehavior character)
	{
		...
		if (planeTrail.Option == PlaneTrail.FixOption.Baked)
		{
			...
			{
				trailData = (animEventInfo.TrailData = Json.Read<TrailBaker.TrailData>(animEventInfo.BakedTrailData));
			}
			if (trailData == null || KUtility.GetSize(trailData.BasePoints) == 0)
			{
	...
	private void LocateProp(AnimationEventInfo animEventInfo)
	{
		...
		Singleton<AssetBundleManager>.Instance().RequestAsset(path, typeof(GameObject), delegate(UnityEngine.Object asset)
		{
			...
			if (!(gameObject == null) && !(this == null))
			{
				...
				GameObject gameObject2 = (GameObject)UnityEngine.Object.Instantiate(asset);
				gameObject2.transform.position = Vector3.zero;
				gameObject2.transform.rotation = Quaternion.identity;
				Transform transform3 = KUtility.FindTransformByName(gameObject2, animEventInfo.BoneName);
				if (transform3 != null)
				{
				...
				{
					gameObject2.transform.Translate(animEventInfo.EmitPosition);
				}
				if (animEventInfo.paramVector2 != Vector3.zero)
				{
					gameObject2.transform.Rotate(animEventInfo.paramVector2);
				}
				_attachedProps.Add(new LocatedPropInfo
				{
					Path = path,
					PropObject = gameObject2,
