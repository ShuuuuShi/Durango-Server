...
public class AlarmMessageQueue : MonoBehaviour, AlarmRewardQueue.IMessageGroup
{
	...
	private void UpdatePosition()
	{
		...
		{
			_msgLabels[i].UpdatePosition(_animSpeed * Time.deltaTime);
		}
	}
		...
		for (int count = _msgLabels.Count; i < count; i++)
		{
			...
			if (alarmMessageWidget.Index == -1)
			{
				if (i < count - 1 || count == 1)
				{
					...
					alarmMessageWidget.TargetPosition = vector3;
				}
				else
				{
					transform.localPosition = vector3 + Vector3.down * 10f;
				}
			}
	...
	{
		AlarmMessageWidget obj = ((_msgLabelPool.Count <= 0) ? _messageComponentBase.transform.parent.gameObject.AddChild(_messageComponentBase.gameObject).GetComponent<AlarmMessageWidget>() : _msgLabelPool.Pop());
