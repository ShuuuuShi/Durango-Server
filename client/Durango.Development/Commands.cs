...
public class Commands : Singleton<Commands>
{
	...
	private void Start()
	{
		...
		_clientCheatDispatcher.RegisterCommand("pos", delegate
		{
			...
			Vector2 vector2 = Util.ClientPositionToTilePosition(PlayerBehavior.LocalPlayer.CurrentPosition);
			return true;
		});
		...
		Dictionary<string, string[]> dictionary = Json.ReadFromFile<Dictionary<string, string[]>>("cheat_macro_definition");
	...
	private bool ClientCheatMoveMinimapCoordinate(string strX, string strY)
	{
		...
		Vector3 position = MapPositionParser.HumaneTileToPosition(new Vector2(x, y));
	...
	private bool ClientCheatUnlockAllMenuItems()
	{
		...
		foreach (MenuType item in values)
		{
			GameSystem<MenuSystem>.Instance().EnableMenu(item, enable: true, checkHidden: false);
		}
		return true;
	}
	...
	private bool ClientCheatSpawnHotAirBalloon()
	{
		Singleton<AssetBundleManager>.Instance().RequestAsset("Models/Prop/vehicle/hot_airballoon/hot_airballoon_01.prefab", typeof(GameObject), delegate(UnityEngine.Object asset)
		{
			...
			{
				GameObject gameObject = UnityEngine.Object.Instantiate(asset, Maths.GetRandomSurroundingPos(localPlayer.transform.position, 500f), Quaternion.identity) as GameObject;
			}
		});
		return true;
	}
	...
	private bool ClientCheatDownloadPersonalIsland()
	{
		...
		{
			string json = Json.Write(msg, indented: true);
			DumpedPersonalIsland dumpedPersonalIsland = Json.Read<DumpedPersonalIsland>(json);
		});
		return true;
	}
	...
	{
		string arg = "pe10gr_1";
		string requestUrl = $"https://s3-ap-northeast-1.amazonaws.com/k1server-dumped-islands.k.nexon.com/common/terrains/terrains_{arg}";
		Dictionary<string, byte[]> dictionary = DumpedIslandUtils.DownloadDumpedDatas(requestUrl);
		byte[] array = dictionary["whole.biomes"];
		byte[] bytes = dictionary["1,1"];
		ChunkData chunkData = new ChunkData();
		chunkData.LoadFromBytes(bytes);
		return true;
	}
}
