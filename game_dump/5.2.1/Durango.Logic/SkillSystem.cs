...
public class SkillSystem : GameSystem<SkillSystem>
{
	...
	[CompilerGenerated]
	private sealed class _003CCoInitSkillRewards_003Ed__53 : IEnumerator<object>, IDisposable, IEnumerator
	{
		private int _003C_003E1__state;

		private object _003C_003E2__current;

		public SkillSystem _003C_003E4__this;

		public RewardYaml rewards;
		...
		[DebuggerHidden]
		public _003CCoInitSkillRewards_003Ed__53(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			...
			SkillSystem skillSystem = _003C_003E4__this;
			switch (num)
			{
			default:
				return false;
			...
			if (skillSystem._skills.Count == 0)
			{
				_003C_003E2__current = null;
				_003C_003E1__state = 1;
				return true;
			}
			for (int i = 0; i < skillSystem._skills.Count; i++)
			{
				skillSystem._skills[i].InitRewards(rewards);
			}
			return false;
		}
	...
	private IEnumerator CoInitSkillRewards(RewardYaml rewards)
	{
		...
		return new _003CCoInitSkillRewards_003Ed__53(0)
		{
	...
	public void SkillNeeded(Node skill)
	{
		if (skill != null)
		{
			...
			{
				UIManager.FindScript<SkillGroup>().Open(skill);
			};
			...
			UIManager.Popup.Tooltip<ConfirmPopup>().AddButton(new MessageBox.Button(T._("스킬 보기")), action).Show(comment, 600f);
		}
	}
	...
	private void OnReceiveSkillMsg(Skills msg, PacketHeader header)
	{
		...
		foreach (KeyValuePair<Shared.Skill.Category, Messages.SkillCategory> category in msg.Categories)
		{
			...
			{
				bool num3 = skillCategory.IsResearching();
				bool flag = skillCategory.IsReadyToResearch();
				...
				bool flag2 = skillCategory.IsResearching();
				if (num3 != flag2)
				{
					_eventedCategories.Add((!flag2) ? new KeyValuePair<CategoryEventType, Durango.Logic.Skill.Category>(CategoryEventType.ResearchFinished, skillCategory) : new KeyValuePair<CategoryEventType, Durango.Logic.Skill.Category>(CategoryEventType.ResearchStarted, skillCategory));
				}
				...
				bool flag3 = skillCategory.IsReadyToResearch();
				if (flag != flag3 && flag3)
				{
		...
		{
			num4 += _skills[n].UsedSp();
		}
		RemainSkillPoint = SkillPoint - num4;
		UntrainedCount = msg.UntrainedCount;
		for (int num5 = 0; num5 < Skills.Count; num5++)
		{
			Skills[num5].UpdateState();
		}
		RaiseSkillEvent();
	}
	...
	public static StatusEffect FindSkillExpModifier(Shared.Skill.Category category, out float value)
	{
		...
		foreach (StatusEffect item in GameSystem<StatusEffectSystem>.Instance().GetStatusEffects().List)
		{
