...
## 2026-08-27 — MemoryBot bridge ควบคุมผู้เล่นโดยไม่แย่งเมาส์

- แก้ build ของ `tools/MemoryBotMod` ที่อ้าง `SkillDebug()` ซึ่งไม่มี implementation
- build/install `DurangoMemoryBot.dll` สำเร็จ และโหลดผ่าน real client ครบ 3 เฟส
- ยืนยัน `ping`, `read player.local`, `player.move_to` และ `player.stop` ผ่าน TCP loopback `127.0.0.1:8193`
- ตำแหน่งเปลี่ยนจริงจาก `[8100,35500]` เป็น `[8184.107,35584.11]` โดยไม่มี mouse/keyboard automation

## 2026-08-27 — ยืนยัน mod แบบ real client + server

- เปิด `DurangoV2.exe` ต่อ server จริงพร้อม `ExampleClientMod.dll` และยืนยัน loader ครบ 3 เฟส,
  `OnGameReady` และ F10 ผ่าน `game/clientmods.log`
- ยืนยัน `ExampleMod` ผ่าน client จริง: `hello`, `modgive`, `inv`, `player.died` และ storage
  ของ `ExampleGameplayMod` (`events=1, storage=ok`)
- ยืนยันข้ามฝั่งด้วย `control multi-3 tp 40 177` แล้ว client mod อ่านตำแหน่งใหม่ได้
- เพิ่ม best-effort file trace ใน `client/ClientModLoader.cs` เพราะ `Debug.Log` จาก net35
  reference ถูกตัดโดย conditional attribute ใน build นี้

---

## 2026-08-27 — Mod Platform M2 gameplay events ต่อเนื่อง (typed payload + cancellable craft/gather/build)

- เพิ่ม `GameplayEvents.cs` และ `IGameplayEvent`: typed gameplay contract แบบ additive พร้อม `ActionId`, `Values`, cancellation state
- event context ของ PluginManager implement typed contract จริง และรับ payload จาก recipe/generator/blueprint/combat เช่น `recipe_id`, `output_count`, `tool_item_id`, `entity_id`, `damage`, `tile_x/tile_y`
- เพิ่ม `craft.before` cancel หลัง validation ก่อน consume resource พร้อมข้อมูล recipe/output; เพิ่ม payload/dispatch ใน gather/build/combat
- เพิ่ม `farm.planted`, inventory removal และ payload event ในเส้นทางเก็บเกี่ยว/ปลูก/ทุบ/วาง/ตี
- แก้ duplicate `MatchesAny` helper ใน RemoteDrive ที่ทำให้ server build ไม่ผ่าน (ลบเฉพาะ helper ซ้ำ ใช้ของ Crafting เดิม)
- build `mod-sdk`, `DurangoServer`, `ExampleMod`, `test-client` ผ่าน 0 error; `--recipe-check` ผ่านทั้งหมด
- smoke server ผ่าน: ExampleMod v1.1 โหลด, `modgive/count/inv`, death event broadcast และ storage file จริง

**M2 ยังไม่เสร็จทั้งหมด**: before/cancel ของ butchery/farm/combat/build-complete/destroy ยังต้องผูกและทดสอบ, typed context ควรแยกคลาสเฉพาะ event เพิ่ม, ต้องมี ExampleGameplayMod + regression cancel/duplicate packet/soak test

---

## 2026-08-27 — Mod Platform M0/M1 + gameplay event wiring (Phase 2 เริ่มแล้ว)

- เพิ่ม optional event bus contracts: `EventPriority`, `IModEventsApi`, `IModEventContext`, `IModIdentity`, `IModStorage` โดยไม่เปลี่ยน lifecycle interface เดิม
- loader เรียง DLL แบบ deterministic, รองรับ metadata ID/API/dependencies, event handler sort ตาม priority → mod ID → registration sequence และ isolate exception/timing ต่อ mod
- เพิ่ม namespaced atomic storage ใต้ `saves/mods/<mod-id>/`; `ExampleMod` เขียน `install.json` ได้จริง และ flush ตอน Ctrl+C
- `/admin/mods` เพิ่ม ID/API/dependencies/events/event errors/timing และ `has_event_bus`
- เพิ่ม event catalog สำหรับ lifecycle, inventory, craft/gather/farm/build/combat/quest/progression/travel/chat; ผูก post-commit หลายเส้นทาง และ cancelable before hook สำหรับ craft/gather/build placement
- server build + mod SDK + ExampleMod ผ่าน 0 error; console smoke ผ่าน inventory/death event; storage file ตรวจพบจริง
- **ยังไม่ถือว่า M2 เสร็จ**: event context ยังมี payload เป็น generic `Data`, before/cancel ยังไม่ครบทุกกลุ่ม, ต้องเพิ่ม ExampleGameplayMod และ regression tests ทุก action

---

## 2026-08-27 — ModSDK V1.1 ทั้งสองฝั่ง + ExampleClientMod + docs ฝั่ง client (เต็มสูตร)

ต่อยอดระบบ mod จาก 24 ส.ค. — ทำ "ชุดเต็ม" ที่เคยอ้างไว้แล้วว่าหาย/ยังไม่มี: hook ใหม่, ตัวอย่าง
client mod, เอกสาร client modding, admin panel โชว์ hook ใหม่

### mod-sdk (เซิร์ฟ) V1.1 — additive (mod เก่าไม่พัง)

- `IModApi.OnPlayerDied(handler)` — ยิงจาก `ServerPlayer.Die()` ท้ายสุด state set ครบแล้ว
  (Dead=true, เลือด=0, battle จบ) · PluginManager เก็บ `_diedHandlers` + `FirePlayerDied()`
- `IModPlayer` เพิ่ม **มุมมองกระเป๋า**: `CountItem(prototypeId)` / `GetInventorySummary()` /
  `GiveItem(prototypeId, count)` — give ผ่าน `ModGiveItems()` บน ServerPlayer (MakeGatheredItem
  durability/tag/performance ครบเหมือนเก็บเอง + MarkDirty+SendInventory) — เปิดเฉพาะ *อ่าน+ใส่*
  ไม่เปิดลบ/ย้ายของให้ mod
- `/admin/mods` เพิ่ม field `has_died_hook`

### client-mod-sdk V1.1 — additive

- `IClientModApi.OnUpdate(Action<float>)` — ทุกเฟรม dt=Time.deltaTime บน __ClientModDriver
  (DontDestroyOnLoad) — ClientModLoader.PumpFrame dispatch

### ใหม่: `tools/ExampleClientMod/`

mod ตัวอย่าง client ครบทุก hook ในไฟล์เดียว (F9 FPS / F10 ตำแหน่ง / F11-F12 จุดจำ*ไม่ warp*
กัน anti-cheat / OnGameReady popup) — build net35 ผ่าน, DLL ติดตั้ง `game/mods/ExampleClientMod.dll`
(ปิดบอท MemoryBot เดิมไม่แตะ)

### เอกสาร

- **ใหม่** `docs/client/Modding.md` — สถาปัตยกรรม/วิธีเขียน/API/กับดัก client mod (net35,
  ห้าม duplicate DurangoClientModSdk.dll ฯลฯ) — ปิดจุดที่ comment อ้างไว้แล้วไฟล์ไม่มี
- `docs/../server/Modding.md` — ตาราง API เพิ่ม V1.1, §4 log เทสรอบ V1.1, §5 กับดักเพิ่มเคส
  คำสั่งชน built-in (`give` → ใช้ `modgive`), §6 แก้สถานะล้าสมัย "ฝั่ง client ยังไม่ได้ทำ"

### เทสแล้ว (server จริง + console bot)

- build: mod-sdk / ExampleMod / DurangoServer Debug / client (build-client.ps1 = Assembly-CSharp +
  SDK install) / ExampleClientMod — 0 error ทั้งหมด
- bot ยิงคำสั่ง: `cheat hello`, `cheat inv`, `cheat count stone`, `cheat modgive stone 3`,
  `cheat hurt`×4 → ตายจริง ได้ broadcast `[ExampleMod] ☠ ... ล้มลงที่ tile 40,177!` ✅
- 🐛 กับดักที่เจอระหว่างเทส: คำสั่ง mod ชื่อ `give` โดน built-in `give` กลืนเงียบ ๆ — เอกสาร §5
  บันทึกแล้ว; saves บอททดสอบ (modtestbot*) ลบทิ้งหมด
- ✅ เปิดเกมยืนยัน loader/client mod แล้ว; หลักฐานเต็มอยู่ที่ `game/clientmods.log` และ
  `docs/client/Modding.md` §4

---

## 2026-08-26 — DinoWorld Launcher (ตัวเปิดเกมแบบเกมออนไลน์) + endpoint /launcher/*

เจ้าของสั่ง: "ทำ launcher เข้าเกม มีไว้โหลดแพทช์และส่งคำสั่งเพื่อเชื่อมเซิฟโดยตรง มีประกาศเหมือนพวกเกม ragnarok"
(ผ่าน mockup `docs/design/mockups/launcher_mockup.html` ธีมดำเข้ม+ชมพูเข้ม โลโก้ผีเสื้อ 8-bit ก่อน 1 รอบ)

### ใหม่: `tools/Launcher/` → DinoWorldLauncher.exe

- 📰 ประกาศ/อีเวนต์ (แท็บกรอง) · 🟢 สถานะเซิร์ฟ (ชื่อ/ผู้เล่น/เวอร์ชัน refresh 5 วิ) · ▶ **เข้าเกมต่อเซิฟตรง**
- "เข้าเกม" = เปิด `DurangoV2.exe` + env `DURANGO_AUTOCONNECT=<ip[:port]>` — client รองรับอยู่แล้ว
  (`client/Durango.Offline/Server.cs` AutoConnectTarget) ⇒ **ไม่แก้ client เลย**
- อัปเดต/ซ่อมไฟล์ = logic เดียวกับ tools/Updater (manifest→zip→SHA256→temp→robocopy /MIR)
  fail-safe เหมือนเดิม: พังอะไรก็ตามยังกดเข้าเกมเวอร์ชันเดิมได้เสมอ
- ตั้งค่า ip:port + auto-patch เซฟที่ `launcher_settings.json` ข้างเกม
- build: `dotnet publish tools/Launcher/DinoWorld.Launcher.csproj -c Release -r win-x64 -p:SelfContained=true`
  (exe เดียว ~120MB, วางข้าง DurangoV2.exe) — รายละเอียด `docs/../server/Launcher.md`

### ใหม่: server route `/launcher/*` (`../server/ServerCore/Gateway.Launcher.cs`)

โซนผู้เล่น ต่างจาก `/admin/*`: **อ่านอย่างเดียว ไม่มี action ไม่ต้อง token**

| Route | คืน |
|---|---|
| `GET /launcher/news` | ประกาศจาก `data/launcher_news.json` (cache 5 วิ — แก้ไฟล์ไม่ต้อง restart) |
| `GET /launcher/status` | `{name, players, max_players, tps, latest_version}` subset ปลอดภัยของ admin/status |
| `GET /launcher/version` | `{version, zip_url?, sha256?}` จาก `data/launcher_patch.json` (รูปแบบ manifest เดียวกับ Updater) |

data ใหม่: `data/launcher_news.json` (ประกาศ), `data/launcher_patch.json` (manifest แพตช์ — zip_url ว่าง = ยังไม่แจก)

**เทสแล้ว**: server build 0 error · รันเซิร์ฟจริง (พอร์ตทดสอบ 8399) curl ทั้ง 3 route ได้ข้อมูลถูกต้อง ·
Launcher publish ผ่าน (ยังไม่ได้เปิด GUI จริง — ต้องเอาไปวางข้าง DurangoV2.exe)
...
`docs/operations/BETA-1.0-PLAYERS.html` — หน้าเดียวจบว่าเข้าไปแล้วจะเจออะไร ระบบไหนเปิด/ปิด/ยังไม่มี ·
...
- ทำไมถึงสำคัญกับความสนุก: ผู้เล่นจำได้ว่า "ทางนั้นมีแร็ปเตอร์ ยังไปไม่ได้" — ความกลัวมีทิศทาง
  และวันที่กล้าเดินเข้าไปคือวันที่รู้สึกว่าตัวเองเก่งขึ้นจริง (ดู `docs/project/GOAL.md`)
...
- `docs/project/GOAL.md` — ทำเสร็จแล้วเกมมีระบบอะไร · เล่นไปทางไหน · สนุกตรงไหน 5 ข้อ · และ**จะไม่เป็นอะไร**
- `docs/project/ROADMAP.md` — beta 4 รอบ (B1 เกาะเดียว → B2 เพื่อน 5-10 → B3 หลายเกาะ → B4 ซ้อมเปิด) → เปิดจริง
