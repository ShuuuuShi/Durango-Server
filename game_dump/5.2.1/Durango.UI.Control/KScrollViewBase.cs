...
public abstract class KScrollViewBase : MonoBehaviour
{
	...
	public float ViewBreadth => UIUtility.GetSize(vc: new Vector2(Vector.y, Vector.x), size: ViewSize);
	...
	private void MoveToImpl(float offset, bool instant, bool restrictWithinPanel, Action onFinish)
	{
		...
		{
			SpringPanel.Begin(panel.gameObject, vector2, 8f).onFinished = OnFinishMoveTo;
		}
	}
	...
	{
		Vector4 baseClipRegion = Panel.baseClipRegion;
		return new Vector3(baseClipRegion.x, baseClipRegion.y) - Vector * (ViewLength * 0.5f - (float)_padding);
	}
