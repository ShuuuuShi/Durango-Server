...
public class LearnSkillToDo : ToDoBase
{
	...
	private void SkillLevelChanged(Durango.Logic.Skill.Skill skill)
	{
		if (skill.Level != 0)
		{
			bool num = string.Compare(_id, skill.Id, StringComparison.OrdinalIgnoreCase) == 0;
			bool flag = string.Compare(_sub, skill.SubId, StringComparison.OrdinalIgnoreCase) == 0;
			bool flag2 = _lv <= 0 || _lv == skill.Level;
			if (num && flag && flag2)
			{
				CallComplete();
			}
