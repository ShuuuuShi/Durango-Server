...
public class ShopVouchersPopup : TooltipBase
{
	...
	protected override void FillData()
	{
		...
		IEnumerable<string> enumerable = ((_targetVouchers == null) ? wallet.Vouchers.Select((VoucherInfo x) => x.VoucherId) : _targetVouchers);
		foreach (string item in enumerable)
		{
			if (SingletonDict<string, Voucher>.TryGetValue(item, out var value) && value.Visible)
			{
				string hexColor = value.GetHexColor();
				Color iconColor = NGUIText.ParseColor24(hexColor);
				...
				VoucherWidget component = _voucherList.Nodes.GetNext().GetComponent<VoucherWidget>();
				component.Set(value.Icon, iconColor, title, value.Description, expiry, count, clicked);
			}
		}
