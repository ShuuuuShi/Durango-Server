...
public class WarpRushGroup : UIBase, IUIInitializable, INotificationable
{
	...
	void IUIInitializable.Init()
	{
		...
		GameSystem<WarpRushSystem>.Instance().RewardedRankingUpdated += delegate
		{
			...
			{
				string value = SingletonDict<Category, Ranking>.Instance.Get(ranking)?.GetCurrentAndPrevRevisionId(utcNow).Value;
				if (!string.IsNullOrEmpty(value) && GameSystem<WarpRushSystem>.Instance().AnyRewardLeft(ranking))
				{
