...
public class ChattingGroup : ChattingGroupBase
{
	...
	private void OnPushToggle()
	{
		...
		ChattingGroupBase.ShowToggleButtonTooltip((!flag) ? T._("채널에 새로운 대화가 있어도 알림을 받지 않습니다.") : T._("채널에 새로운 대화가 있으면 알림을 받습니다."), _pushStateSprite, Vector3.zero);
		RefreshChattingTab();
	}

	private void OnChatHideToggle()
	{
		bool num = ((base.CurrentConv == null) ? GameSystem<SocialSystem>.Instance().ChannelInfo.ToggleHide(base.FilterType) : GameSystem<SocialSystem>.Instance().ChannelInfo.ToggleHide(base.CurrentConv));
		((!num) ? _chatUnhideSprite : _chatHideSprite).Set(_hideStateSprite);
		ChattingGroupBase.ShowToggleButtonTooltip((!num) ? T._("채널의 대화 내용을 한줄 채팅에 표시합니다.") : T._("채널의 대화 내용을 한줄 채팅에 표시하지 않습니다."), _hideStateSprite, Vector2.up * 8f);
		RefreshChattingTab();
	}
