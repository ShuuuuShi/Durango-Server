...
public class UIPanelClone : MonoBehaviour
{
	...
	private DrawcallClone GetClone(int index)
	{
		if (_list.Count == index)
		{
			...
			drawcallClone.Copy = gameObject.transform;
			drawcallClone.MeshFilter = gameObject.AddComponent<MeshFilter>();
			drawcallClone.MeshRender = gameObject.AddComponent<MeshRenderer>();
		...
		while (uIPanel != null)
		{
			if (uIPanel.hasClipping)
			{
				...
				if (uIPanel != panel)
				{
					...
					Vector3 eulerAngles2 = uIPanel.cachedTransform.rotation.eulerAngles;
