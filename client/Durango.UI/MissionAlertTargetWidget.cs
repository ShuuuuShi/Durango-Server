...
public class MissionAlertTargetWidget : MonoBehaviour
{
	...
	public void UpdateIndicator()
	{
		...
		for (int i = 0; i < TerrainMeta.Indicators.Count; i++)
		{
			...
			if (blueprint == null || !blueprint.HasComponent(Component))
			{
				continue;
			}
			...
			if (indicator2 != null)
			{
				...
				MapIndicator indicator3 = Singleton<MapIndicators>.Instance().GetIndicator(point.ToString(), IndicatorType.Artifact);
				if (indicator3 != null)
				{
					_indicator = indicator3;
					break;
				}
			}
		}
