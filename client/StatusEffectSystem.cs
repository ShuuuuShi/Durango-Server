...
public class StatusEffectSystem : GameSystem<StatusEffectSystem>
{
	...
	{
		Durango.Logic.StatusEffects statusEffects = MakeOrGet(msg.EntityId);
		statusEffects.SetStatusEffects(msg);
	}
	...
	public Durango.Logic.StatusEffect GetStatusEffectFromCommodity(string commodityId)
	{
		...
		return (KUtility.GetSize(commodity.Contents.StatusEffects) <= 0) ? null : GetStatusEffect(commodity.Contents.StatusEffects[0].status_effects_id);
	}
}
