...
public class PlayGuideHelperGroupBase : UIBase
{
	...
	private static IEnumerator DisplayGuidePopup(string title, string comment, SpotlightTarget spotlight, Transform target)
	{
		...
		tooltip.Set(title, comment);
		tooltip.CommentWidth = spotlight.comment_width;
		tooltip.Direction = spotlight.direction;
		Vector2 offset = new Vector2(spotlight.x_offset, spotlight.y_offset);
		tooltip.DragLock = true;
		tooltip.Show(target.gameObject, offset, 3600f);
		tooltip.ModifyDrawPanel(target);
		tooltip.LockSkip(GuideTooltip.SpotlightGuideUnskippableTime);
		SoundManager.PlayEvent("ui_guide_pop_up");
		tooltip.AddOnFinished(GameSystem<PlayGuideSystem>.Instance().OnGuideMsgFinished);
	}
	...
	private Vector3 CalcArrowHelperTarget(GuideEvent guideEvent, HelperTarget helper)
	{
		...
		case "immovable":
		{
			...
			return (!(nearestImmovable != null)) ? Vector3.zero : nearestImmovable.InteractionPosition;
		}
		case "nearest_poi":
		{
	...
	{
		return (!helperTile.HasValue) ? Vector3.zero : Durango.Terrain.Util.TilePositionToClientPosition(helperTile.Value, tileCenter: true);
	}
	...
	private static Vector2 StringToTile(string tile)
	{
		...
		return (array.Length == 2) ? new Vector2(array[0].ToInt(), array[1].ToInt()) : Vector2.zero;
	}

	private static Vector3 TileToClientPosition(string tile)
	{
		Vector2 vector = StringToTile(tile);
		return (!(vector != Vector2.zero)) ? Vector3.zero : Durango.Terrain.Util.TilePositionToClientPosition(vector, tileCenter: true);
	}
}
