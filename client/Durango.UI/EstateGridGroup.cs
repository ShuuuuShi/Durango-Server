...
public class EstateGridGroup : UIBase
{
	...
	private void RefreshDeclareGrid()
	{
		...
		for (int i = 0; i < 12; i++)
		{
			for (int j = 0; j < 12; j++)
			{
				...
				rectGridArea.Tile = point * 4;
				rectGridArea.Size = Point2.one * 4;
				RectGridArea rectGridArea2 = rectGridArea;
				rectGridArea2.BgColor = ((!flag) ? PresetColor.UILightRed : PresetColor.UILightGreen);
				if (point == _selectedUnit)
				{
					rectGridArea2.BgColor = PresetColor.UIYellow;
					rectGridArea2.ButtonStyle = PresetButton.Style.Solid;
					rectGridArea2.ButtonColor = Color.white;
					...
					rectGridArea2.ButtonText = ((num != 0) ? T._("선언 {0}", Durango.Logic.Item.Inventory.CurrencyFormat(num, icon)) : T._("선언"));
					rectGridArea2.OnSelect = OnDeclareEstateClick;
				}
				_areaList.Add(rectGridArea2);
			}
		}
		Durango.Utils.Singleton<GridAreaViewer>.Instance().Show(_areaList);
	}
	...
	private void OnSelectGrid(GameObject obj)
	{
		...
		Vector3 position = MainCamera.ScreenPosToWorldPos(currentTouch.pos);
	...
	private void OnSuccess(EstateLicense license)
	{
		...
		EstateGroup estateGroup = UIManager.FindScript<EstateGroup>();
		estateGroup.NotifyEstateLicenseChanged(owner, license);
	}
	...
	private void RefreshResetWidget()
	{
		...
		{
			double num = ((!_expandEstate.License.RemovableAt.HasValue) ? 0.0 : _expandEstate.License.RemovableAt.Value);
			double num2 = num - Connections.Frontend.GetPredictedServerTime();
			if (num2 < 0.0)
			{
				text = null;
				period = 0f;
			...
			else
			{
				...
				text = T._("<em>{0}</em> 후 초기화 가능", TimedeltaFormatter.ColonFormat(num2));
				period = 0.5f;
				_resetLabel.text = text;
				int num3 = _resetLabel.width + 20;
				if (num3 > _resetWidget.width)
				{
					_resetWidget.width = num3;
					UIUtility.UpdateAnchors(base.transform);
				}
			}
		}));
