...
public class WorldRoutesViewerShadow : CustomFillSprite
{
	...
	private void ProcessClouds()
	{
		...
		if (_isInitClouds)
		{
			...
			{
				float num3 = _clouds[num2].CreatedAt + _clouds[num2].Duration;
				if (num3 < time)
				{
			...
			if (count < num && time - _lastCloudCreatedAt > _cloudMakeDelay)
			{
				MakeCloud();
			}
		}
		else
		{
			...
			{
				float num4 = UnityEngine.Random.value * 0.9f;
				CloudStruct value = _clouds[j];
				value.CreatedAt -= value.Duration * num4;
				_clouds[j] = value;
			}
		}
		mChanged = true;
