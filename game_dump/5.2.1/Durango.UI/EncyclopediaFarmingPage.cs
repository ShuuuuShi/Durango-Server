...
public class EncyclopediaFarmingPage : MonoBehaviour, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		{
			obj.GetComponent<EncyclopediaCropWidget>().Clicked += OnClickCropItem;
		});
	}
	...
	public void Show()
	{
		...
		if (farmingEncyclopediaDataList != null)
		{
			...
			Array.Sort(array, delegate(KeyValuePair<string, FarmingEncyclopediaData> d1, KeyValuePair<string, FarmingEncyclopediaData> d2)
			{
				...
				return (num3 == 0) ? string.CompareOrdinal(d1.Key, d2.Key) : num3;
			});
			...
			for (int i = 0; i < array2.Length; i++)
			{
				...
				_scrollView.Nodes.GetNext().GetComponent<EncyclopediaCropWidget>().Set(keyValuePair.Key, keyValuePair.Value);
