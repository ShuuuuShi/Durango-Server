...
public class BalloonContainer : MonoBehaviour
{
	...
	{
		AddAnnounceBalloon(type, tilePos, info.EntityId, info.Name).SetPortrait(info);
	}
	...
	{
		AddAnnounceBalloon(type, tilePos, entityId, titleName).SetSprite(entityId, spriteName, spriteSize);
	}
	...
	public void SetWorldmapMode(bool isWorldmmap)
	{
		...
		{
			_announceBalloons[i].SetTitleVisible(isWorldmmap);
		}
	}
