		...
		for (int k = 0; k < 16; k++)
		{
			...
			for (int l = 0; l < 3; l++)
			{
				for (int m = 0; m < 3; m++)
				{
					...
					if (isOcean)
					{
						Vector2 vector = Util.ClientPositionToTilePosition(chunkPos) + new Vector2(m, l);
						Vector2 vector2 = vector * 255f / tileCount;
						ref Color32 reference = ref array[k][num2];
						reference = new Color32(b, (byte)vector2.x, (byte)vector2.y, a);
					}
					else
					{
