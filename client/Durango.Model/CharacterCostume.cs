...
public class CharacterCostume
{
	...
	{
		return !_isMale && type == CostumeType.Beard;
	}
					...
					if (!(componentInChildren == null))
					{
						...
						if (!string.IsNullOrEmpty(partName))
						{
							...
							if (!(transform == null))
							{
								...
								Transform[] componentsInChildren = _rootBone.GetComponentsInChildren<Transform>();
								Transform[] array = Prefabs.MappingBones(componentsInChildren, componentInChildren.bones);
								if (array != null)
								{
	...
	private Material GetBodyMaterial(bool isNude)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				bool flag = material.mainTexture.name.Contains("body_nude");
				if (flag == isNude)
				{
					return material;
				}
	...
	private List<GameObject> GetParticles(CostumeType type, bool create = false)
	{
		...
		return (_particles == null) ? null : _particles[(int)type];
	}
