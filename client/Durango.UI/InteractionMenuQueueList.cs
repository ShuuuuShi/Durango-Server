...
public class InteractionMenuQueueList : MonoBehaviour
{
	...
	private InteractionQueueIconWidget PopNext()
	{
		...
		{
			GameObject gameObject = base.gameObject.AddChild(_baseIcon);
			interactionQueueIconWidget = gameObject.GetComponent<InteractionQueueIconWidget>();
			UIEventListener.Get(gameObject).onClick = OnIconClicked;
		}
