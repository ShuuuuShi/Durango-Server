...
public class WarpAcceleratorInfoWidget : MonoBehaviour
{
	...
	private void InitializeStateSprite()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			float fillAmount = (component.fillAmount = (num2 - num) * 288f / 360f);
			next.fillAmount = fillAmount;
			next.depth = component.depth + 1;
			if (i > 0)
			{
				GameObject next2 = _phaseStateSeparators.GetNext();
				next2.transform.localEulerAngles = new Vector3(0f, 0f, 144f - 288f * num);
			}
			num = num2;
		}
	...
	private void RefreshOverInfoWidgets()
	{
		...
		{
			bool flag = false;
			AcceleratorStatus status = item.Warpaccelerator.Status;
			...
			if (flag)
			{
				Artifact artifact = Durango.Utils.Singleton<ArtifactManager>.Instance().Find(item.EntityId);
				if (!(artifact == null))
				{
					WarpAcceleratorOverWidget next = _overInfoWidgets.GetNext();
					next.Set(artifact, item.Warpaccelerator);
				}
			}
		}
	...
	private void Refresh()
	{
		...
		case AcceleratorStatus.Processing:
		{
			...
			Yaml.WarpAccelerator warpAccelerator = Yaml.Util.Singleton<Constants>.Instance.WarpAccelerator;
			float phaseTime = warpAccelerator.PhaseTime;
