...
public abstract class TerrainBase : Singleton<TerrainBase>
{
	...
	public static bool IsPlayerInitialized
	{
		get
		{
			...
			{
				return Singleton<TerrainBase>.Instance().Initialized && Singleton<TerrainBase>.Instance().PlayerPositionIntiailized;
			}
			return false;
		}
	}
	...
	private void CreateChunkPrefabs()
	{
		...
		if (gameObject != null)
		{
			...
			gameObject2.name = "Ocean";
		}
		GameObject gameObject3 = LoadChunk(_riverChunk);
		if (gameObject3 != null)
		{
			GameObject gameObject4 = UnityEngine.Object.Instantiate(gameObject3);
			gameObject4.name = "River";
		}
		GameObject gameObject5 = LoadChunk(_lakeChunk);
		if (gameObject5 != null)
		{
			GameObject gameObject6 = UnityEngine.Object.Instantiate(gameObject5);
			gameObject6.name = "Lake";
		}
	}
	...
	private void InitChunkPool()
	{
		...
		// [แก้เอง] โหลดแมพล่วงหน้า: เดิม range 1 = เห็นแค่ 3×3 chunk (48×48 tile)
		// พอเดินข้ามขอบ chunk ระบบจะทิ้ง 3 chunk แล้วสร้างใหม่ 3 chunk *ในระยะที่มองเห็น*
		// ⇒ นี่คืออาการ "วิ่งไปแล้วแมพรีเฟรชเป็นระยะ" ที่เห็นในเกม
		//
		// range 2 = 5×5 chunk (80×80 tile) การโหลดจึงเกิดห่างออกไปอีก 1 chunk (16 tile)
		...
		ChunkPool = new ChunkPool(num, range, TerrainChunk, base.transform);
		if ((bool)ocean)
		{
			ocean.Init(num);
		}
		...
		for (int i = 0; i < num; i++)
		{
			...
			chunk.Init((!ocean) ? null : ocean.GetWaterChunk(i), (!lake) ? null : lake.GetWaterChunk(i), (!river) ? null : river.GetWaterChunk(i));
		}
		Initialized = true;
	}
	...
	{
		string url = $"{GameManager.GatewayUrl}/terrains/{GameManager.Region.TerrainId}/{coord.x},{coord.y}";
	...
	private void OnDisappearEntityOnTile(DisappearEntityOnTile msg, PacketHeader header)
	{
		...
		if (tileObject != null)
		{
			...
			else if (tileObject.Artifact != null && !string.IsNullOrEmpty(msg.EntityId) && !Singleton<ArtifactManager>.Instance().RemoveArtifact(msg.EntityId))
			{
			}
		}
	}
	...
	public Biome GetTileBiome(Vector3 worldPos)
	{
		...
		return (!(chunkFromWorldPosition == null)) ? chunkFromWorldPosition.GetTileBiome(worldPos) : Biome.Invalid;
	}

	public byte GetRawTileBiome(Vector3 worldPos)
	{
		TerrainChunkBase chunkFromWorldPosition = GetChunkFromWorldPosition(worldPos);
		return (!(chunkFromWorldPosition == null)) ? chunkFromWorldPosition.GetRawTileBiome(worldPos) : byte.MaxValue;
	}

	public bool IsCollidableMasked(Vector3 worldPos)
	{
		byte rawTileBiome = GetRawTileBiome(worldPos);
		return Util.IsCollidableMasked(rawTileBiome);
	}
	...
	public Vector3 GetNearestBiome([NotNull] Biome[] biomes, Vector3 clientPosition)
	{
		...
		return (!flag) ? Vector3.zero : Util.WorldPositionToClientPosition(worldPosition);
	}

	public bool HasNaturalObjectInSquareRange(int[] entityTypes, Vector3 clientPosition, int tileLength)
	{
		Vector3 position = Util.ClientPositionToWorldPosition(clientPosition);
		Vector2 vector = Util.WorldPositionToTilePosition(position);
	...
	public float GetWaterDepth(Vector3 worldPosition)
	{
		...
		return (!(chunkFromWorldPosition == null)) ? chunkFromWorldPosition.GetTileWaterDepth(worldPosition) : 0f;
	}
	...
	{
		TerrainChunkBase chunkFromWorldPosition = GetChunkFromWorldPosition(worldPosition);
		return (!(chunkFromWorldPosition == null)) ? chunkFromWorldPosition.GetTileWaterFlow(worldPosition) : Vector2.zero;
	}

	public ImmovableBase GetMoveAffectingObject(Vector3 worldPosition)
	{
		TerrainChunkBase chunkFromWorldPosition = GetChunkFromWorldPosition(worldPosition);
		return (!(chunkFromWorldPosition == null)) ? chunkFromWorldPosition.GetMoveAffectingObject(worldPosition) : null;
	}
	...
	public void RestoreWorldSpritePoolVisibility()
	{
		...
		{
			TerrainChunkBase chunk = ChunkPool.GetChunk(i);
			chunk.RestoreWorldSpritePoolVisibility();
		}
	}
}
