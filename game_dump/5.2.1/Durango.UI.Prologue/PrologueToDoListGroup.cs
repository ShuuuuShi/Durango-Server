...
public class PrologueToDoListGroup : UIBase
{
	...
	private void ToDoSystem_ListUpdated(List<ToDoBase> sources, bool added)
	{
		...
		{
			Object.Destroy(_itemList[k].Control.gameObject);
		}
	...
	{
		GameObject obj = _itemParent.gameObject.AddChild(_checkBoxPrefab);
		obj.transform.localPosition = _checkBoxPrefab.transform.localPosition;
		obj.SetActive(value: true);
		Item item = new Item
		{
			Key = todo.Key
		};
		PrologueToDoCheckBoxControl component = obj.GetComponent<PrologueToDoCheckBoxControl>();
