...
public class ContextActionGroupBase : UIBase
{
	...
	private void OnPetActiveSkillUsed(PetActiveSkillUsed msg)
	{
		...
		PetSkillStates.State state = petManager.PlayerPetSkillStates.GetState(msg.SkillId);
		...
		if (petActiveSkill != null)
		{
			...
			{
				ActionProgressGauge actionProgressGauge = Durango.Logic.Timer.Timer.Play<ActionProgressGauge>(new Durango.Logic.Timer.Timer(playerPetId, null, petActiveSkill.Duration));
