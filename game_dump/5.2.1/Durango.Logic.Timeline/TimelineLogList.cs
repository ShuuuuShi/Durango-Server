...
public class TimelineLogList
{
	...
	{
		Http.RequestYml($"{GameManager.GatewayUrl}/entities/{id}/timeline?page={page}&page_size={pageSize}", onResult);
	}

	private static void RequestClanLogs(string id, int page, int pageSize, [NotNull] Action<TimelineLogSet> onResult)
	{
		Http.RequestYml($"{GameManager.GatewayUrl}/clans/{id}/timeline?page={page}&page_size={pageSize}", onResult);
	}

	private static void RequestEstateLogs(string id, int page, int pageSize, [NotNull] Action<TimelineLogSet> onResult)
	{
		Http.RequestYml($"{GameManager.GatewayUrl}/entities/{id}/estate_timeline?page={page}&page_size={pageSize}", onResult);
	}

	private static void RequestClanEstateLogs(string id, int page, int pageSize, [NotNull] Action<TimelineLogSet> onResult)
	{
		Http.RequestYml($"{GameManager.GatewayUrl}/clans/{id}/territory_timeline?page={page}&page_size={pageSize}", onResult);
	}
