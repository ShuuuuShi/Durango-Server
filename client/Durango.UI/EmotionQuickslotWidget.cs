...
public class EmotionQuickslotWidget : MonoBehaviour
{
	...
	{
		List<Emoticon> emoticons = GameSystem<SocialSystem>.Instance().Emotional.Emoticons;
		List<List<Emoticon>> list = emoticons.Where((Emoticon elem) => elem.IsSubscribe() && elem.Available).ToList().Split(12);
	...
	private void UpdateMotions()
	{
		...
		List<List<Durango.Logic.Social.Motion>> list = motions.Where((Durango.Logic.Social.Motion elem) => elem.IsSubscribe() && elem.Available).ToList().Split(12);
	...
	public EmoticonWidget FindEmoticonWidget(string key)
	{
		...
		{
			GameObject gameObject = _emoticonGridList.Nodes[i];
			EmoticonWidget[] componentsInChildren = gameObject.GetComponentsInChildren<EmoticonWidget>();
