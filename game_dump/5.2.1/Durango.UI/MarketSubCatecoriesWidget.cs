...
public class MarketSubCatecoriesWidget : MonoBehaviour
{
	...
	private void SetCategory([CanBeNull] Category.Main main, bool isSelected, [CanBeNull] Category.Sub current)
	{
		...
		for (int size = KUtility.GetSize(category.Subs); i < size; i++)
		{
			...
			targetNode.transform.Find("Text").GetComponent<UILabel>().text = targetData.Name;
			SelectableWidget component2 = targetNode.GetComponent<SelectableWidget>();
			component2.Selected = isSelected && current != null && current.Id == targetData.Id;
			component2.Clicked = delegate
			{
