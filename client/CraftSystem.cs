...
public class CraftSystem : GameSystem<CraftSystem>
{
	...
	public void DoNextCraftQueue()
	{
		if (_craftQueue.Count != 0)
		{
			...
			craft.RecipeId = craftQueueItem2.Recipe.Id;
			craft.Materials = craftQueueItem2.MakeMaterialIds();
			craft.ToolItemId = ((craftQueueItem2.Tool != null) ? craftQueueItem2.Tool.Id : string.Empty);
			craft.Workbench = ((!(craftQueueItem2.Workbench == null)) ? new PropKey?(craftQueueItem2.Workbench.GetPropKey()) : null);
			Craft msg = craft;
			RegisterPostCraftEvents(Connections.Frontend.Send(msg), craftQueueItem2.Recipe.Id);
		}
	}
