...
public class InventoryContainerBase : MonoBehaviour, IUIInitializable
{
	...
	private bool FilterItem([CanBeNull] ItemData item)
	{
		...
		if (CheckBeingInCategory(item))
		{
			return CheckContainingTag(item);
		}
		return false;
	}
	...
	protected virtual void UpdateTabList()
	{
		if (_other != null || _pinnedTabList)
		{
			...
			nodes.BeginLoad();
			nodes.GetNext().GetComponent<KeyValueLabel>().Set(T._("가방"), null);
			if (_other != null)
			{
				if (_other.Type == Durango.Logic.Item.Inventory.InventoryType.Warehouse)
				{
					...
					{
						nodes.GetNext().GetComponent<KeyValueLabel>().SetKey(_other.Categories[i].Key);
					}
				...
				{
					nodes.GetNext().GetComponent<KeyValueLabel>().SetKey(TargetInventoryName(_other, T._("알수없음")));
					_warehouseTabConfigBtn.gameObject.SetActive(value: false);
					_warehouseTabAddBtn.gameObject.SetActive(value: false);
				}
			}
			...
			for (int j = 0; j < nodes.Count; j++)
			{
				...
				if (inventory != null && inventory.State == Durango.Logic.Item.Inventory.InventoryState.Loaded)
				{
					...
					{
						component.SetValue($"{inventory.Categories[j - 1].Value:0} / {Yaml.Util.Singleton<Constants>.Instance.Warehouse.SectionSize:0}");
					}
					else if (inventory.Capacity > 0)
					{
						component.SetValue($"{inventory.CurrentSize():0} / {inventory.Capacity:0}");
					}
				}
				else
				{
				...
				nodes[j].GetComponent<Selectable>().Selected = j == _selectedTab;
			}
			...
			if (_warehouseTabAddBtn.gameObject.activeSelf)
			{
				...
				_warehouseTabAddBtn.GetComponent<UIWidget>().SetPosition(node.GetPosition(1f, 0f) + _tabList.Margin * Vector3.right, 0f, 0f);
			}
		}
		else
		{
	...
	private void UseItem(IList<ItemData> items)
	{
		...
		if (firstItem.PrototypeId == "skill_reset_ticket")
		{
			...
			messageBox.AddKeyValueInfo(value: string.Format("[icon=icon_sp] <em>{0}</em> / {1}    [preset=animation_arrow]    [icon=icon_sp] <em>{1}</em> / {1}", GameSystem<SkillSystem>.Instance().RemainSkillPoint, GameSystem<SkillSystem>.Instance().SkillPoint), key: T._("스킬 포인트"));
	...
	private static void ChangeDisplay(IList<ItemData> items)
	{
		...
		UIManager.FindScript<EditPlayerDisplayGroup>().OpenEditPlayerCostume(ticket);
	}
	...
	private void AskDropSelectItem()
	{
		...
		if (list.Count == 0)
		{
			UIManager.SystemMsg((num2 == selectedList.Count) ? T._("<em>착용</em> 중인 아이템은 버릴 수 없습니다.") : ((num == selectedList.Count) ? T._("<em>잠긴</em> 아이템은 버릴 수 없습니다.") : ((num3 != selectedList.Count) ? T._("<em>착용</em> 중이거나 <em>잠긴</em> 물품은 버릴 수 없습니다.") : T._("버릴 수 없는 아이템입니다."))));
			return;
		}
	...
	private static void DropToSpecificTile(DumpItems dumpItems, string warningMessage)
	{
		...
		UIManager.FindScript<BuildGridGroupBase>().Open(new BuildGridGroupBase.Arguments
		{
			...
			Confirmed = delegate(BuildSystem.GridResult result)
			{
				...
				if (!DropToEstate(EstateSystem.GetEstateInfo(result.Tile), warningMessage, action))
				{
					action();
				}
			}
	...
	private static bool DropToEstate(EstateInfo estate, string warningMessage, Action drop)
	{
		...
		case OwnerType.PersonalPlayer:
			...
			{
				UIManager.MessageBox.Show(T._("<em>{0}</em> 님의 사유지에 아이템을 버리시겠습니까?", info.GetNameFreq(21, string.Empty)), warningMessage, delegate(bool ok)
				{
					if (ok)
					{
						drop();
		...
		case OwnerType.ClanWarphole:
			...
			{
				UIManager.MessageBox.Show(T._("<em>{0}</em> 부족의 영토에 아이템을 버리시겠습니까?", (clan != null) ? clan.Name : string.Empty), warningMessage, delegate(bool ok)
				{
					if (ok)
					{
						drop();
