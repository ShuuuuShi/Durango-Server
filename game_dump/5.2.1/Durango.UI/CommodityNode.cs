...
public class CommodityNode : SelectableWidget, IScreenResizeReceiver
{
	...
	public void Set(Commodity commodity, ProductType productType)
	{
		...
		if (_priceLabel != null)
		{
			...
			{
				string text = Durango.Logic.Item.Inventory.CurrencyFormat(commodity.Price, commodity.CurrencyType);
				...
				_priceLabel.text = text + " <help>" + salesFeeTooltip + "</help>";
			}
			else
			{
		...
		if (_durationLabel != null)
		{
			...
			case ProductType.Sold:
				...
				{
					double num3 = predictedServerTime - commodity.PurchasedAt.Value;
					text2 = ((num3 < 60.0) ? T._("방금") : ((!(num3 < 3600.0)) ? T._("{0} 전", TimedeltaFormatter.Format(num3, 2, "hour")) : T._("{0} 전", TimedeltaFormatter.Format(num3, 2, "min"))));
					break;
				}
			...
			{
				double num2 = commodity.DeletesAt - predictedServerTime;
				text2 = ((!(num2 < 0.0)) ? TimedeltaFormatter.Format(num2, 2, "hour") : T._("만료"));
				break;
			}
			default:
			{
				double num = commodity.ExpireAt - predictedServerTime;
				text2 = ((!(num < 0.0)) ? TimedeltaFormatter.Format(num, 2, "hour") : T._("만료"));
				break;
			}
			}
			_durationLabel.text = text2;
		}
