...
public class PlayerPreviewWidget : MonoBehaviour
{
	...
	private void Update()
	{
		if (!(_uiModelRender == null) && !(_previewModel == null))
		{
			...
			float x = _uiModelRender.ModelTransform.localPosition.x;
			float x2 = Maths.CalculateSpring(x, target, ref _velocity, 1f, 6.5f, Time.deltaTime);
			_uiModelRender.ModelTransform.localPosition = _uiModelRender.ModelTransform.localPosition.WithX(x2);
		}
	}
