...
public static class Times
{
	...
	public static DateTimeOffset UnixTimeToServerTime(double unixTime)
	{
		...
		double offset = _serverTimeProvider.GetOffset();
		TimeSpan offset2 = TimeSpan.FromHours(offset);
		return dateTimeOffset.ToOffset(offset2);
	}
	...
	{
		double val = until - Connections.Frontend.GetPredictedServerTime();
		val = Math.Max(val, 0.0);
		return TimedeltaFormatter.Format(val, scope, granularity);
	}

	public static string Timeago(double time)
	{
		double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
		int num = (int)(predictedServerTime - time);
		if (num < 60)
		{
			return T._("방금");
		}
