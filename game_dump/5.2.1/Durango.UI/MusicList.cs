...
public class MusicList : MonoBehaviour
{
	...
	{
		return KUtility.GetSize(_musics) < Yaml.Util.Singleton<Constants>.Instance.Musician.SlotCount;
	}
	...
	private void OnImportMusic()
	{
		...
		UIManager.MessageBox.Show(T._("미디 파일 불러오기"), "<alert_icon/> " + MusicManager.GetCopyrightWarningText(), delegate(int index)
		{
			switch (index)
			{
			case 0:
	...
	private void ImportFromFile()
	{
		...
		genericSelector.SetSelected(delegate(int index)
		{
			KUtility.DelayedCall(this, delegate
			{
				...
				if (index < size)
				{
					using (FileStream stream = new FileStream(files[index], FileMode.Open))
					{
						ImportMidi(stream);
						return;
					}
				}
				using MemoryStream stream2 = new MemoryStream(imported[index - size].bytes);
				ImportMidi(stream2);
			}, 0.1f);
		});
	...
	private void ImportFromLink()
	{
		...
		UIManager.Popup.Tooltip<TextInputPopup>().Show(ImportFromLink, T._("불러올 악보 링크를 적어주세요."), null, isMultiline: true, null, 0);
	}
	...
	public void Set(List<KeyValuePair<MusicId, Messages.Music>> musics)
	{
		...
		if (_musics != null)
		{
			...
			{
				_scrollList.Nodes.GetNext().GetComponent<MusicNodeWidget>().Set(music.Key, music.Value);
			}
		}
	...
	private void OnMusicCreate([NotNull] Durango.Logic.Music.Music music)
	{
		...
		for (int i = 0; i < music.Notes.Count; i++)
		{
			...
			if (music.TickToTimer(item.Tick) >= num)
			{
				flag2 = true;
				break;
			}
