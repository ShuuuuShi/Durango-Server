...
public class PetMilestoneHelpItemWidget : MonoBehaviour
{
	...
	public void SetMiletone(string tagId, float origin, float weight)
	{
		...
		if (tag != null && tag.PetFoodReference != null)
		{
			...
			for (int i = 0; i < petFoodReference.Length; i++)
			{
				Prototype itemPrototype = PrototypeYaml.GetItemPrototype(petFoodReference[i]);
	...
	private void OnClickItemWidget(GameObject obj)
	{
		...
		if (num != -1)
		{
			Prototype itemPrototype = PrototypeYaml.GetItemPrototype(_tag.PetFoodReference[num]);
			if (itemPrototype != null)
			{
