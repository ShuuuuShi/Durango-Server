...
public class UIWidget : UIRect
{
	...
	public delegate void OnPostFillCallback(UIWidget widget, int bufferOffset, UIGeometry.Arguments arguments);
	...
	protected float mVisibleRatio = 1f;
	...
	private Vector3 _drawPanelPosition;

	private bool _isOtherTreePanel;

	[HideInInspector]
	[SerializeField]
	private UIPanel _drawPanel;

	protected bool _movedEventFlag;
	...
	public Vector2 PivotOffsetSize => Vector2.Scale(pivotOffset * 2f - Vector2.one, GetSize() * 0.5f);
	...
	public override float visibleRatio
	{
		get
		{
	...
	protected void UpdateFinalAlpha(int frameID)
	{
		...
		float num = mColor.a * mVisibleRatio;
		finalAlpha = ((!(uIRect != null)) ? num : (uIRect.CalculateFinalAlpha(frameID) * num));
	}
	...
	public virtual void MarkAsChanged()
	{
		if (NGUITools.GetActive(this))
		{
			...
			if (uIPanel != null && base.enabled && NGUITools.GetActive(base.gameObject) && !mPlayMode)
			{
				SetDirty();
				CheckLayer();
			}
	...
	public void AddOnChange(Action func)
	{
		if (func != null)
		{
			onChange = (Action)Delegate.Combine(onChange, func);
			if (mStarted)
			{
				func();
			}
	...
	private UIPanel FindDrawPanel()
	{
		if (_drawPanel == null)
		{
			_isOtherTreePanel = false;
			return UIPanel.Find(base.cachedTransform, createIfMissing: true, base.cachedGameObject.layer);
		}
		if (NGUITools.IsChild(_drawPanel.cachedTransform, base.transform))
		{
			_isOtherTreePanel = false;
		}
		else
		{
			_isOtherTreePanel = true;
			_drawPanelPosition = _drawPanel.cachedTransform.localPosition;
		}
		return _drawPanel;
	}
	...
	{
		if (_isOtherTreePanel)
		{
			Vector3 localPosition = panel.cachedTransform.localPosition;
			if (localPosition != _drawPanelPosition)
			{
				_drawPanelPosition = localPosition;
				MarkAsMoved();
			}
		}
		if (_movedEventFlag || mTrans.hasChanged)
		{
			_movedEventFlag = false;
			if (onChange != null)
			{
				onChange();
			}
	...
	{
		geometry.WriteToBuffers(v, u, c, n, t, extention);
	}
	...
	public virtual void OnFill(UIGeometry.Arguments arguments)
	{
	}
}
