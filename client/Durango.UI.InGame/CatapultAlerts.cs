...
public class CatapultAlerts : AreaOfEffectAlert
{
	...
	{
		float num = hideAt - showAt;
		int num3;
		if (num > 2f)
		{
			float num2 = ((!(_longParticleBaseSize > 0f)) ? 1f : (radius / _longParticleBaseSize));
			num3 = ParticleManager.Emit(_longParticle, position, Quaternion.identity, comeForwardToCamera: false, groundDecal: false, Vector3.one * num2);
		}
		else
		{
			float num4 = ((!(_shortParticleBaseSize > 0f)) ? 1f : (radius / _shortParticleBaseSize));
			num3 = ParticleManager.Emit(_shortParticle, position, Quaternion.identity, comeForwardToCamera: false, groundDecal: false, Vector3.one * num4);
		}
	...
	private void Update()
	{
		...
		for (int num = _particleDeactiveAt.Count - 1; num >= 0; num--)
		{
			float hideAt = _particleDeactiveAt[num].HideAt;
			if (hideAt < time)
			{
