...
public class PartyGroup : UIBase
{
	...
	private void Start()
	{
		...
		{
			PartyPlayerInfoWidget component = obj.GetComponent<PartyPlayerInfoWidget>();
			component.Kicked += PartyPlayerInfoWidget_Kicked;
			component.ButtonClicked += PartyPlayerInfoWidget_ButtonClicked;
			component.Clicked += PartyPlayerInfoWidget_Clicked;
		});
		...
		electLeader.Clicked = (Action)Delegate.Combine(electLeader.Clicked, (Action)delegate
		{
			...
			{
				_scrollView.Nodes[i].GetComponent<PartyPlayerInfoWidget>().ToggleElectLeader(flag2);
			}
		});
	...
	private static void InviteParty()
	{
		...
		UIManager.FindScript<PlayerSearchGroup>().OpenForMultiple(num, T._("파티 초대"), list2, delegate(IList<string> list)
		{
			if (list == null)
			{
				return;
	...
	private void Refresh()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			PartyPlayerInfoWidget component = _scrollView.Nodes.GetNext().GetComponent<PartyPlayerInfoWidget>();
			if (i < memberCount)
			{
				Member member = partySystem.GetMember(i);
