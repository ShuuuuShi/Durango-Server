...
public class PathMovable
{
	...
	public void Process(double at)
	{
		...
		for (int num2 = _movements.Count - 1; num2 >= 0; num2--)
		{
			...
			if (KUtility.GetSize(movement.Path) != 0 && movement.Path[0].Time < at)
			{
				Movement? next = null;
	...
	private void ApplyLocation(Location? prev, Location? next, double at)
	{
		if (prev.HasValue || next.HasValue)
		{
			...
			else
			{
				...
				float num2 = (float)(at - value2.Time) / (float)(value3.Time - value2.Time);
				Vector3 vector2 = Vector3.Lerp(value2.Position.ToClientPosition(), value3.Position.ToClientPosition(), num2);
				float num3 = _owner.ProcessWaterDepth(vector2);
				vector2.y = (float)(value2.Floor * 200) + Mathf.Lerp(value2.Height, value3.Height, num2) + num3;
				_owner.CurrentPosition = vector2;
				_owner.Floor.Value = value2.Floor;
				float num4 = Mathf.DeltaAngle(value2.Yaw, value3.Yaw);
				float yaw = value2.Yaw + num4 * num2;
	...
	public static string GetLastMotionName(Move msg)
	{
		...
		if (num == -1)
		{
			return "Barehand_Stand";
		}
		return msg.Movements[num].MotionName;
	}
