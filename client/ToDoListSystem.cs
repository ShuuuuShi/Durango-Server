...
public class ToDoListSystem : GameSystem<ToDoListSystem>
{
	...
	{
		return CollectionCount == 0 && _queuedItems.Count == 0;
	}
	...
	public void RemoveAll()
	{
		...
		{
			Durango.Logic.PlayGuide.ToDoCollection toDoCollection = _collections[i];
	...
	{
		return (!(collection is MissionToDoCollection)) ? ((!(collection is EntryTodoCollection)) ? 1 : 2) : 0;
	}
	...
	private void UpdateTweenTest()
	{
		...
		for (int collectionCount = toDoListSystem.CollectionCount; i < collectionCount; i++)
		{
			...
			{
				ToDoBase current = enumerator.Current;
				current.CallComplete();
