...
public class LineChatWidget : MonoBehaviour
{
	...
	{
		int absolute = _chattingLine.rightAnchor.absolute;
		if (absolute != offset)
		{
	...
	{
		string format = ((!(chat.Body is RadioDictation)) ? "[{3}]{0}[-] [{2}]{1}[-]" : "[{3}]{0}[-] [{2}][icon=icon_stt_s_stroke] {1}[-]");
		return string.Format(format, chat.Name, chat.FindText().Replace("\n", " "), NGUIText.EncodeColor(chat.GetMsgColor(Color.white)), NGUIText.EncodeColor(chat.GetNameColor()));
	}
}
