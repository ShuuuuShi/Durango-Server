...
public class MarketCategoriesWidget : MonoBehaviour
{
	...
	private void Init()
	{
		...
		{
			GameObject gameObject = nodes[j + 2];
			Category category3 = _categoryList[j];
			gameObject.transform.Find("name").GetComponent<UILabel>().text = category3.MainCategory.Name;
			gameObject.transform.Find("icon").GetComponent<UISprite>().spriteName = category3.MainCategory.Icon;
		}
