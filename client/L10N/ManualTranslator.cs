...
public static class ManualTranslator
{
	...
	{
		return (lastErrorFromAccountProvider != 0) ? $"{InvalidArguments} ({errorCode} / {lastErrorFromAccountProvider})" : $"{InvalidArguments} ({errorCode})";
	}

	public static string GetInitializeFailed(int errorCode, int lastErrorFromAccountProvider = 0)
	{
		return (lastErrorFromAccountProvider != 0) ? $"{InitializeFailed} ({errorCode} / {lastErrorFromAccountProvider})" : $"{InitializeFailed} ({errorCode})";
	}
}
