...
public class ClanAllyPage : ClanMenuPage, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		for (int i = 0; i < values.Count; i++)
		{
			...
			case AllyState.Ally:
				...
				{
					ClanAllyWidget component = obj.GetComponent<ClanAllyWidget>();
					component.ButtonClicked = _parent.SuggestBreak;
				});
				break;
			case AllyState.Suggested:
				values[i].Pool.Init(delegate(GameObject obj)
				{
					ClanAllyWidget component2 = obj.GetComponent<ClanAllyWidget>();
					component2.ButtonClicked = _parent.SuggestAllyCancel;
				});
				break;
			case AllyState.BeenSuggested:
				values[i].Pool.Init(delegate(GameObject obj)
				{
					ClanAllyWidget component3 = obj.GetComponent<ClanAllyWidget>();
					component3.ButtonClicked = _parent.BeenSuggestedAlly;
				});
				break;
			case AllyState.BreakSuggested:
				values[i].Pool.Init(delegate(GameObject obj)
				{
					ClanAllyWidget component4 = obj.GetComponent<ClanAllyWidget>();
					component4.ButtonClicked = _parent.BreakAlly;
				});
				break;
			case AllyState.BeenBreakSuggested:
				values[i].Pool.Init(delegate(GameObject obj)
				{
					ClanAllyWidget component5 = obj.GetComponent<ClanAllyWidget>();
					component5.ButtonClicked = _parent.BeenBreakSuggestedAlly;
				});
				break;
			case AllyState.Empty:
				values[i].Pool.Init(delegate(GameObject obj)
				{
					ClanAllyEmptyWidget component6 = obj.GetComponent<ClanAllyEmptyWidget>();
					component6.ButtonClicked = OnClickEmptyWidget;
				});
				break;
			default:
	...
	private void Set([NotNull] Clan clan, AllySlot[] slots)
	{
		...
		for (int size2 = KUtility.GetSize(slots); k < size2; k++)
		{
			...
			if (slot.State == AllySlotState.Locked)
			{
				...
				ClanAllyLockWidget component = uIWidget.GetComponent<ClanAllyLockWidget>();
				component.Set(slot);
				continue;
			}
			if (slot.IsAlly)
			...
			if (!(uIWidget == null))
			{
				ClanAllyWidget component2 = uIWidget.GetComponent<ClanAllyWidget>();
				component2.Set(slot, hasPermission);
			}
		}
		int l = 0;
		for (int size3 = KUtility.GetSize(slots); l < size3; l++)
		{
			...
			{
				ClanAllyEmptyWidget component3 = AddSlot(AllyState.Empty).GetComponent<ClanAllyEmptyWidget>();
				component3.Set(hasPermission);
			}
		}
		for (int m = num; m < ally.SlotOpensAt.Length; m++)
		{
			ClanAllySealedWidget component4 = AddSlot(AllyState.Sealed).GetComponent<ClanAllySealedWidget>();
			component4.Set(ally.SlotOpensAt[m]);
		}
		EndLoad();
	}
	...
	{
		List<UIWidget> widgets = _allySlotList.Widgets;
		widgets.Clear();
	...
	private UIWidget AddSlot(AllyState state)
	{
		...
		GameObject orAdd = objectPool.Pool.GetOrAdd(objectPool.Count++);
		UIWidget component = orAdd.GetComponent<UIWidget>();
