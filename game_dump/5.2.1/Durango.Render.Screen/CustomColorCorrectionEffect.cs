...
public class CustomColorCorrectionEffect : Singleton<CustomColorCorrectionEffect>
{
	...
	{
		if (Mathf.Approximately((!_atmosphericEffectOn) ? 0f : 1f, _curAtmosphereRatio))
		{
			return;
		}
	...
	public void GenerateLookupTexture(bool force = false)
	{
		...
		for (int i = 0; i < 256; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				byte b = (byte)Mathf.Clamp(GetColorCorretionResult(original, num, num2, num3) * 255f, 0f, 255f);
				switch (j)
				{
				case 0:
					pixels[i].r = b;
	...
	private void SetGlobalLightColor()
	{
		...
		{
			Texture2D texture2D = ToReadableTexture((!(CurrentSet.WorldLightTex != null)) ? CurrentSet.RampTex : CurrentSet.WorldLightTex);
