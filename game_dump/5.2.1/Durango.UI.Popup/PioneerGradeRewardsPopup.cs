...
public class PioneerGradeRewardsPopup : TooltipBase
{
	...
	private void UpdateNodes()
	{
		...
		foreach (PioneerGradeReward pioneerGradeReward in rewards)
		{
			...
			next.FindComponent<UISprite>("GradeBG").color = ((grade != cur.Grade) ? PresetColor.UIBlackAlpha40 : PresetColor.UIYellow);
			next.FindComponent<UILabel>("Description").text = ((grade <= cur.Grade) ? pioneerGradeReward.Texts.After : pioneerGradeReward.Texts.Before);
		}
		nodes.EndLoad();
