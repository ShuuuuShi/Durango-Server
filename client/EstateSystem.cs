...
public class EstateSystem : GameSystem<EstateSystem>
{
	...
	private static bool IsValidEstateInfo(EstateInfo info)
	{
		...
		return (activatedAt <= 0.0 || activatedAt < predictedServerTime) && (num <= 0.0 || predictedServerTime < num);
	}
	...
	{
		return GameManager.Region.IsAfterSafeHouse() && PlayerBehavior.LocalPlayer.Driver.IsWarpAllowed;
	}
