...
public class ArchipelagoInfo : DiscoveryInfo
{
	...
	public bool _Set(Biome biome, int unstableFactor)
	{
		...
		bool flag = statistics.Value.RepresentPowers.Get(RepresentType.CollectingPower, 0f) >= (float)recommends.CollectingPower;
		...
		bool flag2 = statistics.Value.ResistanceLevels.Get(derived, 0) >= recommends.ResistanceLevel;
