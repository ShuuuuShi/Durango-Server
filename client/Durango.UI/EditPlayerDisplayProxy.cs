...
public class EditPlayerDisplayProxy
{
	...
	public void SetTornBody(string m, string f)
	{
		...
		_tornBody[1] = f;
		PlayerCostumeTable.ClothState? clothState = _clothState;
		if (clothState.HasValue)
		{
			PlayerCostumeTable.ClothState value = _clothState.Value;
			if (value == PlayerCostumeTable.ClothState.Torn)
			{
				SetDirty();
			}
		}
	...
	public void SetNudeBody(string m, string f)
	{
		...
		{
			PlayerCostumeTable.ClothState value = _clothState.Value;
			if (value == PlayerCostumeTable.ClothState.Nothing)
			{
				SetDirty();
			}
		}
	...
	private static string[] GetSuitableClothColor(int jobIndex)
	{
		...
		List<ClothesColorTableInfo> colorTableClothesMap = ColorTableLoader.ColorTableClothesMap;
		ClothesColorTableInfo clothesColorTableInfo = colorTableClothesMap.Find((ClothesColorTableInfo elem) => elem.Keyword.Contains(targetModelKey));
	...
	{
		Color[] all = ColorTableLoader.GetAll("color_create.raw");
		return all.ShuffleTake(count).Select(NGUIText.EncodeColor).ToArray();
	}
