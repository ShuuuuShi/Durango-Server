...
public class WalletInfo : MonoBehaviour
{
	...
	public void SetCurrency(Currency currency)
	{
		...
		string arg = ((!PresetCurrencyWidget.IsChargable(currency)) ? string.Empty : "  [FFFFFF34][icon=box_plus][-]");
		_label.text = $"[c]{Durango.Logic.Item.Inventory.CurrencyFormat(balance)}[/c]{arg}";
		_label.alpha = alpha;
	}
	...
	private void GetTooltipText(out string title, out string comment)
	{
		...
		if (_currency.HasValue)
		{
			title = ((Enum)(object)_currency).GetName();
