...
public class Commodity : IComparable<Commodity>
{
	...
	public bool IsFree
	{
		get
		{
			...
			if (!isFree.HasValue)
			{
				...
				{
					_isFree = Money.Amount == 0 && Data.OriginalPriceAmount == 0 && Data.VoucherAmount == 0;
				}
			}
	...
	public bool IsFirstPurchaseBonus(string purchaseId = null)
	{
		if (HasFirstPurchaseBonus())
		{
			...
			return string.IsNullOrEmpty(firstPurchasedId) || firstPurchasedId == purchaseId;
		}
		return false;
	}
	...
	{
		return !string.IsNullOrEmpty(Data.VoucherId) && Data.VoucherAmount > 0;
	}
	...
	{
		return (Platform.Instance.Store == Platform.StoreType.Steam) ? Data.SteamDlcAppId : 0u;
	}
	...
	public string GetRemainingTime()
	{
		...
		double seconds = purchasableTime.GetPurchaseEndsAt() - predictedServerTime;
		return TimedeltaFormatter.Format(seconds, 2, "min");
	}
	...
	public bool IsQuestPurchase(CommodityCondition.Type? type = null)
	{
		if (KUtility.GetSize(SubCommodities) > 0)
		{
			...
			return SubCommodities[0].AcceptCondition.ConditionType == type.GetValueOrDefault() && type.HasValue;
		}
		return false;
	}
	...
	private void AppendContentDescription(ShopContents contents)
	{
		...
		foreach (string text in motions)
		{
			...
			if (contentDescription == null)
			{
				ContentDescription contentDescription2 = new ContentDescription();
				contentDescription2.SourceKey = text;
	...
	private void AppendContents([CanBeNull] IEnumerable<CommodityContent> contents)
	{
		...
		foreach (CommodityContent content in contents)
		{
			if (!content.hide_in_shop)
			{
				...
				{
					ContentDescription contentDescription2 = new ContentDescription();
					contentDescription2.SourceKey = content.key;
					contentDescription = contentDescription2;
					ContentDescriptions.Add(contentDescription);
				}
