...
`RemoveArtifact()` ลบของในกล่องทิ้งด้วย — **ทุบกล่องแล้วของข้างในหายเลย** ยังไม่มีระบบดรอปลงพื้น
...
- **ไม่เช็คระยะ** — ส่ง `EntityId` ของกล่องที่อยู่คนละมุมเกาะก็หยิบของได้ (`Tile` ที่ client ส่งมายังไม่ได้ใช้ตรวจ)
- **ไม่มีสิทธิ์/ล็อก** — ใครก็เปิดกล่องคนอื่นได้ (component `Secured` ยังไม่ได้ใช้)
- **ทุบกล่องแล้วของหาย** — ควรดรอปลงพื้นหรือคืนให้เจ้าของ
