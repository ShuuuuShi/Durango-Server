...
public class MainCamera : Singleton<MainCamera>
{
	...
	{
		Vector3 nguiPos = Singleton<MainCamera>.Instance().Camera.WorldToScreenPoint(world);
		return ScreenPosToNGUIPos(nguiPos, relativeTo);
	}
	...
	private void OnScreenResize()
	{
		...
		else if (!Camera.orthographic)
		{
			...
			float num2 = UnityEngine.Screen.height;
	...
	private void UpdateCameraPosition()
	{
		...
		bool flag = _needUpdateCamera || Mathf.Abs(_prevFinalZoom - num) > 0.001f;
		if (flag)
		{
			_prevFinalZoom = num;
	...
	public void UpdateCameraNearFar()
	{
		...
		float num4 = base.transform.localPosition.y - _maxHeightForNearPlane;
		float num5 = num4 / Mathf.Cos(f);
		float a = num5 * num3 - 100f;
		float num6 = base.transform.localPosition.y + _additionalHeightForFarPlane;
		float f2 = num - num2;
		float num7 = num6 / Mathf.Sin(f2);
		float a2 = num7 * num3;
