...
public class InteractionSystem : GameSystem<InteractionSystem>
{
	...
	private void Start()
	{
		...
		Durango.Utils.Singleton<GameManager>.Instance().MainSceneLoaded += delegate
		{
			...
			_drawTimer.Started += delegate(PredictTimer timer)
			{
				...
				IconProgressGauge iconProgressGauge4 = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(timer.Timer);
				iconProgressGauge4.AddIcon(CurrentMenu.Icon);
			};
			...
			_washBodyTimer.Started += delegate(PredictTimer timer)
			{
				...
				IconProgressGauge iconProgressGauge3 = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(timer.Timer);
				iconProgressGauge3.AddIcon(IconMap.Get(Interaction.WashBody));
			};
			...
			_drinkWaterTimer.Started += delegate(PredictTimer timer)
			{
				...
				iconProgressGauge2.AddIcon(IconMap.Get(Interaction.DrinkWater));
			};
			...
			_lookAroundTimer.Started += delegate(PredictTimer timer)
			{
				...
				iconProgressGauge.AddIcon(IconMap.Get(Interaction.LookAroundArtifact));
			};
		};
	...
	public static GameObject PropInteractionObjectFilter(GameObject o)
	{
		...
		return (!ObjectIdentifier.IsDeadBody(o)) ? null : o;
	}
	...
	public static GameObject ImmovableObjectFilter(GameObject o)
	{
		...
		return (!(componentInParent != null)) ? null : componentInParent.gameObject;
	}
	...
	{
		return _menuList.HasPlayingTimer() || GameSystem<GatheringSystem>.Instance().IsGathering;
	}
		...
		if (interactionHandler == null)
		{
			interactionHandler = delegate(InteractionObject obj)
			{
				...
				if (!(gameObject != null) || !gameObject.GetComponent<SelectableObject>().MenuClicked(obj.Target, menu))
				{
					...
					string text = menu.Action.ToString();
				}
			};
		}
	...
	private float GetTouchedValidTime(Touched touched)
	{
		...
		return (!(num > 0.0)) ? 0f : Times.UnixTimeToUnityTime(num);
	}
	...
	public void RefreshInteractionMenu()
	{
		...
		if (LastTouched.Level > 0 && LastTouched.Collectible.Generators.Length > 0)
		{
			...
			int levelDiff = LastTouched.Level - level;
	...
	private static void BiomeContextAction(List<InteractionMenuData> result)
	{
		...
		Dictionary<string, PutInContainerInfo> putInContainerInfos = Yaml.Util.Singleton<Constants>.Instance.PutInContainerInfos;
		foreach (KeyValuePair<string, PutInContainerInfo> item in putInContainerInfos)
		{
			if (item.Value.Biomes != null && Array.IndexOf(item.Value.Biomes, biome) != -1)
			{
	...
	private static void TileObjectContextAction(List<InteractionMenuData> result)
	{
		...
		if (value.HasValue)
		{
			int value2 = PlayerBehavior.LocalPlayer.Floor.Value;
