...
public class VehicleAirBalloon : VehicleProp
{
	...
	private bool IsProhibitedLandingArea()
	{
		...
		position.y = 0f;
		Biome tileBiome = Singleton<TerrainBase>.Instance().GetTileBiome(Util.ClientPositionToWorldPosition(position));
		if (tileBiome == Biome.Invalid || Util.IsWater(tileBiome))
		{
			return true;
		}
		if (Collisions.TryCapsuleCast(Collisions.CreateCollisionParam(position, Vector3.zero), out var raycastHit) != 0 && IsCollidable(raycastHit))
		{
			return true;
		}
		return EstateSystem.GetEstateInfo(new Point2(Util.ClientPositionToTilePosition(position))) != null;
	}
	...
	private bool IsCollidable(RaycastHit hit)
	{
		...
		if (!(base.Driver == null))
		{
			return hit.collider.gameObject != base.Driver.gameObject;
		}
		return true;
	}
