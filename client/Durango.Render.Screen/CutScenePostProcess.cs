...
public class CutScenePostProcess : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		{
			UnityEngine.Camera camera = Singleton<MainCamera>.Instance().Camera;
			Vector3 vector = camera.WorldToViewportPoint(_centerObject.transform.position);
	...
	private Texture2D GenerateLookupTex()
	{
		...
		for (int i = 0; i < 256; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				float colorCorretionResult = CustomColorCorrectionEffect.GetColorCorretionResult(original, _hilight[j], _midtone[j], _shadow[j]);
