...
public class TimelineLogBuilder
{
	...
	{
		if (_log.Type == TimelineEvent.Destroy)
		{
			return true;
		}
		return false;
	...
	public void Build(Action<TimelineLogBuilder> completed)
	{
		...
		if (!IsLoadingParams)
		{
			if (!string.IsNullOrEmpty(_log.AgentEntityId) && AgentPlayer == null)
			{
				_isLoadingAgentPlayer = true;
			}
			if (!string.IsNullOrEmpty(_log.TargetEntityId) && TargetPlayer == null)
			{
			...
			if (_isLoadingAgentPlayer)
			{
				...
				playerInfoManager.RequestPlayerInfo(log.AgentEntityId, OnResponseAgentPlayer);
			}
			if (_isLoadingTargetPlayer)
			{
				...
				playerInfoManager2.RequestPlayerInfo(log2.TargetEntityId, OnResponseTargetPlayer);
			}
	...
	private void BuildTextIfParamLoaded()
	{
		...
		int num = KUtility.GetSize(_log.Params);
		if (text != null)
		{
			num++;
		}
		...
		int i = 0;
		for (int size = KUtility.GetSize(_log.Params); i < size; i++)
		{
	...
	private string GetParam(int index)
	{
		...
		{
			if (index == 0 && double.TryParse(_log.Params[0], out var result2))
			{
		...
		{
			if (index == 1 && int.TryParse(_log.Params[1], out var result))
			{
		...
		return _log.Params[index];
	}

	private void BuildText(string[] param)
	{
		bool flag = !string.IsNullOrEmpty(_log.AgentEntityId);
		bool flag2 = !string.IsNullOrEmpty(_log.TargetEntityId) || _log.TargetArtifact.HasValue;
		Dictionary<int, TimelineMessage> messages = Yaml.Util.Singleton<TimelineMessagesYaml>.Instance.messages;
		Messages.TimelineLog log = _log;
		if (messages.TryGetValue((int)log.Type, out var value))
		{
