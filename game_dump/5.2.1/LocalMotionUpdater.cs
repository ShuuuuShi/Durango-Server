...
public class LocalMotionUpdater
{
	...
	{
		if (MotionMap.Instance().GetRideMotion(rideMotionSetName) != null)
		{
	...
	private bool CheckReservedMotions()
	{
		...
		if (!TrySetMotionByState(reservedMotion2.Motion, reservedMotion2.PlaybackRate, reservedMotion2.ForceTransition, reservedMotion2.Equip, reservedMotion2.EquipColor))
		{
			return TrySetMotionByStateClip(reservedMotion2.Motion, string.Empty, reservedMotion2.PlaybackRate, reservedMotion2.ForceTransition, reservedMotion2.Equip, reservedMotion2.EquipColor);
		}
		return true;
	}
	...
	private void UpdateCurrentMotionClip()
	{
		...
		else
		{
			...
			if (Player.Life != null && Player.IsAlive)
			{
				if (_movedByManual)
				{
					state = ((!IsInWater) ? "Run" : ((Player.GetBiome() != Biome.Lava) ? "Water_Swim" : "Lava_Walk"));
					playbackRate = 1f;
				}
	...
	{
		StandStateEnum standStateEnum;
		if ((bool)IsBattleStand)
		{
			standStateEnum = StandStateEnum.BattleStand;
		}
		else
		{
			string text = GameSystem<FatigueSystem>.Instance().FatigueEffect;
			StandStateEnum standStateEnum2 = ((text == "hot") ? StandStateEnum.Hot : ((!(text == "cold")) ? StandStateEnum.Stand : StandStateEnum.Cold));
			standStateEnum = standStateEnum2;
		}
		StandStateEnum standState = standStateEnum;
