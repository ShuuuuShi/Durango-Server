...
public class VerticalLayoutWidget : UIWidget, RectLayout.ICompatible
{
	...
	public class Subject
	{
		...
		public IEnumerable<UIWidget> Collection
		{
			get
			{
				if (HasPool)
		...
		public Vector2 RepresentativeSize
		{
			get
			{
				if (HasPool)
				{
					return Pool.BaseObject.localSize;
				}
				return new Vector2(_childs.Max((UIWidget elem) => elem.width), _childs.Max((UIWidget elem) => elem.height));
			}
		}
	...
	private float ProperWidth
	{
		get
		{
			...
			if (_resizeWidgetWidth && (float)_maxWidthWhenResize != 0f)
			{
				return Mathf.Min(_maxWidthWhenResize, num);
			}
			return num;
		}
	}
	...
	private Vector2 UpdateAsStretched(float w)
	{
		...
		Vector2 vector = (Vector2)localCorners[1] + new Vector2(_padding.Left, 0f - _padding.Up);
