...
public class PvpIslandGroup : UIBase
{
	...
	private void Awake()
	{
		...
		{
			UIManager.Popup.Tooltip<PvpIslandGuidePopup>().Show();
		};
	...
	private void WarpRushSystem_EntreeInfoUpdated(S02EntreeInfo info)
	{
		_waitingCount.SetText(new SyncString(delegate(out string text, out float period)
		{
			...
			string text2 = T._("대기 인원  [FFFFFF3C]<bar/>[-]  <em>{0}</em>[FFFFFF7F]/[-]{1}", info.QueueCount, OptionSystem.GetWarpRushEntryCount());
			string text3 = string.Format("{0}  [FFFFFF3C]<bar/>[-]  <em>{1}</em>", T._("남은 시간"), (!flag) ? " - " : TimedeltaFormatter.Format(num));
			text = text2 + "        " + text3;
