...
public class PlayerBehavior : CharacterBehavior, IAnimationEventPlayable, ICostumable, IMotionPlayable
{
	...
	public bool HasClan => !string.IsNullOrEmpty(ClanId) && Clan.RoleId != -1;

	public bool IsClanOwner => !string.IsNullOrEmpty(ClanId) && Clan.RoleId == 0;
	...
	public Vector3 CameraOrigin
	{
		get
		{
			...
			return (!IsRiding) ? vector : Driver.CalcCameraOrigin(vector);
		}
	}
	...
	public Vector3 FloatingUIPosition => (!IsRiding || !(Driver.Vehicle != null)) ? CurrentPosition : Driver.Vehicle.transform.position;
	...
	public bool IsTired
	{
		get
		{
			Gauge fatigue = Fatigue;
			if (fatigue == null)
			{
				return false;
			}
			Fatigue.State state = GameSystem<FatigueSystem>.Instance().Fatigue.GetState();
			return state == Durango.Logic.Fatigue.State.Danger;
		}
	}
	...
	public string MotionPrefix => (!IsMale) ? "F_" : "M_";
	...
	{
		return (!(Anim != null)) ? null : Anim[base.CurrentAnimClipName];
	}
	...
	public void SetFramework(Transform newFramework)
	{
		...
		list.Add(_meshObjectTransform);
		List<Transform> list2 = list;
		_meshObjectTransform.GetComponentsInChildren(list2);
	...
	public void SetParticleEffects([NotNull] Pair<string, string>[] effects)
	{
		...
		{
			int num = effects.IndexOf(item.Key);
			if (num == -1)
			{
		...
		int i = 0;
		for (int num2 = effects.Length; i < num2; i++)
		{
			Pair<string, string> key = effects[i];
			if (!_effectDict.ContainsKey(key))
			{
				int num3 = ParticleManager.Emit(base.gameObject, key.Item1, key.Item2);
				if (num3 != 0)
				{
					_effectDict.Add(key, num3);
				}
			}
		}
		if (list != null)
		...
		else
		{
			...
			Singleton<AssetBundleManager>.Instance().RequestAsset(path, typeof(GameObject), delegate(UnityEngine.Object asset)
			{
				if (!(this == null) && !(path != _loadingEquipmentPath))
				{
					...
					if (gameObject == null || string.IsNullOrEmpty(_playerEquipment.GetCurrentPath()))
					{
						UpdateWeaponTip();
					}
					else
	...
	public void StopMusic()
	{
		...
		if (IsLocalPlayer)
		{
			bool flag = _musicEndedAt.HasValue && Time.time > _musicEndedAt.Value - 1f;
			_playingMusic = null;
			_musicEndedAt = null;
	...
	private void SetInstrument(string timbre)
	{
		...
		string path = instrument?.InstrumentObject.Path;
		if (string.IsNullOrEmpty(path))
		{
			return;
		}
		Singleton<AssetBundleManager>.Instance().RequestAsset(path, typeof(GameObject), delegate(UnityEngine.Object asset)
		{
			...
			else if (!(_instrumentObject.Item1 != timbre))
			{
				...
				else
				{
					...
					gameObject2.transform.SetPositionAndRotation(position, base.transform.rotation);
					_instrumentObject = new Pair<string, GameObject>(timbre, gameObject2);
				}
			}
		});
	}
	...
	public override Transform GetBodyPartTransform(BodyPart part, bool bAllowNull = false, Vector3 nearPos = default(Vector3))
	{
		...
		{
			return (!bAllowNull) ? MainTransform : null;
		}
	...
	{
		foreach (PlayerAnimationClipInfo clip in clips)
		{
			if (clip != null && TryPlayClip(clip, playBackRates))
			{
				yield return new WaitForSeconds(clip.Length - clip.FadeOutTime);
			}
		}
	}
	...
	public override double GetMoveServerTime()
	{
		...
		return (!IsLocalPlayer) ? (bufferedServerTime - (double)_playerBufferTime.BufferTime) : Connections.Frontend.GetPredictedServerTime();
	}
	...
	private IEnumerator CoReservedMountTarget()
	{
		...
		while (true)
		{
			if (string.IsNullOrEmpty(_reservedMountTargetId))
			{
				yield break;
			}
			if (TryMountTarget(_reservedMountTargetId))
			{
				break;
			}
