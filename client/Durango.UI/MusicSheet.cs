...
public class MusicSheet : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		bool flag = !offset.HasValue || vector != _offset.Value;
		_offset = vector;
		if (flag)
		{
			SyncScroll();
		}
		if (flag || _isDirtyVisibleNotes)
		{
			RefershNoteWidgets();
		}
	}
	...
	private void MoveToPlayHandleToCurrentTouch(bool stop)
	{
		...
		float num = _offset.GetValueOrDefault().x + vector.x - 30f;
		int value = Mathf.RoundToInt(num / _tickWidth);
	...
	public void UpdateMusicRunningTime()
	{
		...
		if (size > 0)
		{
			int lastTick = _music.GetLastTick();
			int num2 = Mathf.CeilToInt((float)lastTick / (float)_music.Division) + 1;
	...
	public void SetGuideLine(float timer, bool scrollSync)
	{
		...
		float num2 = 480f / (float)_music.Division;
		float num3 = num2 / num;
		Vector3 vector = new Vector3(timer * num3, 0f);
		vector.x += 30f;
		float num4 = 0f - _scrollView.transform.localPosition.x;
		Vector3 localPosition = vector + Vector3.left * num4;
		if (scrollSync)
		{
			bool flag = false;
			if (localPosition.x < _scrollMargin && num4 > 0f)
			{
				flag = true;
				num4 = Mathf.Max(0f, num4 - (_scrollMargin - localPosition.x));
			}
			else if (localPosition.x > (float)_widget.width - _scrollMargin)
			{
				flag = true;
				num4 += localPosition.x - ((float)_widget.width - _scrollMargin);
				if (num4 > _limitWidth - (float)_widget.width)
				{
					_limitWidth = num4 + (float)_widget.width;
					UpdateScrollBounds();
				}
			}
			if (flag)
			{
				...
				localPosition2.x = 0f - num4;
				clipOffset.x = num4;
				_scrollView.transform.localPosition = localPosition2;
				_scrollView.panel.clipOffset = clipOffset;
				_scrollView.UpdateScrollbars(recalculateBounds: false);
				localPosition = vector + Vector3.left * num4;
			}
		}
		foreach (MusicNote value in _makingNoteList.Values)
		{
			float num5 = _music.TickToTimer(value.Note.Tick);
			float num6 = timer - num5;
			value.width = (int)(num6 * num3);
			UIUtility.UpdateAnchors(value.transform);
		}
	...
	private MusicNote PopNoteObject()
	{
		...
		{
			GameObject gameObject = UnityEngine.Object.Instantiate(_noteBase.gameObject, _noteBase.transform.parent);
			musicNote = gameObject.GetComponent<MusicNote>();
