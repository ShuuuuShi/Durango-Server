...
public class ClanMakePage : ClanMenuPage, IUIInitializable
{
	...
	private void OnClickMakeButton()
	{
		...
		long balance = InventorySystem.Wallet.GetBalance(selectedCostType.Key);
		if (balance < selectedCostType.Value)
		{
	...
	private void OnScreenResize()
	{
		...
		for (int i = 0; i < _iconWidgets.Length; i++)
		{
			...
			if (i % num3 != 0)
			{
				GameObject gameObject = _iconSeperator.Add();
				gameObject.transform.localPosition = _iconWidgets[i].transform.localPosition - Vector3.right * ((float)num5 / 2f - 1f);
			}
		}
