...
public class AccelerationChecker : Singleton<AccelerationChecker>
{
	...
	private void CameraLean()
	{
		...
		Vector3 vector2 = Vector3.right * vector.x * _moveDirectionCameraLean.x + Vector3.up * vector.y * _moveDirectionCameraLean.y;
		if (vector2 != _moveCamLeanAngle)
		{
			float num = _cameraLeanSpeed * Time.fixedDeltaTime;
			float f2 = vector2.x - _moveCamLeanAngle.x;
			float f3 = vector2.y - _moveCamLeanAngle.y;
			if (Mathf.Abs(f2) < num)
			{
				_moveCamLeanAngle.x = vector2.x;
			}
			else
			{
