...
public static class PetUtil
{
	...
	public static CageStatus ConverInfoToStatus(DomesticationInfo info)
	{
		...
		{
			double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
			if (predictedServerTime < info.DomesticateUntil)
			{
	...
	{
		return (KUtility.GetSize(eatableTags) != 0) ? ((Predicate<ItemData>)((ItemData data) => data.GetPerformanceData("pet_food").HasValue && (from item in data.Tags
			join eatable in eatableTags on item.Id equals eatable
			select item).Any())) : ((Predicate<ItemData>)((ItemData data) => data.GetPerformanceData("pet_food").HasValue));
	}
	...
	public static int GetCurrentPetMilestoneIndex(Messages.Pet pet)
	{
		...
		{
			MilestoneInfo milestoneInfo = pet.Statistics.MilestonesInformation[num];
			if (milestoneInfo.Acquired)
			{
	...
	public static bool IsPetRemainMilestone(Messages.Pet pet)
	{
		...
		int num = 0;
		for (num = KUtility.GetSize(pet.Statistics.MilestonesInformation) - 1; num >= 0; num--)
		{
			MilestoneInfo milestoneInfo = pet.Statistics.MilestonesInformation[num];
			if (milestoneInfo.Acquired)
			{
	...
	{
		return PetReadyToActiveSkill(pet) && !HasPetActiveSkill(pet);
	}
	...
	public static bool PetReadyToActiveSkill(Messages.Pet pet)
	{
		...
		return milestonesInformation.LastOrDefault().Acquired && pet.Stat.LastMilestoneAccepted;
	}
