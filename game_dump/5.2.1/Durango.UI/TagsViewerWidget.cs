...
public class TagsViewerWidget : UIWidget
{
	...
	public void AddTagData(string id, int level)
	{
		...
		{
			bool num = value.IsMajor();
			TagItemWidget next = ((!num) ? _minorTagControls : _majorTagControls).GetNext();
			next.Set(value, level);
			if (!num)
			{
				int num2 = next.NameLabel.width + 20;
				next.width = num2;
			}
		}
	}
	...
	private float UpdateTagsLayout(int padding)
	{
		...
		int num4 = (num3 - 20) / 3;
		float num5 = (float)(num3 - num4 * 3) / 2f;
