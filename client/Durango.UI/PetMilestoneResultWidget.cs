...
public class PetMilestoneResultWidget : MonoBehaviour, IUIInitializable
{
	...
	private void SetGaugeStats(MilestoneResult result)
	{
		...
		{
			PetMilestoneGaugeResultWidget next = _gaugeResults.GetNext();
			next.Set(T._("생명"), $"{prev:0} <em>+{current - prev:0}</em>", prev / current, new Color32(182, 57, 45, byte.MaxValue));
		}
		else
		{
			PetMilestoneGaugeResultWidget next2 = _gaugeResults.GetNext();
			next2.Set(T._("생명"), current.ToString("0"), 1f, new Color32(182, 57, 45, byte.MaxValue));
		}
		if (TryGetChangedStat(result, Derived.LifeSpan, out prev, out current))
		{
			PetMilestoneGaugeResultWidget next3 = _gaugeResults.GetNext();
			next3.Set(T._("수명"), $"{TimedeltaFormatter.Format(prev)} <em>+{TimedeltaFormatter.Format(current - prev)}</em>", prev / current, PresetColor.UIYellow);
		}
		else
		{
			PetMilestoneGaugeResultWidget next4 = _gaugeResults.GetNext();
			next4.Set(T._("수명"), TimedeltaFormatter.Format(current), 1f, PresetColor.UIYellow);
		}
		if (TryGetChangedStat(result, Derived.HungryMax, out prev, out current))
		{
			PetMilestoneGaugeResultWidget next5 = _gaugeResults.GetNext();
			next5.Set(T._("활력"), $"{prev} <em>+{current - prev}</em>", prev / current, PresetColor.UIYellow);
		}
		else
		{
			PetMilestoneGaugeResultWidget next6 = _gaugeResults.GetNext();
			next6.Set(T._("활력"), current.ToString("0"), 1f, PresetColor.UIYellow);
		}
