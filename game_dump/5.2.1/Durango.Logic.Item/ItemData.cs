...
public class ItemData
{
	...
	public bool Dumpable
	{
		get
		{
			if (Prototype != null)
			{
				return !Prototype.DumpLocked;
			}
			return true;
		}
	}
	...
	public bool IsRepairable
	{
		get
		{
			if (RepairRequirement.HasValue)
			{
				return !string.IsNullOrEmpty(RepairRequirement.Value.TagId);
			}
			return false;
		}
	}
	...
	public void Set(PrototypePreset preset)
	{
		...
		PrototypeName = ((Prototype == null) ? string.Empty : Prototype.Name.ToString());
		int num = (OriginalLevel = preset.Level);
		int level2 = num;
		Level = level2;
	...
	private void SetDyeable(IList<ColorChannel> dyeables)
	{
		...
		{
			ColorChannel num = dyeables[i];
			if (_dyeables == null)
			{
				_dyeables = new bool[Icon.Colors.Count];
			}
			int num2 = (int)num;
			if (num2 >= 0 && num2 < _dyeables.Length)
			{
				_dyeables[num2] = true;
			}
		}
	...
	{
		bool num = requiredTags.Count <= 0 || HasTagInContents(requiredTags, ignoreLevel);
		bool flag = requiredMaterials.Count <= 0 || HasTagInContents(requiredMaterials, ignoreLevel);
		return num && flag;
	}
	...
	{
		int size = KUtility.GetSize(_dyeables);
		if (channel >= ColorChannel.ColorR && (int)channel < size)
		{
			return _dyeables[(int)channel];
		}
		return false;
	}
	...
	public bool IsDomesticatedPet()
	{
		if (Reins.HasValue)
		{
			return Reins.Value.Domesticated;
		}
		return false;
	}
	...
	private bool ExistTag(OrTagFilter tagFilters, bool ignoreLevel = false)
	{
		if (tagFilters != null)
		{
			...
			for (int i = 0; i < count; i++)
			{
				if (GetSuitableTag(tagFilters[i], ignoreLevel) != null)
				{
					return true;
				}
			}
	...
	private bool HasTagInContents(OrTagFilter tagFilters, bool ignoreLevel = false)
	{
		if (ContentCount > 0)
		{
			return ExistTagInContents(tagFilters, ignoreLevel);
		}
		return ExistTag(tagFilters, ignoreLevel);
	}
