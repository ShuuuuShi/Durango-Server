...
public class WarpRushSystem : GameSystem<WarpRushSystem>
{
	...
	[CompilerGenerated]
	private sealed class _003CCoUpdateDayAndPhase_003Ed__100 : IEnumerator<object>, IDisposable, IEnumerator
	{
		...
		[DebuggerHidden]
		public _003CCoUpdateDayAndPhase_003Ed__100(int _003C_003E1__state)
		{
	...
	{
		if (_warpRushTotalResources == null)
		{
			return 0;
		}
		return _warpRushTotalResources.Get(stoneType, 0);
	}
	...
	public static string GetDeliveryMessage(bool isLevelUpReward, ResourceType resourceType)
	{
		if (isLevelUpReward)
		{
			return T._("{0} 레벨 달성 보상 획득", GetResourceName(resourceType));
		}
		return T._("{0} 교환 완료", GetResourceName(resourceType));
	}

	private static void RequestSurvivorRegionInfo()
	{
		GameManager.Region.IsWarpRush();
	}

	private IEnumerator CoUpdateDayAndPhase()
	{
		//yield-return decompiler failed: Unexpected instruction in Iterator.Dispose()
		return new _003CCoUpdateDayAndPhase_003Ed__100(0);
	}
	...
	public bool IsCashRewardPurchasable()
	{
		...
		if (commodity.CommodityInfo.MaxPurchasableCount.HasValue)
		{
			return commodity.CommodityInfo.MaxPurchasableCount.Value > 0;
		}
		return false;
	}
	...
	public bool IsCashRewardOnSale()
	{
		...
		bool num = !string.IsNullOrEmpty(commodity.GetRemainingTime());
		bool flag = purchaseLimit.MaxCount > 0 || purchaseLimit.PeriodicCountsLimit.Counts > 0 || purchaseLimit.PeriodicLimit.Days > 0;
		return num && flag;
	}
	...
	{
		Http.RequestYml(GameManager.GatewayUrl + "/ranking/" + key.Key.ToString().ToSnakeCase() + "/" + PlayerBehavior.LocalPlayer.EntityId + "?rev=" + key.Value, delegate(RankingInfo info)
		{
