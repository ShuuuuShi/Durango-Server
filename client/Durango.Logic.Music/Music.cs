...
public class Music
{
	...
	public static Music CreateFromMabinogiMML(string text)
	{
		...
		if (text2.StartsWith("MML@", StringComparison.InvariantCultureIgnoreCase))
		{
			...
			string[] tracks = text2.Split(',');
			return CreateFromMML(tracks);
		}
		return null;
	}
	...
	private static Music CreateFromMML(IEnumerable<string> tracks)
	{
		...
		foreach (string track in tracks)
		{
			...
			int tempo;
			IEnumerable<KeyValuePair<double, MML.Note>> enumerable = MMLParser.ToNotes(track, out tempo);
			if (music == null)
			{
				music = new Music();
				music.Division = 120;
			...
			foreach (KeyValuePair<double, MML.Note> item3 in enumerable)
			{
