...
public class DeliveryWidget : AnimationWidget
{
	...
	private void UpdateTitleLabel()
	{
		...
		if (!string.IsNullOrEmpty(prototypeId) || !string.IsNullOrEmpty(tagId))
		{
			...
			{
				UIManager.FindScript<MarketGroup>().OpenAndSearch(prototypeId, prototypeLevel, tagId);
			};
		}
		else
		{
	...
	private bool ItemListFilter(ItemData item)
	{
		...
		if (CheckTags(condition, item) && CheckPrototype(condition, item))
		{
			return CheckCollectSource(condition, item);
		}
		return false;
	}
