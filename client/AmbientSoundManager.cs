...
public class AmbientSoundManager : Singleton<AmbientSoundManager>
{
	...
	{
		if (_ambientSoundSets.TryGetValue(tileSet, out var value))
		{
			if (Biome.TemperateForest <= biome && (int)biome < value.Length)
			{
	...
	private static float MinSquaredDistance(int radiusInTile, Vector3 worldPos, Biome currentBiome)
	{
		...
		for (int i = -radiusInTile; i <= radiusInTile; i++)
		{
			for (int j = -radiusInTile; j <= radiusInTile; j++)
			{
				...
				{
					Point2 chunkCoords = Util.WorldPositionToChunkCoords(vector);
					Vector3 vector2 = Util.ChunkCoordsToWorldPosition(chunkCoords);
					int num2 = (int)((vector - vector2).x / 200f);
					int num3 = 16 - num2 - 1;
					j += num3;
					continue;
				}
				Biome tileBiome = chunkFromWorldPosition.GetTileBiome(vector);
				if (tileBiome == currentBiome)
				{
					num = sqrMagnitude;
					if (num <= 40000f)
					{
