...
public class CheatCommandPanelContainer : MonoBehaviour
{
	...
	public CheatCommandPanel CreatePanel(string name, string command)
	{
		...
		{
			GameObject gameObject = _baseObject.gameObject.transform.parent.gameObject.AddChild(_baseObject.gameObject);
			value = gameObject.GetComponent<CheatCommandPanel>();
	...
	public CheatCommandPanel GetPanel(string name)
	{
		...
		return (!_panels.TryGetValue(name, out value)) ? null : value;
	}
