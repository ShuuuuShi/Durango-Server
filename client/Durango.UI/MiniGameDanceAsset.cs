...
public class MiniGameDanceAsset : ResourceSingleton<MiniGameDanceAsset>
{
	...
	{
		List<DanceNoteData> list = null;
		return musicName switch
		{
			"minigame_01" => MiniGame01Notes, 
			"minigame_02" => MiniGame02Notes, 
			_ => throw new NotImplementedException("invalid music selected"), 
		};
	}
	...
	{
		target.Clear();
		List<DanceNoteData> source = MusicNameToList(playingMusicName);
		IOrderedEnumerable<DanceNoteData> orderedEnumerable = source.OrderByDescending((DanceNoteData elem) => elem.TimeKey);
		foreach (DanceNoteData item in orderedEnumerable)
		{
			target.Push(item);
		}
	}
	...
	public void AddNote(string name, float timeKey)
	{
		...
		{
			List<DanceNoteData> list = MusicNameToList(name);
			list.Add(new DanceNoteData
			{
