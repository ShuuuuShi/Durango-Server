...
public class KeyCodeDictionary : Dictionary<KeySet, InputCommand>
{
	...
	public InputCommand this[KeyCode a, Modifier mod = Modifier.None, Layer layer = Layer.Default, Trigger trigger = Trigger.Down]
	{
		...
		{
			InputCommand value;
			return TryGetValue(new KeySet(a, mod, layer), out value) ? value : InputCommand.None;
		}
		set
		{
	...
	{
		List<KeySet> value;
		return (!_reverseMap.TryGetValue(command, out value)) ? null : value;
	}
	...
	private bool CheckSafe(KeySet newKeySet, InputCommand command)
	{
		using (Enumerator enumerator = GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				{
					string text = $"KeyCode SafeAdd Failed due to Key Conflict Between Command [{command}] and Command [{current.Value}] ";
					return false;
				}
			}
		}
