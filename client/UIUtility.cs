...
public static class UIUtility
{
	...
	public static Vector2 WidgetsGridReposition<T>(IEnumerable<T> nodes, ListObjectPool spliter, Vector2 dir, Vector3 basePos, float breadth, Vector2 baseNodeSize, float rowMargin, float colMargin, out int rowItemCount, out float rowSize, out float colSize, float rowPivot = 0f, Vector2? pivot = null, bool instant = true)
	{
		...
		{
			UIWidget widget = GetWidget(node);
		...
		{
			UIWidget widget2 = GetWidget(node2);
			if (IsVisibleWidget(widget2))
			{
				widget2.gameObject.transform.localScale = Vector3.one * num2;
				int num7 = num6 % rowItemCount;
				int num8 = num6 / rowItemCount;
				Vector2 vector6 = widget2.pivotOffset - vector3;
				...
				if (instant)
				{
					widget2.transform.localPosition = vector7;
				}
				else
				{
					TweenPosition.Begin(widget2.gameObject, 0.2f, vector7);
				}
				num6++;
			}
		}
		if (spliter != null && spliter.BaseObject != null)
		{
			...
			UIWidget component = spliter.BaseObject.GetComponent<UIWidget>();
			float size3 = GetSize(component.localSize, vector2);
			int num11 = (int)((vector2.x != 0f) ? size3 : breadth);
			int num12 = (int)((vector2.x != 0f) ? breadth : size3);
			int i = 0;
			for (int count = spliter.Count; i < count; i++)
			{
				UIWidget component2 = spliter[i].GetComponent<UIWidget>();
				component2.width = num11;
				component2.height = num12;
				Vector2 vector8 = component2.pivotOffset - vector3;
	...
	public static int ColorComparison(Color color1, Color color2)
	{
		...
		return (seperatedHsvColor.x - seperatedHsvColor2.x != 0) ? (seperatedHsvColor.x - seperatedHsvColor2.x) : ((seperatedHsvColor.y - seperatedHsvColor2.y != 0) ? (seperatedHsvColor.y - seperatedHsvColor2.y) : ((seperatedHsvColor.z - seperatedHsvColor2.z != 0) ? (seperatedHsvColor.z - seperatedHsvColor2.z) : 0));
	}
	...
	{
		Vector2 vector = new Vector2(pivotX, pivotY) - widget.pivotOffset;
		return Vector3.Scale(vector, widget.localSize);
	}
	...
	public static Vector3 ToRootPosition(GameObject obj)
	{
		...
		return (!(parent == null)) ? ToRootPosition(parent.gameObject, obj.transform.localPosition) : Vector3.zero;
	}

	public static Vector3 ToRootPosition(GameObject parent, Vector3 pos)
	{
		GameObject root = NGUITools.GetRoot(parent);
		return root.transform.InverseTransformPoint(parent.transform.TransformPoint(pos));
	}
	...
	public static void DoPoolAsMethod<T, TU>(ref List<T> objs, IList<TU> dataList, Transform parent, Func<TU, T> selectPrefab, Action<T, TU, int> initalize) where T : Component
	{
		...
		{
			T val2 = objs[k];
			val2.transform.SetSiblingIndex(k);
		}
	}
	...
	private static void DoPooling<T, TU>(IList<T> objs, IList<TU> data, Transform parent, Func<TU, T> prefabSelector, Action<T, TU, int> initalize) where T : Component
	{
		...
		{
			TU arg = data[i];
			T original = prefabSelector(arg);
			if (i >= objs.Count)
			{
				T val = UnityEngine.Object.Instantiate(original, parent);
			...
			{
				T val2 = objs[i];
				if (val2.GetType() != original.GetType())
				{
					int num = -1;
					for (int num2 = objs.Count - 1; num2 >= i + 1; num2--)
					{
						T val3 = objs[num2];
						if (val3.GetType() == original.GetType())
						{
							num = num2;
							break;
						}
					...
					else
					{
						objs.Add(objs[i]);
						int index = i;
						T val4 = UnityEngine.Object.Instantiate(original, parent);
						objs[index] = val4.GetComponent<T>();
					}
				}
			}
			T arg2 = objs[i];
			arg2.gameObject.SetActive(value: true);
			initalize(arg2, data[i], i);
		}
	}
	...
	{
		UISpriteLabel uISpriteLabel = label as UISpriteLabel;
		return (!object.ReferenceEquals(uISpriteLabel, null)) ? uISpriteLabel.GetChildWidget(key) : null;
	}

	public static UIWidget GetChildSprite(UILabel label, int index)
	{
		UISpriteLabel uISpriteLabel = label as UISpriteLabel;
		return (!object.ReferenceEquals(uISpriteLabel, null)) ? uISpriteLabel.GetChildWidget(index) : null;
	}
