...
public class ChattingChannelOption : MonoBehaviour, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		_optionButton.GetComponent<SelectableWidget>().Clicked = OnClickOption;
		RefreshBar();
	}
	...
	private void LateUpdate()
	{
		...
		{
			ChattingGroupBase.ShowToggleButtonTooltip((!((CurrentConv != null) ? GameSystem<SocialSystem>.Instance().ChannelInfo.IsHidden(CurrentConv) : GameSystem<SocialSystem>.Instance().ChannelInfo.IsHidden(FilterType))) ? T._("채널의 대화 내용을 전체 채팅에 표시합니다.") : T._("채널의 대화 내용을 전체 채팅에 표시하지 않습니다."), _hideButton, Vector3.zero);
			_showHideTooltip = false;
		}
	}
}
