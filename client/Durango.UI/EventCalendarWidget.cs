...
public class EventCalendarWidget : CalendarWidget, IUIInitializable
{
	...
	{
		string arg = T._("[84847D]이벤트 기간 출석[-] {0}일", _calendar.CountDays(RewardState.Completed));
		int num = _calendar.CountDays(RewardState.Restorable);
		string arg2 = T._("[84847D]재출석 가능[-] [FFD85B]{0}일[-]", num);
		_bottomLabel.text = ((!UIManager.IsPortraitWidget(base.gameObject)) ? $"{arg} / {arg2}" : $"{arg}\n{arg2}");
		alpha = 1f;
		bool active = false;
		...
		for (int size = KUtility.GetSize(rewards); i < size; i++)
		{
			...
			CalendarNodeWidget component = nodes.GetNext().GetComponent<CalendarNodeWidget>();
	...
	private void AppendixRewardNodeClicked(CalenderReward calendarReward)
	{
		...
		{
			string voucherId = calendarReward.Voucher.Value.VoucherId;
			Voucher voucher = SingletonDict<string, Voucher>.Get(voucherId);
			if (!voucher.IsValid() || calendarReward.Voucher.Value.Count <= 0)
			{
			}
			text = voucher.Name;
