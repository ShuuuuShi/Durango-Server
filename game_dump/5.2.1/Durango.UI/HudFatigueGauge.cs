...
public class HudFatigueGauge : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		if (_fatigueLevelGuides != null)
		{
			_ = _fatigueLevelGuides.Length;
			_ = 3;
		}
		if (_arrowSprites != null)
		{
			_ = _arrowSprites.Length;
			_ = 4;
		}
	...
	private void OnFatigueLevelChanged(FatigueSystem.FatigueLevel level)
	{
		...
		foreach (TweenColor obj in upperTweenColors)
		{
			obj.enabled = false;
			obj.SetOnFinished((EventDelegate)null);
		}
