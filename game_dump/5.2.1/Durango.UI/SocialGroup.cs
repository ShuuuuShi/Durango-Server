...
public class SocialGroup : UIBase, INotificationable
{
	...
	private void Start()
	{
		...
		}, (Durango.Player.PlayerInfo info, int count) => (count <= 1) ? T._("{0} 님의 친구요청을 받았습니다", info.GetNameFreq(21, string.Empty)) : T._("{0} 님 외 {1}명 에게 친구요청을 받았습니다", info.GetNameFreq(21, string.Empty), count - 1), (Durango.Player.PlayerInfo info) => info.GetPortraitArgument(), majorAlarm: true, 1.8f, delegate
		{
			Open(MenuType.Add);
		});
		...
		}, (Durango.Player.PlayerInfo info, int count) => (count <= 1) ? T._("<em>{0}</em> 님과 친구가 되었습니다.", info.GetNameFreq(21, string.Empty)) : T._("<em>{0}</em> 님 외 {1}명과 친구가 되었습니다.", info.GetNameFreq(21, string.Empty), count - 1), (Durango.Player.PlayerInfo info) => info.GetPortraitArgument(), majorAlarm: true, 1.8f, null);
	...
	public void AcceptAllFriend()
	{
		...
		foreach (string entityId in receivedFriendRequests)
		{
