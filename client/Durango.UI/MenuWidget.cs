...
public class MenuWidget : SelectableWidget
{
	...
	{
		return (!MenuContainer.HasChildren(type)) ? GameSystem<MenuSystem>.Instance().IsRecentlyUnlocked(type) : MenuContainer.GetChildren(type).Any((MenuType c) => GameSystem<MenuSystem>.Instance().IsRecentlyUnlocked(c));
	}
