...
public class BuildSystem : GameSystem<BuildSystem>
{
	public struct GridResult
	{
		...
		public string BlueprintId => (Blueprint != null) ? Blueprint.Id : null;
	}
	...
	private IEnumerator CoOnOccupiedMsg(Occupied msg)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			artifact = Durango.Utils.Singleton<ArtifactManager>.Instance().Find(msg.EntityId);
			if (artifact != null)
			{
				break;
			}
	...
	{
		Vector2 tilePosition = msg.Tile.ToVector2() + msg.Size.ToVector2() * 0.5f;
	...
	{
		Vector2 tilePosition = msg.Tile.ToVector2() + msg.Size.ToVector2() * 0.5f;
		Vector3 pos = Util.TilePositionToClientPosition(tilePosition);
		int? floor = msg.Floor;
		if (floor.HasValue)
		{
	...
	{
		Artifact artifact = Durango.Utils.Singleton<ArtifactManager>.Instance().Find(msg.ArtifactId);
		if (!(artifact == null))
		{
	...
	private void OnPostTouched(InteractionMenuList menuList, InteractionObject target)
	{
		...
		if (GameManager.ClusterMode == Mode.Editable)
		{
			...
			if (artifactComponent == null && artifact.Models.HasPatternTex())
			{
