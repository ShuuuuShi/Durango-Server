...
public class LocalMoveOperator
{
	...
	public float LastSentYaw
	{
		get
		{
			return (!Mathf.Approximately(_lastSentYaw, -999f)) ? _lastSentYaw : Player.CurrentYaw;
		}
		private set
		{
			_lastSentYaw = value;
	...
	private bool ProcessYaw(LocalMoveNavigator.YawParam yawParam, ref float yaw, ref bool isSnapYaw)
	{
		...
		float num = Maths.DistanceAngDeg(yaw, targetYaw);
		if (num <= 1f || isSnapYaw)
		{
			yaw = targetYaw;
			isSnapYaw = true;
	...
	private static bool CancelMoveIfNotLoaded(Vector3 curPos, Vector3 delta)
	{
		...
		Biome tileBiome = Singleton<TerrainBase>.Instance().GetTileBiome(Util.ClientPositionToWorldPosition(vector));
		if (tileBiome == Biome.Invalid)
		{
			return true;
		}
	...
	private Vector3 ProcessCollisionWithSliding(Vector3 curPos, int floor, float height, LocalMoveNavigator.MoveParam moveParam, Vector3 delta, ref bool addMove, bool isRootMotionDelta)
	{
		...
		if (GameManager.IsPrologueMode)
		{
			...
			CollisionParam param2 = Collisions.CreateCollisionParam(curPos, delta);
			if (Collisions.TryCapsuleCast(param2, out var _) == RayCastResult.Pass)
			{
	...
	private bool CheckCollision(LocalMoveNavigator.MoveParam moveParam, CollisionParam param, bool collideOnOverlapped, out RaycastHit hit)
	{
		...
		{
			GameObject gameObject2 = hit.collider.gameObject;
			if (gameObject2.GetComponentInParent<VehicleBase>() == componentInChildren)
			{
				_completeMoveTarget = true;
			}
		}
