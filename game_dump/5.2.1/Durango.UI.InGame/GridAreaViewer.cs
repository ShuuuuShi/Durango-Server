...
public class GridAreaViewer : Singleton<GridAreaViewer>
{
	...
	private void Start()
	{
		...
		Transform obj = _overPanel.transform;
		obj.parent = Singleton<UIManager>.Instance().UIRoot.transform;
	...
	private void LateUpdate()
	{
		...
		{
			Vector2 vector = MainCamera.WorldToNGUIPos(Util.TilePositionToClientPosition(_areaStructs[_buttonIndexes[i]].CenterTile));
			_buttons[i].transform.localPosition = vector;
		}
	...
	public void FillGridTexture()
	{
		...
		{
			_areaStructs[i].Draw(geometry, _panel.alpha);
		}
