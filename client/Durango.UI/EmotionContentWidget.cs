...
public class EmotionContentWidget : UIWidget
{
	...
	public Vector2 UpdateLayoutItems()
	{
		...
		Vector3[] array = _contentWidget.localCorners;
		Vector2 vector = (Vector2)array[1] + new Vector2(_padding.x, 0f - _padding.y);
		int rowItemCount;
		float rowSize;
		float colSize;
		...
		return (rowItemCount != 0) ? (new Vector2(rowSize, colSize) + _padding * 2f) : (_prefab.localSize + _padding * 2f);
	}
