...
public class ChattingTabList : MonoBehaviour, IUIInitializable
{
	...
	public void Set(IList<KeyValuePair<ChatFilterType, uint>> tabs, IEnumerable<Conversation> rooms)
	{
		...
		for (int count = _tabs.Count; i < count; i++)
		{
			...
			_tabs[i].GetComponent<ChattingTabWidget>().Set(tabName, subText, pushOff, hided);
		}
		UpdateConversations();
	}
	...
	public void Select(ChatFilterType type)
	{
		...
		{
			_tabs[j].GetComponent<ChattingTabWidget>().Selected = j == num;
		}
		int k = 0;
		for (int count2 = _rooms.Count; k < count2; k++)
		{
			_rooms[k].GetComponent<ChattingRoomTabWidget>().Selected = false;
		}
	}
	...
	public void Select(string id)
	{
		...
		for (int count = _tabs.Count; j < count; j++)
		{
			_tabs[j].GetComponent<ChattingTabWidget>().Selected = false;
		}
		int k = 0;
		for (int count2 = _rooms.Count; k < count2; k++)
		{
			_rooms[k].GetComponent<ChattingRoomTabWidget>().Selected = k == num;
		}
