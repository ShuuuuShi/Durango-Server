...
public class PrologueManager : Durango.Utils.Singleton<PrologueManager>
{
	...
	[CompilerGenerated]
	private sealed class _003CCoAddStatusEffects_003Ed__88 : IEnumerator<object>, IDisposable, IEnumerator
	{
		private int _003C_003E1__state;

		private object _003C_003E2__current;

		public PrologueManager _003C_003E4__this;
		...
		[DebuggerHidden]
		public _003CCoAddStatusEffects_003Ed__88(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			...
			PrologueManager prologueManager = _003C_003E4__this;
			switch (num)
			{
			default:
				return false;
			case 0:
				...
				prologueManager.AddStatusEffect("hungry", T._("배고픔"), T._("배가 출출하다."), "icon_se_satietylow");
				_003C_003E2__current = new WaitForSeconds(0.5f);
				_003C_003E1__state = 1;
				return true;
			case 1:
				_003C_003E1__state = -1;
				prologueManager.AddStatusEffect("thirst", T._("갈증"), T._("목이 마르다."), "icon_se_waterlow");
				return false;
			}
		}
	...
	public PrologueNPCFloatingGroup NPCFloatingGroup
	{
		get
		{
			if (_npcFloatingGroup != null)
			{
				return _npcFloatingGroup;
			}
			return _npcFloatingGroup = UIManager.FindScript<PrologueNPCFloatingGroup>();
		}
	}

	private PrologueCharacterSelectGroupBase PrologueCharacterSelectUI
	{
		get
		{
			...
			return _prologueCharacterSelectUI = UIManager.FindScript<PrologueCharacterSelectGroupBase>();
		}
	}
	...
	public AnimationClip[] GetPlayerClips(bool male)
	{
		if (male)
		{
			return _playerClipsMale;
		}
		return _playerClipsFemale;
	}
	...
	private IEnumerator CoAddStatusEffects()
	{
		...
		return new _003CCoAddStatusEffects_003Ed__88(0)
		{
	...
	private void OnChangeCostumeSelectCharacter()
	{
		...
		if ((bool)targetComponent)
		{
			string assetBundlePathBase = ResourceSingleton<PlayerCostumeTable>.Instance().GetRandom(PlayerCostumeTable.Category.Hair, targetComponent.IsMale).AssetBundlePathBase;
	...
	private void OnDamaged(Damaged msg)
	{
		...
		{
			if (msg.Damage.Result == DamageResult.Dodged && GameSystem<PrologueToDoListSystem>.Instance().FindToDo("dodge") != null)
			{
		...
		bool isDead = false;
		ToDoBase toDoBase = GameSystem<PrologueToDoListSystem>.Instance().FindToDo("active_action");
		if (toDoBase != null)
		{
			GameSystem<PrologueToDoListSystem>.Instance().SetProgress("active_action", toDoBase.CurrentProgress + 1);
			if (toDoBase.CurrentProgress >= toDoBase.TargetProgress)
			{
				GameSystem<PrologueToDoListSystem>.Instance().SetCompleted("active_action", completed: true);
				PlayTrexCutScene();
				isDead = true;
			}
			else if (toDoBase.CurrentProgress == 1)
			{
				DelayedCall(delegate
				{
	...
	public void PrologueFinished()
	{
		...
		GetPrologueCamera().enabled = false;
		DelayedCall(delegate
		{
			FullScreenMovieGroupBase.Play(Platform.Instance.PrologueMovieUrl, once: true, FullScreenMovie_Finished);
		}, 1f);
	...
	private void RequestCreatePlayer()
	{
		...
		PlayerDisplay data = default(PlayerDisplay);
		data.hair = display.Hair;
		data.body_color = display.BodyColor;
		data.head_color = display.HeadColor;
		data.skin_color = display.SkinColor;
		data.hair_color = display.HairColor;
		data.eye_color = display.EyeColor;
		data.lip_color = display.LipColor;
		data.portrait = display.Portrait;
		data.portrait_bg = display.PortraitBg;
		data.portrait_bg_color = display.PortraitBgColor;
		data.beard = display.Beard;
		data.voice_type = display.VoiceType;
		data.body_size = display.BodySize;
