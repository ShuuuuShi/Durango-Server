...
public static class LocalizeSystem
{
	...
	private static string _Get(string key)
	{
		...
		string value;
		return (!_keyValueDict.TryGetValue(key, out value)) ? key : _Get(value);
	}
	...
	public static bool Has(string key)
	{
		...
		return key[0] == '#' && _keyValueDict.ContainsKey(key);
	}
	...
	public static void LoadLegacyCatalog(Dictionary<string, string> result)
	{
		...
		try
		{
			...
			for (int num = array.Length; i < num; i++)
			{
				...
				arg = textAsset.name;
				Dictionary<string, string> dictionary = Json.Read<Dictionary<string, string>>(textAsset.text);
				if (dictionary == null)
				{
					continue;
				}
