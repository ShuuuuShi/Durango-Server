...
public class NavigateGroup : UIBase
{
	...
	private void SetCharacterPoint([CanBeNull] CharacterBehavior character, bool rescue)
	{
		if (!(character == null))
		{
			...
			{
				character.Revived += RemoveCharacterPoint;
				bool num = character is AnimalBehavior;
				string icon = ((!num) ? "todo_icon_friend_dead" : "todo_icon_animal_dead");
				Color value = ((!num) ? PresetColor.UIDarkRed : PresetColor.UIWhite);
