...
public class MarketSearchWidget : MonoBehaviour
{
	...
	private void Init()
	{
		...
		{
			string value = _textInput.value.Trim();
	...
	public void Open([NotNull] SearchOption option)
	{
		...
		if (transform != null)
		{
			...
			component.text = ((_searchOption.MainCategory == null) ? T._("속성") : T._("결과 내 검색"));
		}
		ShowSearchHistory();
	...
	private void ShowSearchHistory()
	{
		...
		{
			SearchInfoItemNode component = _searchHistoryItems.GetNext().GetComponent<SearchInfoItemNode>();
			component.Set(item, OnClickSearchHistory);
		}
	...
	private void OnClickPriceWidget()
	{
		...
		bool flag = current == _minPriceWidget;
		PriceRangePredicate price = _searchOption.Price;
		long? num = ((!flag) ? price.Max : price.Min);
		numberInputPopup.Show((!num.HasValue) ? 0 : num.Value, Currency.TStone, T._("가격 설정"), OnPriceInputConfirmed);
