...
public class Ride
{
	...
	{
		string.IsNullOrEmpty(_defaultMotionSetName);
		if (_motionSets == null)
		{
			return;
		}
		...
		{
			if (!string.IsNullOrEmpty(motionSet.Value.DisMount) && !string.IsNullOrEmpty(motionSet.Value.Mount) && !string.IsNullOrEmpty(motionSet.Value.StandMount))
			{
				string.IsNullOrEmpty(motionSet.Value.RunMount);
			}
		}
		if (_motions != null)
		{
			...
			{
				_motionSets.ContainsKey(motion.Value);
			}
		}
