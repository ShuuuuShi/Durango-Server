...
public class FriendListPage : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		foreach (FriendListColumnHeaderWidget obj in headers)
		{
			obj.Clicked = (Action<string>)Delegate.Combine(obj.Clicked, new Action<string>(OnListSort));
		}
	...
	private IEnumerable<string> GetSortedList(Durango.Player.PlayerInfo[] infos, string sortedKey, SortableColumnWidget<string>.State state)
	{
		...
		if (state != 0)
		{
			...
			else if (!(sortedKey == _connectHeader.Value))
			{
				_ = sortedKey == _relationHeader.Value;
			}
		}
