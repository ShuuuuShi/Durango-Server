...
public abstract class CharacterBehavior : MonoBehaviour
{
	...
	{
		bool flag = _survivalGauges == null;
		if (gauges != null)
		{
		...
		Life = life;
		if (flag && this.SurvivalGaugeInitialized != null)
		{
	...
	{
		return (!GameManager.IsPrologueMode) ? Connections.Frontend.GetBufferedServerTime() : ((double)Time.time);
	}
