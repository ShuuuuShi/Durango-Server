...
public class VoxelStatueEditorGroup : UIBase
{
	...
	private void OnConfirmed()
	{
		...
		UIManager.FindScript<BuildGridGroup>().Open(blueprint, delegate
		{
			if (voxel != null)
			{
				...
				int num = Mathf.Max(1, _tileSize);
				float num2 = (float)(num * 200) / (float)Mathf.Max(voxel.Size.X, voxel.Size.Z);
	...
	protected override void DefaultUri()
	{
		...
		voxelStatue.Bytes = array;
		voxelStatue.Size = new Size3(num, num, num);
		VoxelStatue voxelStatue2 = voxelStatue;
		voxelStatue2.Colors = ColorTableLoader.GetAll($"color_board_L{1:00}.raw");
		Open(voxelStatue2);
	}
}
