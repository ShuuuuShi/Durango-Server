...
public class TextBuilder : IDisposable
{
	...
	public class TextTokens
	{
		...
		public bool IsEmpty()
		{
			return Tokens.Count == 0 && Options.Count == 0;
		}
	...
	public struct TextToken
	{
		...
		{
			return Type == TokenType.Space || Character == 45;
		}
	}
		...
		if (!flag && useEllipsis && full && result.Count > 0 && tokens.Options.Count > 0)
		{
			...
			if (num17 < (float)Width)
			{
				...
				if (num18 != -1)
				{
					...
					TextToken textToken = default(TextToken);
					textToken.Type = TokenType.Character;
					textToken.Character = 46;
					textToken.PrevCharacter = 46;
					textToken.Size = new Vector2(glyph.advance, lastOption.FontSize);
					TextToken token3 = textToken;
					for (int k = 0; k < 3; k++)
					{
						result.Add(token3);
					}
	...
	{
		TextBuilder textBuilder = ((Pool.Count <= 0) ? new TextBuilder() : Pool.Pop());
		textBuilder.Reset();
		return textBuilder;
	}
