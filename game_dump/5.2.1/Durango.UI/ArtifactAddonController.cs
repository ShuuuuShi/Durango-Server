...
public class ArtifactAddonController : MonoBehaviour
{
	...
	private void Update()
	{
		if (IsSelectAddon)
		{
			...
			if (preview != null)
			{
				...
				if (SelectIndex != -1)
				{
					float t = (Mathf.Cos(Time.time % 1f * (float)Math.PI * 2f) + 1f) * 0.5f;
	...
	public void UnselectAnimation(Vector3 uiPos)
	{
		if (!(Artifact == null))
		{
			...
			Vector3 vector = MainCamera.NGUIPosToWorldPos(_unselectEndUIPos) - unselectBeginWorldPos;
			_unselectBeginTime = Time.time;
			_unselectEndTime = _unselectBeginTime + vector.magnitude / 3000f;
		}
	}
	...
	private void UnselectAnimationUpdate()
	{
		if (_isUnselectAnimation && !(Artifact == null))
		{
			...
			float num2 = (time - _unselectBeginTime) / num;
			Vector3 unselectBeginWorldPos = _unselectBeginWorldPos;
			...
			Vector3 position = Vector3.Lerp(unselectBeginWorldPos, vector, num2) - Artifact.transform.position;
			float num3 = num2 * 2f - 1f;
			num3 = 1f - num3 * num3;
			position += vector3 * num3 * vector2.magnitude * 0.1f;
	...
	private void UpdateAddonPreview(ModularAddon addon)
	{
		...
		bool num = addon.IsEmptyDoor();
		string modelKey = ((!num) ? addon.ModelKey : _modular.GetModel("wall"));
		_addonPreview = preview.Load("preview_addon", modelKey, null, "preview").SetMaterial(previewMaterial).SetActive(active: false);
		string modelKey2 = ((!num) ? _modular.GetModel("wall") : null);
		string modelPostfix = ((!num) ? addon.GetWallPostfix() : null);
	...
	private void AttachPreveiwToPoint()
	{
		if (!(Artifact == null))
		{
			Vector3 position = MainCamera.ScreenPosToWorldPos(UICamera.currentTouch.pos) - Artifact.transform.position;
			_selectedPlaceWalls.Clear();
	...
	private bool GetTouchedArtifactWall(out Point2 tile, out Direction dir)
	{
		...
		for (int num4 = array.Length; i < num4; i++)
		{
			...
			float num5 = Mathf.Abs(vector2.y - Mathf.Lerp(num, num2, 0.5f));
			if (!(num5 < num3))
			{
				continue;
			}
