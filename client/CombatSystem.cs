...
public class CombatSystem : GameSystem<CombatSystem>
{
	...
	private void Awake()
	{
		...
		{
			bool flag = damaged.AttackerId == PlayerBehavior.LocalPlayer.EntityId;
			bool flag2 = damaged.VictimId == PlayerBehavior.LocalPlayer.EntityId;
			if (flag || flag2)
			{
	...
	public void UseBattleAction(string id, DamageableEntity target, bool targetSelect)
	{
		...
		if (KUtility.GetSize(battleAction.Data.AttackInfo) > 0 && (target == null || !target.IsAlive))
		{
			...
			foreach (DamageableEntity enemy in DamageableEntities.GetEnemies())
			{
				...
				{
					Vector3 currentPosition2 = enemy.GetCurrentPosition();
					float sqrMagnitude = (currentPosition2 - currentPosition).sqrMagnitude;
	...
	private void OnTensionChanged(TensionChanged msg, PacketHeader header)
	{
		...
		if (!(animal == null))
		{
			...
			Dictionary<string, string> renderEffects = msg.RenderEffects;
			if (renderEffects.TryGetValue("color", out var value))
			{
	...
	{
		return (GameManager.Region.Role() == Role.Outpost || GameManager.Region.IsPvpIsland()) && OptionSystem.GetBattlePvPEnabled();
	}
	...
	public static bool IsHostilePlayer([NotNull] PlayerBehavior otherPlayer)
	{
		...
		return IsPvPEnabled() && !GameSystem<PartySystem>.Instance().IsInParty(otherPlayer.EntityId) && !ClanSystem.IsMyClanOrAlliance(otherPlayer);
	}
