...
public class PlayerSlotList : UIWidget, IUIInitializable
{
	...
	public void Set(List<PlayerInfo> players, int emptyCount, int playerSlotCount, int lockedCount, bool exceeded)
	{
		...
		{
			nodes.GetOrAdd(num).GetComponent<PlayerSlotNode>().Set(PlayerSlotNode.SlotType.HasPlayer, players[num]);
			num++;
		}
		for (int j = 0; j < emptyCount; j++)
		{
			nodes.GetOrAdd(num).GetComponent<PlayerSlotNode>().Set(PlayerSlotNode.SlotType.Empty, null);
			num++;
		}
		for (int k = 0; k < lockedCount; k++)
		{
			nodes.GetOrAdd(num).GetComponent<PlayerSlotNode>().Set(PlayerSlotNode.SlotType.Locked, null);
			num++;
		}
		nodes.Set(num);
	...
	public void Select(string entityId)
	{
		...
		{
			PlayerSlotNode component = _scrollView.Nodes[i].GetComponent<PlayerSlotNode>();
