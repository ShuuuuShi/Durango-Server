...
public class KGridScrollView : NodesScrollView
{
	...
	private Vector2 GridSize
	{
		get
		{
			if (base.Vector.x == 0f)
			{
				return new Vector2(_rowSize, _colSize);
			}
			return new Vector2(_colSize, _rowSize);
		}
	}
	...
	protected override int CalcNodeIndex(float offset)
	{
		if (_rowItemCount > 0)
		{
			return Mathf.Max(0, Mathf.FloorToInt(offset / (_colSize + (float)base.Margin))) * _rowItemCount;
		}
		return 0;
	}
	...
	protected override int ToIntOffset(int currentIndex, int sign)
	{
		if (_rowItemCount > 0)
		{
			...
			return Mathf.Max(num, num + sign) * _rowItemCount;
		}
