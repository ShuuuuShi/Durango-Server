...
public class MusicSheetEditor : MonoBehaviour
{
	...
	public void Init()
	{
		...
		uIEventListener.onClick = (UIEventListener.VoidDelegate)Delegate.Combine(uIEventListener.onClick, (UIEventListener.VoidDelegate)delegate
		{
			...
			textInputPopup.Show(delegate(string text)
			{
	...
	private void FinishMakeNote(Note note)
	{
		...
		uint soundId = _makingNotes[num].SoundId;
		MusicManager.StopMidi(soundId);
		...
		int num6 = ((num3 != -1) ? num3 : (_music.Notes.Count - 1));
		for (int num7 = num6 - 1; num7 > num4; num7--)
		{
			if (_music.Notes[num7].Midi == note.Midi)
			{
				RemoveAtNote(num7);
			}
		}
	...
	private void OnTempoEdit()
	{
		...
		stringSelector.Set(items.Select((int? value) => (!value.HasValue) ? T._("직접 입력") : value.Value.ToString()), delegate(int index)
		{
			int? num = items[index];
			if (num.HasValue)
			{
			...
			{
				TextInputPopup textInputPopup = UIManager.Popup.Tooltip<TextInputPopup>();
				textInputPopup.Show(delegate(string value)
				{
					SetBpm(value.ToInt());
	...
	private void SetGuideLine(float timer, bool keyboardSync, bool scrollSync, bool selectNote)
	{
		...
		_noteSelector.Hide();
		using Dictionary<int, Note>.Enumerator enumerator3 = _currentTimerNotes.GetEnumerator();
		if (enumerator3.MoveNext())
		{
			SelectNote(enumerator3.Current.Value);
		}
	}
