...
public class ToDoListGroupBase : UIBase
{
	...
	private void UpdateNodeTween()
	{
		...
		{
			float tweenRatio = toDoListSystem.GetCollection(i).TweenRatio;
			ToDoIconNode toDoIconNode = nodes.Get<ToDoIconNode>(i);
			if (tweenRatio < 1f)
			{
				bool num = toDoIconNode.Alpha > 0f;
				toDoIconNode.Alpha = tweenRatio;
				_lastNodeAlpha = tweenRatio;
				if (num != toDoIconNode.Alpha > 0f)
				{
					flag = true;
				}
			}
	...
	private void OnSeasonUpdated()
	{
		...
		{
			nodes.Get<ToDoIconNode>(i).RereshSeasonInfo();
		}
	}
	...
	{
		if (_scrollView.Nodes.Count == 0 && !_noticeButton.activeSelf)
		{
			return !_webEventButton.activeSelf;
		}
		return false;
	}
	...
	private static void OnCollectionSelected(ToDoCollection collection, bool selected)
	{
		...
		foreach (string key in navigationKey)
		{
			if (point.Has(key))
			{
	...
	private void SelectNodeByCollection(ToDoCollection collection)
	{
		...
		for (int i = 0; i < nodes.Count; i++)
		{
			if (nodes.Get<ToDoIconNode>(i).Collection == collection)
			{
				SelectNode(i);
				break;
			}
