...
public class ItemContextPerformance : ItemContextBase
{
	...
	private void UpdateLayout()
	{
		...
		{
			_simpleValues[i].UpdateLayout(_body.width);
		}
		...
		for (int k = 0; k < _widgets.Length; k++)
		{
			if (UIUtility.IsVisibleWidget(_widgets[k]))
			{
				if (num5 > 0)
				{
					_separators.GetNext().transform.localPosition = _widgets[k].GetPosition(0.5f, 1f) + Vector3.up * 26f * 0.5f;
				}
				num5++;
			}
		}
	...
	{
		_simpleValues.GetNext().Set(key, value);
	}
	...
	private void FillData(Performance data)
	{
		...
		foreach (KeyValuePair<string, float> num in data.Nums)
		{
			...
			else
			{
				...
				if (value.Type == PerformanceVisibleType.Ratio)
				{
	...
	private bool CheckEquipablity(ItemData item, Prototype prototype)
	{
		...
		return GameSystem<StatisticsSystem>.Instance().Level >= num;
	}
	...
	{
		return LocalizeSystem.Get("#attribute_value_" + attrValue);
	}
	...
	private void FillEffectOn([NotNull] ItemData item)
	{
		...
		foreach (Pair<string, int> statusEffect in item.GetStatusEffects())
		{
			...
			string iconColor = statusEffectTemplate.IconColor;
			Color color;
			string spriteName;
	...
	private void FillData(ItemColor colors)
	{
		...
		for (int i = 0; i < colors.Count; i++)
		{
			...
			{
				_colors.GetNext().transform.Find("upper").GetComponent<UISprite>().color = color;
			}
		}
	}
	...
	private void FillSkill(Messages.Pet pet)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (petActiveSkill2 != null)
			{
				...
				next.transform.Find("Icon").GetComponent<UISprite>().spriteName = petActiveSkill2.Icon;
				next.transform.Find("Name").GetComponent<UILabel>().text = petActiveSkill2.Name;
			}
		}
		_skillsPool.EndLoad();
	}
