...
public class FindImmovable : ToDoBase
{
	...
	{
		if (_types.Contains(artifact.EntityType))
		{
			Vector3 vector = Durango.Terrain.Util.TilePositionToClientPosition(artifact.WorldTile);
			if ((vector - PlayerBehavior.LocalPlayer.CurrentPosition).sqrMagnitude < _radius * _radius)
			{
				CallComplete();
			}
		}
