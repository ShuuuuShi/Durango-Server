...
public class ResistanceInfo : MonoBehaviour
{
	...
	public void Set(Biome biome, string typeName, string iconName, int level, int currentExp, int totalExp, float expRate, bool isHighlighted)
	{
		...
		string text = "<em>" + typeName + "</em>";
		string text2 = T._("<em>{0} 기후</em>에서 피로도 소모량이 감소합니다. <em>{0} 기후</em>에서만 <em>경험치</em>를 얻을 수 있습니다.", LocalizeUtil.Get(biome));
		string text3 = T._("다음 레벨까지 필요한 경험치");
		string text4 = T._("현재 경험치 획득 비율");
		string text5 = T._("경험치 <em>{0:P0}</em> 획득중", expRate);
		string text6 = string.Format(T.Culture, "({0:N0}/{1:N0})", currentExp, totalExp);
		string text7 = text2 + "\n\n[size=20][D4CEBEBE]" + text3 + "\n" + text6 + "[-][/size]\n\n" + text4 + "\n" + text5;
		string text8 = ((!isHighlighted) ? typeName : ("<em>" + typeName + "</em>"));
		_labels.SetKey(text8 + "  <help>title=" + text + ",comment=" + text7 + ",width=340,sign=1,direction=horizontal</help>");
