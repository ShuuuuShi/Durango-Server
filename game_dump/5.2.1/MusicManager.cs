...
public class MusicManager : Singleton<MusicManager>
{
	...
	public static void PlayMusic(MusicId musicId, Messages.Music music, ItemData item)
	{
		...
		if (performanceData.HasValue && performanceData.Value.Strs != null && !string.IsNullOrEmpty(performanceData.Value.Strs.Get("timbre")))
		{
			string id = item.Id;
	...
	private void SaveMusicsToLocal()
	{
		...
		foreach (KeyValuePair<MusicId, Messages.Music> music in _musics)
		{
			...
			using FileStream fileStream = AppData.OpenFile(text + "\\" + text2 + ".mid");
