...
public class MusicNoteEditor : MonoBehaviour
{
	...
	{
		int num = (int)((MainCamera.ScreenPosToNGUIPos(UICamera.lastEventPosition).x - _dragTouchPos.x) / 30f) * MusicSheetEditor.TickCountPerTerm;
		ChangeBegin(_dragBeginTick + num);
		ChangeEnd(_dragEndTick + num);
		UpdatePosition();
	}
	...
	{
		int num = (int)((MainCamera.ScreenPosToNGUIPos(UICamera.lastEventPosition).x - _leftButtonTouchPos.x) / 30f) * MusicSheetEditor.TickCountPerTerm;
		ChangeBegin(_leftButtonTouchedTick + num);
		UpdatePosition();
	}
	...
	{
		int num = (int)((MainCamera.ScreenPosToNGUIPos(UICamera.lastEventPosition).x - _rightButtonTouchPos.x) / 30f) * MusicSheetEditor.TickCountPerTerm;
		ChangeEnd(_rightButtonTouchedTick + num);
		UpdatePosition();
	}
