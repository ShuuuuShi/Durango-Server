...
public class AccessRightsSettingPopup : TooltipBase
{
	...
	private void SelectFriendType(Shared.Player.FriendType friendType)
	{
		...
		{
			SelectableWidget component = _friendTypeList.Nodes[i].GetComponent<SelectableWidget>();
	...
	{
		return (nodeIndex == 0) ? Shared.Player.FriendType.BestFriend : Shared.Player.FriendType.JustFriend;
	}
	...
	protected override void FillData()
	{
		EstateSystem.GetEstateLicenses(delegate(EstateLicenses licenses)
		{
			...
			for (int num = 1; num >= 0; num--)
			{
				...
				FriendTypeWidget component = _friendTypeList.Nodes[index].GetComponent<FriendTypeWidget>();
				component.Set(title, description, delegate
				{
					Hide();
