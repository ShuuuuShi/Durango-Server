...
public abstract class RoutesViewerBackground : CustomFillSprite
{
	...
	protected void DrawGrids(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		for (float num4 = GridOffset.x; num4 < num2 - 1f; num4 += GridSize)
		{
			if (num4 > 0f)
			{
				DrawSprite(verts, uvs, cols, new DrawParam
				{
					Sprite = ResourceSingleton<UISpriteManager>.Instance().GetSprite(WhiteSprite),
					Position = InnerRect.position + new Vector2(num4, -22f),
	...
	{
		int num = Mathf.CeilToInt(OutterRect.width * OutterRect.height * _waveDensity) / WaveSpriteSet.Length;
		for (int i = 0; i < WaveSpriteSet.Length; i++)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				float num2 = _waveMinScale + (float)_random.NextDouble() * (_waveMaxScale - _waveMinScale);
				DrawSprite(verts, uvs, cols, new DrawParam
				{
					Sprite = sprite,
					Position = vector,
					Pivot = new Vector2(0.5f, 0.5f),
					Size = new Vector2((float)size.x * num2, (float)size.y * num2),
					Rect = new Rect(0f, 0f, 1f, 1f),
					Color = _waveColor
				});
			}
	...
	{
		int num = Mathf.CeilToInt(OutterRect.width * OutterRect.height * _scatterDensity) / ScatterSpriteSet.Length;
		for (int i = 0; i < ScatterSpriteSet.Length; i++)
		{
			...
			{
				Vector3 vector = new Vector3(InnerRect.xMin + InnerRect.width * (float)_random.NextDouble(), InnerRect.yMin + InnerRect.height * (float)_random.NextDouble());
				float num2 = _scatterMinScale + (float)_random.NextDouble() * (_scatterMaxScale - _scatterMinScale);
