...
public class EntryTodoCollection : Durango.Logic.PlayGuide.ToDoCollection
{
	...
	public override SyncString GetMessage()
	{
		return new SyncString(delegate(out string text, out float period)
		{
			...
			string text2 = T._("난투섬 입장을 위한\n대기열에 등록했습니다.");
			string text3 = T._("대기 인원 <em>{0}</em>[FFFFFF7F]/{1}[-]", entreeInfo.QueueCount, warpRushEntryCount);
			string text4 = string.Format("{0} {1}", T._("남은 시간"), (!flag) ? " - " : TimedeltaFormatter.Format(num));
			text = "\n[d4cebe][size=20]" + text2 + "[/size][-]<br>9</br>[c][size=24]" + text3 + "[/size][/c]<br>5</br>[ffffff7f]" + text4 + "[-]";
