...
public class PlayerInfoManager : Singleton<PlayerInfoManager>
{
	...
	public void SearchPlayerInfos(string searchKey, string searchFreq, Action<FoundPlayerInfo[]> response)
	{
		...
		{
			string arg = WWW.EscapeURL(searchKey.Trim());
			string arg2 = ((searchFreq != null) ? WWW.EscapeURL(searchFreq.Trim()) : string.Empty);
			string url = $"{GameManager.GatewayUrl}/players?name={arg}&freq={arg2}";
			Http.RequestYml(url, delegate(FoundPlayersJson json)
			{
