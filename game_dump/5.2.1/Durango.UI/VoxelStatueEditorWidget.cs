...
public class VoxelStatueEditorWidget : UIWidget
{
	...
	private void SelectColor(int index)
	{
		...
		{
			_colorIndexSelector.Nodes[i].GetComponent<SelectableWidget>().Selected = i == index;
		}
		if (index != -1)
		{
	...
	{
		obj.transform.Find("Sprite").GetComponent<UISprite>().color = col;
	}
	...
	private void UpdateModel()
	{
		...
		if (!(_modelObject != null))
		{
			...
			_modelObject.AddComponent<MeshFilter>().sharedMesh = _mesh;
			_modelObject.AddComponent<MeshRenderer>().sharedMaterial = new Material(_voxelShader)
			{
				mainTexture = _voxelTexture
			};
			GameObject gameObject = _modelObject.AddChild();
			MeshFilter meshFilter = gameObject.AddComponent<MeshFilter>();
			MeshRenderer meshRenderer = gameObject.AddComponent<MeshRenderer>();
			Material material = new Material(Shader.Find("Durango/Unlit/Diffuse"));
			BlendUtil.SetBlendMode(material, BlendMode.Transparent);
			material.SetFloat("_Culling", 0f);
