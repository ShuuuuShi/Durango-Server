...
public class AreaOfEffectVisualizer : Singleton<AreaOfEffectVisualizer>
{
	...
	public static int ShowVehicleProjectileFired(VehicleProjectileFired fired)
	{
		...
		bool flag = localPlayer != null && localPlayer.Driver.Vehicle != null && localPlayer.Driver.Vehicle.EntityId == fired.EntityId;
		Vector3 position = fired.TargetPosition.ToClientPosition();
		double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
		float time = Time.time;
		float showAt = ((!flag) ? (time - (float)(predictedServerTime - fired.WarnSince)) : time);
	...
	public static int ShowAttackAlerted(Type type, AttackAlerted alert)
	{
		...
		bool flag = alert.DamageType == DamageType.Melee || alert.DamageType == DamageType.CircularArea || alert.DamageType == DamageType.Ranged;
		bool flag2 = alert.DamageType == DamageType.RectangularArea;
		bool flag3 = alert.DamageType == DamageType.Melee || alert.DamageType == DamageType.CircularArea;
		float num4 = ((!alert.Yaw.HasValue) ? 0f : alert.Yaw.Value);
		if (flag && alert.Radius.HasValue && alert.Radius.Value > 0f)
		{
			float startAngle;
			float endAngle;
			if (flag3 && alert.Angles.HasValue)
			{
				startAngle = num4 + alert.Angles.Value.x;
				endAngle = num4 + alert.Angles.Value.y;
			}
			else
			{
				startAngle = 0f;
		...
		if (flag2 && alert.RectSizeHalves.HasValue)
		{
			Vector2 vector2 = alert.RectSizeHalves.Value * 2f;
			if (vector2.x > 0f && vector2.y > 0f)
			{
				return ShowRect(type, vector, vector2.x, vector2.y, num4, num2, num3, num2, num3);
			}
		}
		return -1;
	}
