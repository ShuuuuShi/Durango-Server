...
public class FactionGroup : UIBase, INotificationable
{
	...
	private void OnChangeFactionPoint(Durango.Logic.Faction.Faction faction, int diff)
	{
		...
		{
			Talks talk = array[i];
			if (KUtility.GetSize(talk.List) == 0)
			{
				continue;
			}
			int num = faction.Point - diff;
			if (num >= talk.FriendshipPoint || talk.FriendshipPoint > faction.Point)
			{
				continue;
			}
			if (talk.NoticeType == TalkType.Modal)
			{
				...
				dialogueGroupBase.AddFactionTalks(talk);
				continue;
			}
	...
	private static void OnSupportRewardsAccepted(AcceptedSupportRewards rewards)
	{
		...
		receiveRewardsPopup.ShowAcceptedSupportRewards(rewards);
	}
