...
public class MapContext : Singleton<MapContext>
{
	...
	private void RecalcMapTextureSize()
	{
		...
		else
		{
			...
			float num3 = (uvRect2.width = 300f / (float)MapNGUISize);
			float height = num3;
		...
		for (int i = 0; i < _dirtyChunk.Width; i++)
		{
			for (int j = 0; j < _dirtyChunk.Height; j++)
			{
				...
				for (int k = 0; k < 16; k++)
				{
					...
					{
						int num3 = num + k;
						int num4 = (num2 + l) * MapSize + num3;
						byte biome = (byte)((num4 >= _wholeBiomes.Length) ? 12 : _wholeBiomes[num4]);
						Point2 tile = new Point2(k, l) + new Point2(i * 16, j * 16);
	...
	public Color GetTileColor(byte biome, Point2 tile)
	{
		...
		case OwnerType.ClanWarphole:
			...
			{
				color = ((!(Connections.Frontend.GetPredictedServerTime() <= estateInfo.License.ProtectedUntil.Value)) ? PresetColor.EnemyEstateArea : ((!flag) ? PresetColor.ClanTerritory : PresetColor.PlayerClanTerritory));
			}
			break;
		}
	...
	public Color GetBiomeColor(byte biome)
	{
		...
		if (_biomeColors.Length <= biome)
		{
			return Color.black;
		}
		return _biomeColors[biome];
	}
	...
	public void ZoomOut(bool toPlayer)
	{
		...
		if (toPlayer)
		{
			tilePosition = Durango.Terrain.Util.ClientPositionToTilePosition(PlayerBehavior.LocalPlayer.CurrentPosition);
		}
		else
		{
			int num = TerrainMeta.TileCount / 2;
