...
public class QuestGroup : UIBase, INotificationable
{
	...
	private void Start()
	{
		...
		_questSucceedAlarm = new StackableAlarm<string, KeyValuePair<string, QuestYml>>("QuestSucceed", (KeyValuePair<string, QuestYml> pair) => pair.Key, (KeyValuePair<string, QuestYml> pair, int count) => (count > 1) ? T._("<em>{0}</em> 외 {1}개 퀘스트 완료", pair.Value.Subject, count - 1) : T._("<em>{0}</em> 퀘스트 완료", pair.Value.Subject), "alarm_quest", majorAlarm: true, 1.8f, delegate(KeyValuePair<string, QuestYml> pair)
		{
			if (!base.IsOpened)
			{
