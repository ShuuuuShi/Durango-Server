...
public class ParticleManager : Singleton<ParticleManager>
{
	public class ParticleHelper : MonoBehaviour
	{
		...
		public void Initialize(Transform pool)
		{
			...
			if (_autoDestruct != null)
			{
				...
				{
					componentsInChildren[i].OnSetReplayState();
				}
			}
		}
		...
		public void Play()
		{
			...
			else if (_autoDestruct != null)
			{
				...
				for (int i = 0; i < componentsInChildren.Length; i++)
				{
					componentsInChildren[i].OnResetReplayStage(bClearOldParticle: true);
				}
			}
		}
	...
	private class CameraTransformProvider : NcBillboard.ICameraTransformProvider
	{
		public Transform GetTransform()
		{
			...
			{
				return Singleton<MainCamera>.Instance().transform;
			}
			return null;
		}
	}
	...
	public static void Cache(string assetPath)
	{
		...
		{
			Singleton<ParticleManager>.Instance().CacheParticle(assetPath);
		}
	}
	...
	public static int Emit(GameObject obj, string assetPath, string bone = null, bool follow = true)
	{
		...
		if (follow)
		{
			return EmitFollow(assetPath, Vector3.zero, Quaternion.identity, transform);
		}
		return Emit(assetPath, transform.position, Quaternion.identity);
	}
	...
	{
		GetOrCreateParticlePool(assetPath).MaxCount = count;
	}
	...
	private GameObject RequestParticleInternal(ParticleEmitParam param, ParticlePool pool, int id)
	{
		...
		{
			ParticleHelper particleHelper = UnityEngine.Object.Instantiate(pool.Prefab, pool.Parent).AddComponent<ParticleHelper>();
