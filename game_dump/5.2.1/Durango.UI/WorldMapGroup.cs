...
public class WorldMapGroup : UIBase
{
	...
	private string GetPurchaseMapButtonText()
	{
		...
		if (openMap.HasVoucher() || flag)
		{
			...
			if (flag)
			{
				...
				text = T._("무료 ({0}/{1})", InventorySystem.Wallet.GetVoucherCount(voucherId), SingletonDict<string, Voucher>.Instance.Get(voucherId).CountMax);
			}
			else
			{
				text = "[icon=" + openMap.GetVoucher().VoucherId + "]  1";
			}
			return ButtonType.PurchaseMap.GetName() + "  [preset=round_box? " + text + " ]";
		}
	...
	private void PurchaseMap()
	{
		...
		{
			string text = T._("<em>{0}</em>의 <em>지도</em>를 구입하시겠습니까?", GameManager.Region.Name);
			string text2 = T._("[size=22][icon=icon_timer2] 섬 수명 {0}[/size]", GetIslandLifeTimeText());
			...
			messageBox.Show(text + "\n" + text2, subText, delegate(bool ok)
			{
				if (ok)
				{
