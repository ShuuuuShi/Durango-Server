...
public class EstateOwnerWidget : AnimationWidget
{
	...
	private Option GetOption(EstateInfo info)
	{
		...
		case OwnerType.PersonalPlayer:
			if (info.License.OwnerId == GameManager.PlayerId)
			{
				return _options.My;
			}
			return _options.Other;
		case OwnerType.ClanEstate:
		case OwnerType.ClanWarphole:
			if (ClanSystem.IsMyClan(info.License.OwnerId))
			{
				return _options.MyClan;
			}
			return _options.OtherClan;
		default:
