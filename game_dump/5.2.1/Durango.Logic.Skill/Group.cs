...
public class Group
{
	...
	public int HighestLevel()
	{
		...
		{
			int num3 = ((Skill != null) ? Skill : Skills[i]).HighestLevel();
			if (num3 > num)
			{
				num = num3;
			}
	...
	public int NearestNextAvailableCategoryLevel()
	{
		...
		for (int num2 = ((Skill != null) ? 1 : KUtility.GetSize(Skills)); i < num2; i++)
		{
			int num3 = ((Skill != null) ? Skill : Skills[i]).NearestNextAvailableCategoryLevel();
			if (num3 < num)
			{
				num = num3;
			}
	...
	public bool HasNew()
	{
		...
		{
			if (((Skill != null) ? Skill : Skills[i]).HasNew())
			{
				return true;
			}
		}
