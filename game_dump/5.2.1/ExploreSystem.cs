...
{
	[CompilerGenerated]
	private sealed class _003CCoTravelRegion_003Ed__34 : IEnumerator<object>, IDisposable, IEnumerator
	{
		...
		[DebuggerHidden]
		public _003CCoTravelRegion_003Ed__34(int _003C_003E1__state)
		{
	...
	{
		if (_clearedUnstableFactors == null)
		{
			return 0;
		}
		return _clearedUnstableFactors.Get(new Pair<int, Biome>(level, biome), 0);
	}
	...
	public bool HasRoutes()
	{
		if (_routes == null)
		{
			return _archipelagoRoutes != null;
		}
		return true;
	}
	...
	public Durango.Logic.Explore.Region GetRegion([NotNull] string regionId)
	{
		...
		if (region == null)
		{
			return Durango.Logic.Explore.Region.UnknownRegion;
		}
		return region;
	}
	...
	private IEnumerator CoTravelRegion(Port port, string regionId, string partierId, bool goesNeighbor, float delay)
	{
		...
		return new _003CCoTravelRegion_003Ed__34(0)
		{
			port = port,
			regionId = regionId,
			partierId = partierId,
			goesNeighbor = goesNeighbor,
			delay = delay
		};
	}
		...
		if (_routes != null)
		{
			foreach (KeyValuePair<Role, Dictionary<string, Route[]>> route3 in _routes)
			{
				...
				foreach (KeyValuePair<string, Route[]> item in route3.Value)
				{
					...
					if (!string.IsNullOrEmpty(key2) && SingletonDict<string, RegionTemplate>.Get(key2) != null)
					{
						Route[] array = value;
	...
	{
		UIManager.SystemMsg(T._("{0} 후 섬이 사라집니다.", TimedeltaFormatter.Format(msg.After, 1)), 4f);
	}
