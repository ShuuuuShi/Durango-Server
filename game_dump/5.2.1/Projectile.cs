...
public class Projectile
{
	...
	public bool NeedToDestroy()
	{
		...
		if (!Missed)
		{
			return HitTime + ProjectileSet.ExplosionDelay + num < Time.time;
		}
		return true;
	}
	...
	public bool Process()
	{
		...
		if (ProjectileSet.CurvedTrajectory)
		{
			return ProcessCurvedTrajectory(ProjectileSet);
		}
		return ProcessFlatTrajectory(ProjectileSet);
	}
	...
	private static float CalcArrowScaleAtPinned([CanBeNull] DamageableEntity damageable)
	{
		...
		float t = (Mathf.Max(damageable.XRadius, damageable.YRadius) - 50f) / 100f;
