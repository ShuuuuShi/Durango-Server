...
public class CraftSlotContainer : SlotContainer<CraftSlotInfo>
{
	...
	public void GetEstimation(Action<CraftEstimationInfo?> onResult)
	{
		if (onResult != null)
		{
			...
			estimateCraft.Materials = CreateFirstMaterialsDictionary(canFinished: false);
			estimateCraft.ToolItemId = GetToolItemId();
			estimateCraft.Workbench = GetWorkbench();
			EstimateCraft info = estimateCraft;
