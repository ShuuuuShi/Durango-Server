...
public class UsingActionAlert
{
	...
	public void Set(BattleAction action)
	{
		...
		if (Singleton<PlayerManager>.Instance().GetPlayer(GameManager.PlayerId) != null)
		{
			_ghostIndex = _items.Count;
			attackInfo = _action.Data.AttackInfo;
			foreach (PlayerActionAttackInfo info2 in attackInfo)
			{
				_items.Add(new Item
				{
					Info = info2
		...
		for (int num = _items.Count - 1; num >= 0; num--)
		{
			...
			else
			{
				...
				if (!id.HasValue)
				{
					...
					{
						int num2 = AreaOfEffectVisualizer.ShowAttackAlerted((!flag) ? AreaOfEffectVisualizer.Type.Player : AreaOfEffectVisualizer.Type.Alert, attackAlerted.Value);
						if (num2 == -1)
						{
							_items.RemoveAt(num);
						}
