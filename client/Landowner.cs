	...
	private void OnUpdateEstate()
	{
		...
		if (estateInfo.License.IsProtected())
		{
			...
			if (_protectedTimer == null || _protectedTimer.IsStop)
			{
				...
				UnknownSinceProgressGauge unknownSinceProgressGauge = Timer.Play<UnknownSinceProgressGauge>(_protectedTimer);
				unknownSinceProgressGauge.SetTarget(base.Artifact.gameObject, base.Artifact.InteractionPosition - base.Artifact.transform.localPosition);
			}
			else
			{
