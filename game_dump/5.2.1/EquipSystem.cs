...
public class EquipSystem : GameSystem<EquipSystem>
{
		...
		if (equipPreset != null)
		{
			...
			if (unlockSince.HasValue)
			{
				...
				if (unlockUntil.HasValue)
				{
					...
					return Mathf.Clamp01((float)((equipPreset.UnlockUntil.Value - Connections.Frontend.GetPredictedServerTime()) / num));
				}
			}
		}
		return -1f;
	...
	public static IEnumerable<EquipSlotType> EnumerateEquipPresetTypes(bool includeAvatar = false)
	{
		...
		if (includeAvatar)
		{
			return source.Where((EquipSlotType type) => type != EquipSlotType.Invalid);
		}
		return source.Where((EquipSlotType type) => type != EquipSlotType.Invalid && type != EquipSlotType.Avatar);
	}
