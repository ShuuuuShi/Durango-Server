	...
	private void Start()
	{
		...
		uIEventListener.onClick = (UIEventListener.VoidDelegate)Delegate.Combine(uIEventListener.onClick, (UIEventListener.VoidDelegate)delegate
		{
			...
			if (questYml != null)
			{
				...
				{
					UIManager.FindScript<DialogueGroupBase>().AddQuestMessages(_questId, questYml.QuestEndMessages, addFront: true);
				}
				if (KUtility.GetSize(questYml.QuestStartMessages) > 0)
				{
					UIManager.FindScript<DialogueGroupBase>().AddQuestMessages(_questId, questYml.QuestStartMessages, addFront: true);
				}
			}
		});
	}
