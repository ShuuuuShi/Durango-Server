...
public class MarketGroup : UIBase, INotificationable
{
	...
	private void Start()
	{
		...
		_productSoldAlarm = new StackableAlarm<string, ProductSold>("ProductSold", (ProductSold sold) => sold.Item.Id, (ProductSold sold, int count) => (count > 1) ? T._("<em>{0}</em> 외 {1}개 아이템이 팔렸습니다.", sold.Item.Name, count - 1) : T._("<em>{0}</em> 아이템이 팔렸습니다.", sold.Item.Name), "alarm_market", majorAlarm: true, 1.8f, delegate
		{
			if (!base.IsOpened)
			{
				Open();
