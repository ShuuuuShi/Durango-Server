...
public class Ocean : WaterBase
{
	...
	private void UpdateFoamFactor()
	{
		...
		sharedMaterial.SetFloat(value: (float)((double)_curOceanSet.FoamLength * num), nameID: _foamTimeId);
		...
		sharedMaterial.SetFloat(value: (float)((double)num5 * Math.Sin((_foamTime - 90f) * ((float)Math.PI / 180f)) + (double)num5 + (double)_curOceanSet.MinWetDarkness), nameID: _wetDarknessId);
	}
