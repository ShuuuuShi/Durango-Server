...
public class FactionSupportRequestWidget : UIWidget
{
	...
	public Transform GetButtonTransformIfRequestAvailable()
	{
		...
		return _requestButton.transform;
	}
	...
	private void OnRequestClicked()
	{
		if (_requestButton.Disabled)
		{
			...
			if (faction != null)
			{
				...
				UIManager.SystemMsg((!_supportRequest.IsAvailable()) ? T._("요청 가능 횟수를 모두 사용하였습니다. {0} 뒤 지원 요청 가능합니다.", TimedeltaFormatter.Format(Math.Max(num, val))) : T._("{0} 뒤 지원 요청 가능합니다.", TimedeltaFormatter.Format(num)));
			}
		}
		else
		{
