...
public class ClanInfoPage : ClanMenuPage, IUIInitializable
{
	...
	{
		return _introBulletinBoard.Back() && _noticeBulletinBoard.Back();
	}
	...
	private void RefreshRegionLabel()
	{
		if (_clan != null)
		{
			...
			{
				text = $"{text} [icon=img_market_arrowright]";
			}
	...
	{
		long amount = (long)currency;
		return Durango.Logic.Item.Inventory.CurrencyFormat(amount, Currency.TStone);
	}
	...
	private void ClanRenamingButton_Clicked(GameObject obj)
	{
		...
		if (_hasEditPermission && flag)
		{
			...
			{
				string text2 = TimedeltaFormatter.Format(num);
				string arg = T._("{0} 이내에 변경하지 않으면, 이대로 고정됩니다.", text2);
				text += $"\n[size=10] \n[/size][size=22]<alert>[icon=icon_make_alert] {arg}</alert>[/size]";
			}
	...
	private void OnClickDonateButton()
	{
		...
		numberInputPopup.Show(0L, Currency.TStone, title, DonateToClanFund, 99999999L);
	}
