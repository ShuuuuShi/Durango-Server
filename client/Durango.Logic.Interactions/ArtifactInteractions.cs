...
public class ArtifactInteractions
{
	...
	public void Init()
	{
		...
		{
			IconProgressGauge iconProgressGauge2 = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(timer.Timer);
			iconProgressGauge2.AddIcon(IconMap.Get(Interaction.Fertilize));
		...
		_waterPlantTimer.Started += delegate(PredictTimer timer)
		{
			...
			iconProgressGauge.AddIcon(IconMap.Get(Interaction.Watering));
	...
	private static void BuildArtifact([NotNull] Artifact artifact)
	{
		...
		buildSystem.InteractionBuildArtifact(artifact);
	}
	...
	private static void RemodelArtifact([NotNull] Artifact artifact)
	{
		if (GameManager.ClusterMode == Mode.Online)
		{
			...
			if (blueprint != null && blueprint.Available)
			{
				...
				{
					SelectRemodelingPartPopup selectRemodelingPartPopup = UIManager.Popup.Tooltip<SelectRemodelingPartPopup>();
					selectRemodelingPartPopup.Show(artifact);
				}
			}
			else
			{
		...
		else
		{
			...
			{
				SelectModularPartTexturePopup selectModularPartTexturePopup = UIManager.Popup.Tooltip<SelectModularPartTexturePopup>();
				selectModularPartTexturePopup.Show(artifact);
			}
		}
	}
	...
	private static void SkipPostprocess([NotNull] Artifact artifact)
	{
		...
		messageBox.ShowPayConfirm(cost, Currency.Gem, T._("즉시 완료하시겠습니까?"), delegate(bool ok)
		{
			if (ok)
			{
	...
	private static int GetSkipPostprocessCost([NotNull] Artifact artifact)
	{
		...
		{
			int[] array = Yaml.Util.Singleton<CashYaml>.Instance.instant_construction[i];
			num = num3;
			num2 = num4;
			num3 = array[0];
			num4 = array[1];
	...
	private static void Capsulate([NotNull] Artifact artifact)
	{
		...
		}).On(delegate(Messages.Cost msg, PacketHeader _)
		{
			...
			{
				MessageBox messageBox = UIManager.MessageBox;
				messageBox.ShowPayConfirm(msg.Amount, msg.Currency, T._("<em>{0}</em>{0:-을} 포장하시겠습니까?", artifact.LocalizedName), delegate(bool ok)
				{
					if (ok)
					{
	...
	{
		Durango.Logic.Timer.Timer timer = new Durango.Logic.Timer.Timer("destruct", msg.Duration);
		...
		PlayerController.MotionUpdater.Motion(toolType switch
		{
			1 => "Onehand_Destroy", 
			2 => "Twohand_Destroy", 
			_ => "Barehand_Destroy", 
	...
	{
		PlayerBehavior player = Durango.Utils.Singleton<PlayerManager>.Instance().GetPlayer(delegate(PlayerBehavior target)
		{
	...
	private static string GetAttachmentType(Transform attachment)
	{
		...
		int result;
		return (!int.TryParse(text, out result)) ? string.Empty : text;
	}
	...
	private static void ScribbleDrawing([NotNull] Artifact messageBoard)
	{
		...
		string arg = T._("그림은 1회만 완성할 수 있습니다.");
		canvasUI.SetWarningText($"<alert>[icon=icon_make_alert]{arg}</alert>");
	}
	...
	private static void NameArtifact([NotNull] Artifact artifact)
	{
		if (artifact.BuildCompleted)
		{
			...
			textInputPopup.Show(delegate(string message)
			{
	...
	private static void RepairArtifactImmediately([NotNull] Artifact artifact)
	{
		...
		if (artifact.ArtifactState.IsRepairing())
		{
			...
			MessageBox messageBox = UIManager.MessageBox;
			messageBox.ShowPayConfirm(currentCost, Currency.Gem, T._("수리를 즉시 완료하시겠습니까?"), delegate(bool ok)
			{
				if (ok)
				{
		...
		messageBox2.ShowPayConfirm(repairRequirementPerformance, Currency.Gem, T._("워프젬으로 즉시 수리하시겠습니까?"), delegate(bool ok)
		{
			if (ok)
			{
		...
		popupItemSelector.Filter((ItemData data) => data.HasTag("plantable")).OnConfirmed(delegate(ItemData item)
		{
			if (item != null)
			{
				UIManager.MessageBox.ShowLockConfirm(item, delegate
				{
					...
					{
						IconProgressGauge iconProgressGauge = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(new Durango.Logic.Timer.Timer("plant", msg.Duration));
	...
	private static void OpenTechSupport([NotNull] Artifact artifact)
	{
		...
		TechSupportGroup techSupportGroup = UIManager.FindScript<TechSupportGroup>();
		techSupportGroup.Open(artifact);
	}

	private void ManagePioneerGrade([NotNull] Artifact artifact)
	{
		PioneerGradeManageGroup pioneerGradeManageGroup = UIManager.FindScript<PioneerGradeManageGroup>();
		pioneerGradeManageGroup.Open(artifact);
	}
	...
	private static void Uproot([NotNull] Artifact farm)
	{
		...
		{
			IconProgressGauge iconProgressGauge = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(new Durango.Logic.Timer.Timer("uproot", msg.Duration));
			iconProgressGauge.AddIcon("tool_bare_hands");
			PlayerController.MotionUpdater.Motion("Farming_Gather", msg.Duration);
		});
	}
	...
	private static void GrowRapidly([NotNull] Artifact artifact)
	{
		...
		MessageBox messageBox = UIManager.MessageBox;
		messageBox.ShowPayConfirm(cost, Currency.Gem, comment, delegate(bool ok)
		{
			if (ok)
			{
	...
	{
		PopupItemSelector popupItemSelector = UIManager.Popup.Tooltip<PopupItemSelector>();
		popupItemSelector.Filter((ItemData item) => !item.IsEquipments && ((!isBody) ? item.HasAttribute("slot", "head") : (item.HasAttribute("slot", "body") || item.HasAttribute("slot", "hoody")))).OnConfirmed(delegate(ItemData item)
		{
			if (item != null)
			{
				UIManager.MessageBox.ShowLockConfirm(item, delegate
	...
	private static Messages.Item? GetTouchedMannequinItem(Artifact artifact, bool isBody)
	{
		...
		if (!(lastTouched.EntityId != artifact.EntityId))
		{
			...
			{
				return (!isBody) ? lastTouched.Mannequin.Value.Head : lastTouched.Mannequin.Value.Body;
			}
		}
		return null;
	}
	...
	{
		ArtifactFloor artifactFloor = Yaml.Util.Singleton<Constants>.Instance.ArtifactFloor;
		if (artifactFloor.MaxStories <= artifact.Stories.Value.GetValueOrDefault())
		{
			UIManager.SystemMsg(T._("더 이상 증축할 수 없습니다."));
			return;
		}
		InteractionGroup interactionGroup = UIManager.FindScript<InteractionGroup>();
		interactionGroup.InteractionMenu.ShowSubmenus(Interaction.ExtendFloor, Interaction.ExtendFloorWithRoof, Interaction.ExtendFloorWithoutRoof);
	}
	...
	private static void ArtifactExtendFloor([NotNull] Artifact artifact, bool withRoof)
	{
		...
		MessageBox messageBox = UIManager.MessageBox;
		messageBox.ShowPayConfirm(cost, currency, T._("증축하시겠습니까?"), delegate(bool ok)
		{
			if (ok)
			{
	...
	private static void ChangeFloor(int offset)
	{
		...
		int value = PlayerBehavior.LocalPlayer.Floor.Value;
		int num = value + offset;
		if (num >= 0)
		{
			int? value2 = artifact.Stories.Value;
			if (!value2.HasValue || num < value2.GetValueOrDefault())
			{
