...
public class InteractionMenuListWidget_PC : InteractionMenuListWidgetBase
{
	...
	private void OnRightClickMenu()
	{
		...
		interactionMenuWidgetBase.RemoveFirstQueue();
	}
	...
	public override bool CloseMenus()
	{
		...
		// [แก้เอง] คืน true = "ฉันจัดการคำสั่งปิดนี้แล้ว" ซึ่งทำให้ UIBase.CloseUI() ไม่ไปปิดหน้าต่างจริง
		// ถ้าสั่งปิดแล้วยังไม่หาย ต้องคืน false เพื่อส่งต่อให้คนอื่นปิด ไม่งั้นทุกหน้าต่างปิดไม่ได้เลย
	...
	protected override void ClearInvalidMenu()
	{
		...
		int num3 = 0;
		for (int num4 = Menus.Count - 1; num4 >= 0; num4--)
		{
			if (Menus[num4].Empty)
			{
				RemoveAt(num4);
	...
	private void RepositionMenuItem(InteractionMenuWidget_PC menuWidget)
	{
		...
		float t = VisibleStartDegree - (float)num * num3;
		float num4 = Mathf.Repeat(t, 360f) * ((float)Math.PI / 180f);
	...
	{
		if (InteractionMenuListWidgetBase.IsShow && MenuTarget != null && (bool)PlayerBehavior.LocalPlayer.IsMoving)
		{
			float sqrMagnitude = (PlayerBehavior.LocalPlayer.CurrentPosition - MenuTarget.Position).sqrMagnitude;
			if (!(sqrMagnitude < _maxDistanceFromPlayer * _maxDistanceFromPlayer))
			{
				GameSystem<InteractionSystem>.Instance().SetInteractionTarget(null);
			}
		}
