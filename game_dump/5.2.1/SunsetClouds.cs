...
public class SunsetClouds : MonoBehaviour
{
	...
	private void Start()
	{
		...
		for (int i = 0; i < cloudBaseArray.Length; i++)
		{
			cloudBaseArray[i].gameObject.SetActive(value: false);
		}
	...
	private UIWidget GetActiveCloud()
	{
		...
		else
		{
			...
			uIWidget = Object.Instantiate(_cloudBaseArray[num].gameObject, base.transform).GetComponent<UIWidget>();
