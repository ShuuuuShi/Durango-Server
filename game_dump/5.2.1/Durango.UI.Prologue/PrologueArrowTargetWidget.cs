	...
	private void LateUpdate()
	{
		if (IsEnabled())
		{
			...
			if (vector3.x < num || vector3.x > (float)Screen.width - num || vector3.y < num || vector3.y >= (float)Screen.height - num)
			{
				...
				float num3 = Mathf.Min(b: ((float)Screen.height - num * 2f) / Mathf.Abs(vector4.y), a: num2 / Mathf.Abs(vector4.x));
				Vector3 nguiPos = vector2;
				nguiPos.x += vector4.x * 0.5f * num3;
				nguiPos.y += vector4.y * 0.5f * num3;
				nguiPos.z = 0f;
				base.transform.localPosition = MainCamera.ScreenPosToNGUIPos(nguiPos);
				vector4.Normalize();
				float num4 = Mathf.Atan2(vector4.x, vector4.y) * 57.29578f;
				num4 = 0f - num4;
