...
public abstract class InteractionMenuListWidgetBase : AnimationWidget, IUIInitializable
{
		...
		if (target != null && !(target.EntityId != lastTouched.EntityId))
		{
			...
			if (workbench.HasValue)
			{
				...
				for (int n = 0; n < size; n++)
				{
					...
					{
						InteractionCraftSlotWidget component5 = nodes[num5].GetComponent<InteractionCraftSlotWidget>();
						if (!component5.Valid)
						{
							num3 = num5;
							break;
				...
				for (int num6 = 0; num6 < size2; num6++)
				{
					...
					{
						InteractionCraftSlotWidget component6 = nodes[num8].GetComponent<InteractionCraftSlotWidget>();
						if (component6.Valid && component6.Id == crafting2.Id)
						{
							num7 = num8;
							break;
						}
					...
					{
						InteractionCraftSlotWidget component7 = nodes[num9].GetComponent<InteractionCraftSlotWidget>();
						if (!component7.Valid)
						{
							num7 = num9;
							break;
				...
				{
					InteractionCraftSlotWidget component8 = nodes[num10].GetComponent<InteractionCraftSlotWidget>();
					if (!component8.Valid)
					{
						component8.SetEmpty();
					}
				...
				if (_updateResetFlag)
				{
					TweenAlpha component9 = _craftSlotScrollView.GetComponent<TweenAlpha>();
					component9.ResetToBeginning();
					component9.PlayForward();
				}
	...
	protected InteractionMenuWidgetBase InteractionMenu_Pop()
	{
		...
		{
			GameObject gameObject = _interactionMenuWidget.transform.parent.gameObject.AddChild(_interactionMenuWidget.gameObject);
			interactionMenuWidgetBase = gameObject.GetComponent<InteractionMenuWidgetBase>();
	...
	private void RefreshSubMenus()
	{
		...
		{
			Interaction interaction = _subMenuList[i];
			string text = IconMap.Get(interaction);
			string text2 = interaction.GetName();
