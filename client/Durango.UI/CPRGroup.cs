...
public class CPRGroup : UIBase
{
	private class NoteItem
	{
		...
		public TweenAlpha AlphaTweener => (!Note) ? null : Note.GetComponent<TweenAlpha>();

		public TweenScale ScaleTweener => (!Note) ? null : Note.GetComponent<TweenScale>();
		...
		public Vector3 Pos
		{
			get
			{
				return (!Note) ? Vector3.zero : Note.transform.localPosition;
			}
			set
			{
				if ((bool)Note)
