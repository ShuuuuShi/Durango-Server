...
public class Inventory
{
	...
	{
		string format = currency.Normalize() switch
		{
		...
		return string.Format(T.Culture, format, amount);
	}
	...
	public static bool CheckEnableUseType(IList<ItemData> selectedItems, UseType useType)
	{
		...
		case UseType.TakeOut:
			return GameSystem<InventorySystem>.Instance().PlayerInventory.CanPutIn(selectedItems);
		...
		case UseType.Imprint:
			...
			{
				return selectedItems[0].CanImprint();
			}
			return false;
		default:
			return true;
