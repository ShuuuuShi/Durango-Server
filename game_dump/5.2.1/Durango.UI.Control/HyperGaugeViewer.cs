...
public class HyperGaugeViewer : MonoBehaviour
{
	...
	{
		return (float)((gaugeSprite.type == UIBasicSprite.Type.Filled) ? ((int)(gaugeSprite.fillAmount * (float)gaugeSprite.width)) : ((gaugeSprite.alpha > 0f) ? gaugeSprite.width : 0)) / (float)_background.width;
	}
	...
	private void Update()
	{
		...
		Gauge.CurrentValueAndVelocity(_max, Gauge.CurrentTime, out var value2, out var velocity2);
		float y = _range.y;
		if (_mainUpper != null)
		{
		...
		if (_max.Count > 0 && _subUpper != null)
		{
			Gauge.CurrentValueAndVelocity(_max, Gauge.CurrentTime, out var value3, out value2);
