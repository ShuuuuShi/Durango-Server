...
public class CraftGroupBase : UIBase, IScreenResizeReceiver
{
	...
	private void RefreshCraftOrTechSupportButton(bool byTechSupport)
	{
		...
		// Some PC prefab states leave the footer button inactive. Refreshing its
		// label and Disabled flag is not enough when the GameObject itself is off.
	...
	private void DoBuildSystemWork(string messages, Action work)
	{
		...
		string arg = ((safestItem.SafeLevel != SafeLevel.Locked) ? T._("<em>임무</em> 수행에 필요한 <em>{0}</em>{0:-이} 포함되어 있습니다.", safestItem.Name) : T._("<em>잠금</em> 설정된 <em>{0}</em>{0:-이} 포함되어 있습니다.", safestItem.Name));
		UIManager.MessageBox.Show($"{arg} {messages}", delegate(bool ok)
		{
			if (ok)
			{
				work();
