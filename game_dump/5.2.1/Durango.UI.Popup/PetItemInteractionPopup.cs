...
public class PetItemInteractionPopup : TooltipBase
{
	...
	private void OnClickMarket()
	{
		...
		if (pet != null)
		{
			...
			{
				UIManager.FindScript<MarketGroup>().OpenAndSearch(null, 0, text);
				Hide();
			}
		}
	}
	...
	private void OnUpdateItemByTaskFeed()
	{
		...
		if (petTask.HasValue)
		{
			...
			for (int size = KUtility.GetSize(selectedList); i < size; i++)
			{
				...
				num += PetUtil.GetPetFoodEnergy(item);
			}
			...
			{
				num2 += (double)PetUtil.GetPetFoodRejuvenatingDays(selectedList[j]);
			}
			_ageLabel.SetText(GetAgeString(key, num2 * 24.0 * 60.0 * 60.0));
			if (value.HasValue)
			{
			...
			_infoLabel.SetText(GetPetDescription(key, num));
		}
	}
	...
	private static string GetDomesticationDescription(DomesticationInfo info, List<ItemData> items = null)
	{
		...
		if (KUtility.GetSize(items) > 0)
		{
			...
			if (domesticationProbability - num > 0f)
			{
