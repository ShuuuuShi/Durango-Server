...
public class ItemInfoView : MonoBehaviour
{
	...
	private void FillItemData([NotNull] ItemData item)
	{
		...
		if (item.Durability.Get(currentTime) <= 0f)
		{
	...
	private void SetDurability([CanBeNull] ItemData item)
	{
		...
		bool num = item.Prototype != null && item.Prototype.ImmuneToTime;
		bool flag = item.Prototype != null && item.Prototype.TimeLimited;
		string spriteName;
		Color color;
		string text;
		if (num)
		...
		else if (flag)
		{
			...
			color = Color.black;
			double num2 = durability.When(0f);
			if (num2 > 0.0)
			{
				double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
				text = ((!(num2 > predictedServerTime)) ? T._("파괴됨") : T._("{0} 남음", TimedeltaFormatter.Format(num2 - predictedServerTime, 1, "min")));
			}
			else
			{
				float num3 = durability.Get();
				if (num3 > 0f)
				{
					...
					if (dailyVelocity < 0f)
					{
						num3 /= dailyVelocity;
						num3 *= -86400f;
						text = TimedeltaFormatter.Format(num3, 1, "min");
					}
					else
					{
						text = Util.LocalizedDurability(durability.Get(), durability.Max());
	...
	private void SetAge(Messages.Pet? pet)
	{
		if (pet.HasValue)
		{
			...
			_ageLabel.SetText(new SyncString(delegate(out string text, out float period)
			{
				...
				if (num > 0.0)
				{
					text2 = TimedeltaFormatter.Format(num, 1);
				...
				text = "[icon=icon_timer] " + text2 + "/" + lifeSpanText + " [icon=img_loading_unknown_question1]";
			}));
		}
		else
		{
	...
	private WidgetTooltipControl OnClickDurabilityPanel(GameObject go)
	{
		...
		{
			bool num = _targetItem.Prototype != null && _targetItem.Prototype.ImmuneToTime;
			bool flag = _targetItem.Prototype != null && _targetItem.Prototype.TimeLimited;
			if (num)
			{
				title = T._("내구도 안내");
			...
			{
				float num2 = _targetItem.Durability.Velocity();
				if (num2 < 0f)
				{
					title = T._("내구도 안내");
					text = T._("장비, 도구를 제외한 대부분의 아이템은 <em>하루에 {0:0.#}씩</em> 내구도가 줄어듭니다.", (0f - num2) * 86400f);
				}
			}
		}
		if (string.IsNullOrEmpty(text))
