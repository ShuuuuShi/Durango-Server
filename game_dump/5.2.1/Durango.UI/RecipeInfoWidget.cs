...
public class RecipeInfoWidget : MonoBehaviour, IUIInitializable
{
	...
	private void SetMaterials([NotNull] IList<SlotStruct> slots, SlotStruct toolInfo)
	{
		...
		if (num > 0)
		{
			...
			_materialItemWidgets[0].GetComponent<RecipeMaterialInfoWidget>().Set(T._("재료"), slots);
			if (flag)
			{
				RecipeMaterialInfoWidget component = _materialItemWidgets[1].GetComponent<RecipeMaterialInfoWidget>();
				component.Set(T._("도구"), new SlotStruct[1] { toolInfo });
				component.SetPinButton(null);
			}
			int num2 = 0;
			int i = 0;
