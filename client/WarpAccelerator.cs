...
public class WarpAccelerator : ArtifactComponent
{
	...
	private void SetWaitTimer(double since, double until)
	{
		...
		if (_waitTimer == null || _waitTimer.IsStop)
		{
			...
			TimerProgressGauge timerProgressGauge = Durango.Logic.Timer.Timer.Play<TimerProgressGauge>(_waitTimer);
			timerProgressGauge.SetTarget(base.Artifact.gameObject, new Vector3(base.Size.x, 2f, base.Size.y) * 200f * 0.5f);
		}
		else
		{
	...
	private void UpdateTimer()
	{
		...
		AcceleratorStatus status = warpaccelerator.Value.Status;
		if (status == AcceleratorStatus.Waiting)
		{
