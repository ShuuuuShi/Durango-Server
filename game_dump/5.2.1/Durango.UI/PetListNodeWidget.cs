...
public class PetListNodeWidget : MonoBehaviour, IScreenResizeReceiver
{
	...
	private void UpdateLayout()
	{
		if (_isInit)
		{
			List<UIWidget> list = ((!_addButton.gameObject.activeSelf) ? new List<UIWidget>() : new List<UIWidget> { _addButton });
			List<UIWidget> list2 = list;
			list2.AddRange(_infoNodes.Select((PetListInfoNode node) => node.GetComponent<UIWidget>()));
			...
			{
				float num = UIUtility.WidgetsReposition(list2, Vector3.right, localCenter, 0f, 0.5f);
				component.SetDimensions((int)num, _baseInfoNode.Widget.height + _titleWidget.height);
			}
			else
			{
				float num2 = UIUtility.WidgetsReposition(list2, Vector3.down, localCenter, 0f, 0.5f);
