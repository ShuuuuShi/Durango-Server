...
public class NoticeSystem : GameSystem<NoticeSystem>, INotificationable
{
	...
	private void Start()
	{
		Singleton<GameManager>.Instance().MainSceneLoaded += delegate
		{
			...
			{
				Http.RequestYml<Notice>(GameManager.GatewayUrl + "/notice", UpdateNotice, disableCache: true);
			}
		};
	}
	...
	{
		return Preferences.GetString("last_read_notice", string.Empty) == notice.Url;
	}
