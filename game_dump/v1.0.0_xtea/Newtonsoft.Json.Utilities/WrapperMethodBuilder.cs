...
internal class WrapperMethodBuilder
{
	...
	public void Generate(MethodInfo newMethod)
	{
		...
		Type[] parameterTypes = parameters.Select((ParameterInfo parameter) => parameter.ParameterType).ToArray();
		MethodBuilder methodBuilder = _wrapperBuilder.DefineMethod(newMethod.Name, MethodAttributes.Public | MethodAttributes.Virtual, newMethod.ReturnType, parameterTypes);
		if (newMethod.IsGenericMethod)
		{
