...
public class SkillNodeInfoWidget : MonoBehaviour
{
	...
	private void SetCondition()
	{
		...
		{
			int categoryLevel = GameSystem<SkillSystem>.Instance().GetCategoryLevel(_skill.Category);
			bool num = _skill.CategoryLevel <= categoryLevel;
			bool flag = false;
			int num2 = 0;
			if (num)
			{
				num2 = GameSystem<SkillSystem>.Instance().RemainSkillPoint;
				if (_skill.SkillPoints > num2)
				{
					flag = true;
				}
			}
			color = ((!num) ? new Color(0.73f, 0.2f, 0.22f) : new Color(1f, 0.85f, 0.36f));
			if (num)
			{
				int num3 = _skill.Parent.Level + 1;
				if (_skill.Level > num3)
				{
					text = T._("[icon=icon_make_alert] {0} 필요", _skill.Parent.Get(num3).Name);
				}
				...
				{
					text = T._("[icon=icon_make_alert] SP {0} 부족", _skill.SkillPoints - num2);
				}
			}
			else
			{
