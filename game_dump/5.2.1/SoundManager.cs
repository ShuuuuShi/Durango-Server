...
public class SoundManager : Singleton<SoundManager>
{
	...
	{
		AkSoundEngine.SetState(soundStates.Group, soundStates.State);
		_ = 1;
	}
	...
	{
		int io_rValueType = 1;
		AkSoundEngine.GetRTPCValue(name, null, 0u, out var out_rValue, ref io_rValueType);
		_ = 1;
		return out_rValue;
	}
	...
	{
		AkSoundEngine.SetRTPCValue(parameter.Name, parameter.Value);
		_ = 1;
	}
	...
	private SoundEventInstance GetSoundInstance()
	{
		...
		uint num2 = (soundEventInstance.InstanceId = _instanceIdGenerator++);
		uint key = num2;
	...
	{
		if (!IgnorePreparedCheck)
		{
			return _soundBanksLoader.IsPreparedEvent(eventName);
		}
		return true;
	}
