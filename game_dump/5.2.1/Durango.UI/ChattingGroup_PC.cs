...
public class ChattingGroup_PC : ChattingGroupBase
{
	...
	public bool IsCurrentlyAllChannel
	{
		get
		{
			if (base.FilterType == ChatFilterType.All)
			{
				return base.CurrentConv == null;
			}
			return false;
		}
	}

	public bool IsCurrentlySystemChannel
	{
		get
		{
			if (base.FilterType == ChatFilterType.System)
			{
				return base.CurrentConv == null;
			}
			return false;
	...
	{
		BottomMenuWidget_PC bottomMenuWidget_PC = UIManager.FindScript<BottomLeftMenuGroup_PC>().BottomMenuWidget as BottomMenuWidget_PC;
	...
	private void OnMakeChatRoom(int index)
	{
		...
		UIManager.FindScript<PlayerSearchGroup>().SelectTab(index);
	}
	...
	public void Show(bool isFocus = true)
	{
		...
		for (int i = 0; i < hideOnShrinks.Length; i++)
		{
			hideOnShrinks[i].alpha = 1f;
		}
		hideOnShrinks = _hideOnHides;
		for (int i = 0; i < hideOnShrinks.Length; i++)
		{
			hideOnShrinks[i].alpha = 1f;
		}
		...
		{
			hidableColliders[i].enabled = true;
		}
	...
	public void Shrink()
	{
		...
		for (int i = 0; i < hidableColliders.Length; i++)
		{
			hidableColliders[i].enabled = false;
		}
	...
	private void RefreshChannel()
	{
		...
		if (base.CurrentConv == null)
		{
			...
			if (isCurrentlyAllChannel)
			{
				...
				foreach (ChatStruct chatting in GameSystem<SocialSystem>.Instance().ChattingList)
				{
					if (!GameSystem<SocialSystem>.Instance().ChannelInfo.IsHidden(chatting.Type))
					{
						list.Add(chatting);
					}
				}
				foreach (Conversation item in GameSystem<SocialSystem>.Instance().Conversations.Values.ToList())
				{
					if (!GameSystem<SocialSystem>.Instance().ChannelInfo.IsHidden(item))
					{
						list.AddRange(item.Messages);
					}
				}
	...
	private bool IsChattableFilter(ChatFilterType filter)
	{
		...
		return SocialSystem.IsAllowedChannel(SocialSystem.ConvertToChannelType(filter));
	}
