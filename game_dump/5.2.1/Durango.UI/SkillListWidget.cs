...
public class SkillListWidget : MonoBehaviour
{
	...
	private void OnSkillListUpdate()
	{
		...
		{
			nodes[i].GetComponent<SkillListNode>().UpdateData();
		}
	}
	...
	private static int SkillBundleComparison(Group s1, Group s2)
	{
		...
		int num = s2.GetLearnableCount() - learnableCount;
		if (num != 0)
		{
			return num;
		}
		int num2 = s1.HighestLevel();
		num = s2.HighestLevel() - num2;
		if (num == 0)
		{
			int num3 = s1.NearestNextAvailableCategoryLevel();
			int num4 = s2.NearestNextAvailableCategoryLevel();
			num = num3 - num4;
			if (num == 0)
			{
