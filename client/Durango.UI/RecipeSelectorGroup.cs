...
public class RecipeSelectorGroup : UIBase, INotificationable
{
	private class LastSelectedInfo
	{
		...
		{
			return (category == null) ? "entire_category" : category.Id;
		}
	}
	...
	{
		string argument = UriParser.GetArgument("lv");
		...
		receiveRewardsPopup.ShowRecipeBonusInfo(id, level);
	}
	...
	private void AddRecipeItems([NotNull] Crafting.Category category, Dictionary<string, RecipeListWidget.SubList> result, bool inEntireCategory, RecipeListWidget.SubList favorites = null)
	{
		for (int i = 0; i < category.ItemCount; i++)
		{
			...
			if (IsValidCategoryItem(item))
			{
				...
				RecipeListWidget.SubList orCreateRecipeSubList = GetOrCreateRecipeSubList(key, text, inEntireCategory, result);
				orCreateRecipeSubList.AddItem(item);
				if (item.Like)
				{
	...
	private void RefreshDetailPanel(bool reset)
	{
		...
		if (selectedRecipe.HasValue)
		{
			if (_isBuildCheat)
			{
				RecipeSystem.RecipeType recipeType = selectedRecipe.Value.RecipeType;
				if (recipeType == RecipeSystem.RecipeType.Building)
				{
	...
	private Artifact GetValidWorkbench(Crafting.Recipe recipe)
	{
		if (_selectedWorkbench != null)
		{
			return (!recipe.IsValidWorkbench(_selectedWorkbench)) ? null : _selectedWorkbench;
		}
	...
	private void RecipeInfo_Confirmed()
	{
		...
		case RecipeSystem.RecipeType.Crafting:
		{
			// [แก้เอง] เดิม quickFill เป็น false ⇒ เลือกสูตรแล้วช่องวัตถุดิบว่างเปล่า
			// ต้องกดปุ่มคราฟรอบแรกเพื่อ "เติมของ" แล้วกดอีกรอบถึงจะคราฟจริง
			// (ButtonCraftOrTechSupport_OnClick: state CanQuickFill → QuickFill() แล้ว return เฉย ๆ)
			// ⇒ ใช้ QuickFill ของเกมเองตั้งแต่ตอนเปิด จะได้ "เลือกสูตร → กดคราฟ" จบในครั้งเดียว
			Crafting.Recipe recipe = OpenItemCraftingUI(value.Id, quickFill: true);
			if (recipe != null)
			{
