...
public class PetResetRankPopup : TooltipBase
{
	...
	private void OnConfirm()
	{
		...
		if (!candidate2.HasValue)
		{
			...
			messageBox.Show(T._("정말 진행하시겠습니까?"), T._("<alert>[icon=icon_make_alert] 재접속, 전투, 상점 등으로 화면이 전환되면 등급 초기화가 완료되지 않을 수 있습니다. 안전한 장소에서 진행하시는 것을 추천드립니다.</alert>"), delegate(bool ok)
			{
				if (!ok)
				{
					_isWait = false;
		...
		messageBox2.AddKeyValueInfo(T._("새로운 등급"), $"[size=36]{((_candidate.Value.Rank != PetRank.S) ? _candidate.Value.Rank.ToString() : $"<em>{_candidate.Value.Rank.ToString()}</em>")}[/size]");
		messageBox2.Show(T._("<em>새로운 등급</em>으로 결정하시겠습니까?"), T._("[icon=icon_make_alert] 확정 후에는 이전 등급으로 돌아갈 수 없습니다."), delegate(bool ok)
		{
			if (ok)
			{
				_isWait = true;
	...
	private void ShowResultPage(bool instant)
	{
		...
		TweenAlpha tweenAlpha = TweenAlpha.Begin(_lineSprite.gameObject, 0.2f, 1f);
