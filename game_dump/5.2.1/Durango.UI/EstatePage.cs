...
public class EstatePage : MonoBehaviour, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		{
			UIManager.FindScript<EstateGridGroup>().Open(OwnerType.PersonalPlayer, _largestSize);
		};
		_emptyUrbanEstatePage.DeclareButtonClicked += delegate
		{
			UIManager.FindScript<EstateGridGroup>().Open(OwnerType.Player, _largestSize);
		};
		_emptyClanEstatePage.DeclareButtonClicked += delegate
		{
			UIManager.FindScript<EstateGridGroup>().Open(OwnerType.ClanEstate, _largestSize);
		};
		_pioneerInfoWidget = _pioneerInfoLinker.Object.GetComponent<PioneerInfoWidget>();
		_pioneerInfoWidget.Clicked += delegate
		{
			UIManager.Popup.FindTooltip<PioneerGradeRewardsPopup>().Show();
		};
	}
	...
	private void EstateExpand()
	{
		if (_license.HasValue)
		{
			...
			{
				UIManager.FindScript<EstateGridGroup>().Open(estateInfo, _largestSize);
			}
		}
	}
