...
public class ChattingLine_PC : ChattingLineBase
{
	...
	public override void SetChat(ChatStruct chat, bool isAllChat = false)
	{
		...
		{
			string arg = NGUIText.EncodeColor(chat.GetMsgColor(Color.white));
			string arg2 = chat.Type.GetName();
			_tagLabel.text = $"[{arg}]{arg2}[-]";
	...
	protected override void SetText(ChatStruct chat)
	{
		...
		{
			float num = NameLabel.transform.localPosition.x + (float)NameLabel.width - (float)TextLabel.leftAnchor.absolute;
			int totalWidth = (int)(num / _spaceWidth);
