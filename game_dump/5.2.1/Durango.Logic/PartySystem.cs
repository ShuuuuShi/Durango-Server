...
public class PartySystem : GameSystem<PartySystem>
{
	...
	private void OnParty(Messages.Party msg, PacketHeader header)
	{
		...
		for (int i = 0; i < memberStatus.Length; i++)
		{
			...
			Durango.Logic.Party.Member item = CreateMember(pair.Item1, isLeader: false, pair.Item2);
			_partyMembers.Add(item);
	...
	{
		if (IsAcceptedInParty)
		{
			return IsAcceptedMember(entityId);
		}
		return false;
	}
