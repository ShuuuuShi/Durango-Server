...
public class MenuListGroupBase : UIBase
{
	private class TryKnockLoaclNetwork
	{
		...
		private void OnSelectItem(int index)
		{
			...
			// [แก้เอง] ของเดิมกรอง "IP ของเครื่องตัวเอง" ออก ทำให้เปิดเซิร์ฟกับเล่นบนเครื่องเดียวกัน
			// แล้วกดเข้าจากรายการที่เจอด้วย UDP knock ไม่ได้เลย — เอาเงื่อนไขออก
			// (เดิมเป็น IL patch ใน tools/DllPatcher)
	...
	{
		return base.IsOpened || IsMenuLocked();
	}
	...
	private MenuWidget GetMenuItem(MenuType type)
	{
		...
		case MenuLayout.Landscape:
			if (_landscapeMenuList.TryGetMenuItem(type, out comp))
			{
				return comp;
			}
			break;
		case MenuLayout.Portrait:
			if (_portraitMenuList.TryGetMenuItem(type, out comp))
			{
				return comp;
			}
			break;
		case MenuLayout.Locked:
			if (_lockedMenuList.TryGetMenuItem(type, out comp))
			{
				return comp;
			}
			break;
	...
	public Transform GetMenuTransform(MenuType type)
	{
		...
		return (!(menuItem == null)) ? menuItem.transform : null;
	}
	...
	protected virtual void OnMenuClick(MenuType type)
	{
		...
		{
			TryKnockLoaclNetwork tryKnockLoaclNetwork = new TryKnockLoaclNetwork();
			tryKnockLoaclNetwork.Start();
			return;
		}
		}
	...
	{
		return script != base.VisibleController && (script.Flag & VisibleType.Base) != 0 && (!Platform.Instance.UsePCUI || (script.Flag & VisibleType.HideOnLeftMenu) != 0);
	}
	...
	{
		return !base.IsOpened && !UIBase.HasOpenedFullscreenUI && !IsMenuLocked();
	}
