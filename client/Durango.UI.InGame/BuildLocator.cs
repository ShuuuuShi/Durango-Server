...
public class BuildLocator : Singleton<BuildLocator>
{
	...
	public Vector2 CenterTile
	{
		get
		{
			...
			Vector2 vector2 = WorldTilePos.ToVector2();
			return vector2 + vector * 0.5f;
		}
	}

	public bool IsPerpendicular => Rotation == Rotation.Quarter || Rotation == Rotation.ThreeQuarter;
	...
	private void Update()
	{
		...
		StateColor stateColor = _gridStateColors[(int)((!IsAreaInAndOut) ? CurrentGridMinState : BuildGridState.Invalid)];
		Color previewColor = stateColor.PreviewColor;
		if (_isPresetPreview)
		{
			previewColor.a = num2;
		}
	...
	public void SetArtifactBuildingMode(Arguments arguments)
	{
		...
		clientPosition = Durango.Terrain.Util.ClientPositionToWorldPosition(clientPosition);
		Vector2 vec = Durango.Terrain.Util.WorldPositionToTilePosition(clientPosition);
	...
	private void LoadPreviewModel()
	{
		...
		InitBuildgrid();
		Vector3 position = MainCamera.ScreenPosToWorldPos(new Vector3((float)Screen.width / 2f, (float)Screen.height / 2f));
		Vector2 vec = Durango.Terrain.Util.ClientPositionToTilePosition(position);
		WorldTilePos = ToIntoArea(new Point2(vec));
	...
	private void EnableGridView()
	{
		...
		rectGridArea.Tile = value.position;
		rectGridArea.Size = value.size;
		rectGridArea.TileColorFunc = GetTileStateColor;
		RectGridArea rectGridArea2 = rectGridArea;
		gridAreaViewer.Show(new GridAreaBase[1] { rectGridArea2 }, _arguments.Floor, GridAreaViewer.LayerType.Upper, tweenAlpha: false);
	}
	...
	private BuildGridState GetTileBuildState(Point2 tile)
	{
		...
		if (_arguments.Exterior && tileObject.IsEmpty())
		{
			...
			return (estateInfo2 == null || !estateInfo2.IsLocalPlayers()) ? BuildGridState.Vaild : BuildGridState.Estate;
		}
	...
	private void InputTouched(InputCommandMessage message)
	{
		...
		{
			Vector3 clientPosition = MainCamera.ScreenPosToWorldPos(Input.mousePosition, _arguments.Floor.GetValueOrDefault() * 200);
			clientPosition = Durango.Terrain.Util.ClientPositionToWorldPosition(clientPosition);
			Vector2 vec = Durango.Terrain.Util.WorldPositionToTilePosition(clientPosition);
