...
public class RoutesViewer : MonoBehaviour, IUIInitializable
{
	...
	public void SelectUnstableRoutes([CanBeNull] RegionTemplate template)
	{
		...
		bool flag = false;
		flag = GameSystem<StatisticsSystem>.Instance().Level >= template.AvailableLevel;
		if (!flag)
		{
	...
	public Transform GetIslandTransoform(Role role, Biome biome, int level)
	{
		...
		if (_selectedTemplate == null)
		{
			if (flag)
			{
				return _worldViewer.GetUnstableRoutesTransform(role, biome, level);
			}
			return _worldViewer.GetIslandTransform(role, biome, level);
		}
		if (flag)
		{
			return _unstableViewer.GetIslandTransform();
		}
		return null;
	}
}
