...
public class FactionSupportRequestRewardListWidget : UIWidget
{
	...
	public void Set(string title, SupportRewards rewards, int friendshipPointReward)
	{
		...
		{
			_scroll.Nodes.GetNext().GetComponent<FactionSupportRequestRewardWidget>().Set(friendshipPointReward);
		}
		for (int i = 0; i < num; i++)
		{
			FactionSupportRequestRewardWidget component = _scroll.Nodes.GetNext().GetComponent<FactionSupportRequestRewardWidget>();
			if (i < size)
			{
				component.Set(rewards.Items[i]);
			}
			else
			{
				component.Set(rewards.Moneys[i - size]);
			}
		}
