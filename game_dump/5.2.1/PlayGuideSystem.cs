...
public class PlayGuideSystem : GameSystem<PlayGuideSystem>
{
	...
	private bool AllCompleted
	{
		get
		{
			...
			{
				if (!_flowStacks[i].Recoder.IsFinished())
				{
					return false;
				}
			}
	...
	{
		GuideRole currentGuideRole = _currentGuideRole;
		if (currentGuideRole == GuideRole.Tutorial || (uint)(currentGuideRole - 7) <= 1u || (uint)(currentGuideRole - 11) <= 1u)
		{
			return false;
		}
		return true;
	...
	private GuideStorageData CreateStorageData(bool common)
	{
		...
		{
			CompletedEvents = dictionary
		};
		if (!common)
		{
			Command.SaveDogGuideProgress(guideStorageData);
			guideStorageData.LastReturnerCount = _lastReturnerCount;
			guideStorageData.LastNomadCount = _lastNomadCount;
		}
	...
	private GuideStorageData LoadGuideStorageData(string key, [CanBeNull] Dictionary<string, byte[]> storage)
	{
		...
		if (UseLocalGuideProgress)
		{
			guideStorageData = Json.Read<GuideStorageData>(Preferences.GetString(key, string.Empty));
		}
	...
	public static string GetGuideConfig(GuideConfig config, GuideRole role)
	{
		...
		case GuideConfig.StorageKey:
			text2 = "guide_progress";
			break;
		case GuideConfig.EventFile:
			text = "PlayGuide/";
			text2 = "play_guide_event";
			break;
		case GuideConfig.FlowFile:
			text = "PlayGuide/";
			text2 = "play_guide_flow";
			break;
		}
		string text3 = role.ToString().ToLower();
		return text + text3 + "_" + text2;
	}
	...
	private GuideEvent FindEvent(string eventName)
	{
		...
		return _eventDictionary.Get(eventName);
	}
	...
	private static void CompleteEventToDo([NotNull] GuideEvent guideEvent)
	{
		if (guideEvent.ToDoCollection != null)
		{
			...
			{
				guideEvent.ToDoCollection.ToDoList[i].IsCompleted = true;
			}
	...
	private void TerrainA6_OnLoadingChunksFinished()
	{
		...
		{
			_remainFlowConditions[num2].TryRegister();
		}
		IsGuideBegin = true;
		if (this.Begun != null)
		{
	...
	private void CheckReturnerGuide()
	{
		...
		if (isReturner.HasValue)
		{
			...
			if (_isReturner.GetValueOrDefault())
			{
	...
	{
		FlowStack flowStack = FindFlowStack(flowName);
		if (flowStack != null && !flowStack.Completed)
		{
			return flowStack.Progressed;
		}
		return false;
	...
	{
		foreach (string item in container.List)
		{
			...
			foreach (ToDoBase toDo in guideEvent.ToDoCollection.ToDoList)
			{
				toDo.IsCompleted = false;
				toDo.CurrentProgress = 0;
			}
		}
	}
