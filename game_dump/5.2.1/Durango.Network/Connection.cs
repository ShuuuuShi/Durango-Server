...
public class Connection
{
	...
	private byte[] SendBuffer
	{
		get
		{
			if (_sendBufferIndex == 0)
			{
				return _socketSendBuffer1;
			}
			return _socketSendBuffer2;
		}
	}
	...
	{
		if (!(Mathf.Abs(_bufferPing - ping) < 0.05f))
		{
	...
	public ReplyMessageHandlerRegistrar Send<T>(T msg, bool noReply = false, uint replyOf = 0u)
	{
		...
		if (_hookList != null)
		{
			...
			{
				if (_hookList[num].HookSendingMessage(this, _sequenceNumber, msg, noReply, replyOf))
				{
					...
					return new ReplyMessageHandlerRegistrar(this, num2);
				}
			}
		}
		if (!Connected())
		...
		return new ReplyMessageHandlerRegistrar(this, num3);
	}
	...
	{
		if (_predictedServerTime.NeverSyncedYet)
		{
			return _latestReceivedTime.Time;
		}
		return _predictedServerTime.Time;
	}
	...
	private void StartSend()
	{
		...
		if (_sock != null && _sock.Connected && _socketSendEventArg != null)
		{
			...
			bool num = _sock.SendAsync(_socketSendEventArg);
	...
	private bool RegisterMessageHandlerToRegistry<T>(IDictionary<uint, PacketHandler> registry, MessageHandler<T> handler)
	{
		...
		bool num = registry.ContainsKey(typeCode);
		if (num)
		{
	...
	private bool NeedTimeSynchronization()
	{
		...
		{
			return (double)GetTime() - _predictedServerTime.LastGameTimeAtSynced >= 600.0;
		}
		return true;
	}
	...
	private void PingClockMessageHandler(Clock pass, PacketHeader header)
	{
		...
		float num2 = Ping / 2f;
		if (Math.Abs(pass.ServerTime + (double)num2 - _predictedServerTime.Time) > 0.30000001192092896)
		{
			ForceSyncClock();
		}
	}
	...
	public static bool TryParse(string address, out string host, out int port)
	{
		...
		return int.TryParse(address.Substring(host.Length + 1, address.Length - host.Length - 1), out port);
	}
}
