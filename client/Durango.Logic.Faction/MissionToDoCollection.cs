...
public class MissionToDoCollection : ToDoCollection
{
	...
	public void UpdateTodos(bool added)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			bool flag = !added && (missionToDo2.TargetProgress != missionToDo.GoalCount || missionToDo2.CurrentProgress != missionToDo.Progress || missionToDo2.LocalText != missionToDo.Label);
	...
	{
		return (!string.IsNullOrEmpty(_regionName)) ? ((SyncString)T._("<em>{0}</em> 섬에서 임무를 진행할 수 있습니다.", _regionName)) : base.GetMessage();
	}
